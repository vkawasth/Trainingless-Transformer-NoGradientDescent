...
beating a 24-layer GPT2-medium reference model (val~$=0.250$) trained for 300~CE steps,
...
\begin{theorem}[Fixed-Point Phase Equivariance]
\label{thm:fixed_point}
The phase gradient satisfies
$\partial L/\partial \varphi_k = 0$ when $\varphi_k \in \{0,\pi\}$,
and $\partial L/\partial \varphi_k \neq 0$ otherwise.
Concretely, at \texttt{basin\_state.pt} (phases $\{0,\pi,\pi,\pi,0\}$):
\begin{align*}
  k=0:&\quad |\partial L/\partial\varphi_0| = 7.6\times 10^{-5}
       \quad (\text{ratio }= 0.0013) \\
  k=1:&\quad |\partial L/\partial\varphi_1| = 2.3\times 10^{-3}
       \quad (\text{ratio }= 0.076)
\end{align*}
while layers 2--4 (same phases $\pi$ but higher val) have ratios $0.6$--$2.0$.

This is not global Morse--Bott equivariance but a \emph{fixed-point theorem}:
$\{0,\pi\}$ are the fixed points of the $U(1)$ action on the Bridgeland
phase circle, and the gradient vanishes there by the symmetry
$L(\varphi_k) = L(-\varphi_k)$ (reflection symmetry of the loss at clean phases).
The walls $\mathcal{W}_k = \{f_k = 0\}$ are exactly the zero sets of
$\partial L/\partial\varphi_k$, confirming that the wall geometry is
determined by the fixed-point structure of the phase action.
\end{theorem}
...
\paragraph{Stage IV revision.}
The global Morse--Bott hypothesis fails.
The correct Stage IV theorem is the Fixed-Point Phase Equivariance
(Theorem~\ref{thm:fixed_point}):
$\partial L/\partial\varphi_k = 0$ at $\varphi_k \in \{0,\pi\}$,
not globally.
This is weaker than Morse--Bott but sufficient for the codimension-2
theorem: the wall $\mathcal{W}_k = \{f_k = 0\}$ is the zero set of
$\partial L/\partial\varphi_k$, and the gradient vanishes there
by the reflection symmetry $L(\varphi_k) = L(-\varphi_k)$
at the fixed points $\{0,\pi\}$ of the $U(1)$ phase action.

\paragraph{Computational cost.}
The gradient computation required $2 \times 2 \times 256^2 = 262{,}144$
finite-difference evaluations of $\varphi_k$, each involving a
$256 \times 256$ matrix inverse and eigenvalue computation.
...
ratio $= -0.996, +0.977, -1.004, -1.008$ for all four checkpoints:
