...
class LowMemBergmanWitness:
    ...
    CPU-offloaded Trajectory Tensors, and MaxMin Landmark Sampling.
    """
    ...
    def extract_witness_complex(self, raw_trajectory: list, losses: torch.Tensor, top_k: int = 6):
        ...
        # 1. JL Subspace Projection via Fixed Sketch Matrix
        ...
        r_cutoff = torch.median(sorted_dists[:, 1] - sorted_dists[:, 0]).item() * 1.5 + 1e-4

        # 4. Build Witness Simplices
...
# =====================================================================
# OPTIMIZER & TRANSFORMER ARCHITECTURE
# =====================================================================
...
def OOM_safe_hvp(model, inputs, targets, v):
    """
    Computes HVP by locally re-evaluating forward/backward passes.
    Prevents autograd RuntimeError and graph memory growth across iterations.
    """
    with torch.enable_grad():
        logits = model(inputs)
