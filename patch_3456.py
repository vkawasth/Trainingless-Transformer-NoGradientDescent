#!/usr/bin/env python3
"""
patch_3456.py -- PHASE 3: COMPRESSION + LEAKAGE + SIGN + FISHER-WEIGHTED COMPLEMENT
====================================================================================
...
the K0 split descent are untouched.
  ...
  (6) FISHER-WEIGHTED COMPLEMENT              <-- new in this patch
...
VERIFIED CONFIGURATION
    ...
    python3 patch_3456.py --no-trigger --k 8 --win 24 --geoval 0.6 --run
...
USAGE
    python3 patch_3456.py --no-trigger --k 8 --win 24 --geoval 0.6 --run      # verified
    python3 patch_3456.py --no-trigger --k 8 --win 24 --geoval 0.6 --fisher --run
    python3 patch_3456.py --k 8 --win 24 --geoval 0.6 --theta 0.96 --verbose --run
    python3 patch_3456.py --no-trigger --k 8 --win 24 --geoval 0.6 --fisher --beta 0.5 --run
...
def _sub_project(th_before, step):
    ...
    if _SUB_FISHER and _sub_QF is not None:
        ...
        nu = nu + _SUB_BETA * dF
        for k, ii in _sub_idx.items():
            nu[ii] = nu[ii] + _SUB_ALPHA.get(k, 0.1) * dR[ii]
        if _SUB_ACCUM:
            for k, ii in _sub_idx.items():
                _sub_bank[ii] += (1.0 - _SUB_ALPHA.get(k, 0.1)) * dR[ii]
    else:
        for k, ii in _sub_idx.items():
            nu[ii] = nu[ii] + _SUB_ALPHA.get(k, 0.1) * drop_full[ii]
            if _SUB_ACCUM:
    ...
    if _SUB_VERBOSE and step % 16 == 0:
        ...
        print(f"    [3456] step {step:3d}  capture {_np.mean(_sub_stats['cap'][-16:]):.3f}"
...
def _sub_report():
    ...
    print(f"  [3456] dims {_sub_stats['dims']} (asked __DIMS__)/{_sub_P}  coast:{mode}  "
...
def main():
    ...
    ap.add_argument("--out", default="compiler_phase3456.py")
