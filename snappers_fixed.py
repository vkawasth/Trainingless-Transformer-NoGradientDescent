...
from anchor_probe import AnchorRegistry
...
def action_p_adic_bowl(state):
    ...
    # REPLACED: the previous quartic-fit + wide-grid-search + unchecked-Newton
    # mechanism had three compounding bugs, confirmed numerically on a real
    # run's fitted polynomial:
    #   1. It read a4<0 (negative leading coeff) as "bowl detected" -- for a
    #      quartic that actually means L(t)->-infinity at the edges, i.e.
    #      UNBOUNDED, the opposite of a bowl.
    #   2. The grid+Newton search scanned t in [-0.5, 2.0] while only 5
    #      points in [0, 0.6] were ever measured -- guaranteed extrapolation.
    #   3. Newton's method was never checked against the second derivative,
    #      so it could (and in the traced example, did) converge to a local
    #      MAXIMUM of the fitted curve and report it as "the minimum".
    # Fix: reuse AnchorRegistry.bisect_probe + refine_with_polynomial, which
    # already (a) only trusts a critical point strictly inside the tested
    # range, and (b) picks the lowest FITTED value among in-range candidates
    # -- so it can't wander off to an untested extrapolated region.
    print("    Evaluating loss along direction (guarded polynomial search)...")
    gap_registry = AnchorRegistry()
    gap_key = ('p_adic_bowl', best_p, round(v0_val, 3))

    def probe_bowl_direction(t):
        ...
        print(f"      t={t:.4f}: val={v:.4f}, phi={phi}/5")
        return {'val': v, 'phi': phi, 'tau': tau, 'rm2sigma': rm2}

    gap_registry.bisect_probe(gap_key, probe_bowl_direction, lo=0.01, hi=0.6,
                               max_probes=6, val_ref=v0_val)
    best_t, degree = gap_registry.refine_with_polynomial(gap_key, prefer_cubic=True)
    best_anchor = gap_registry.best_anchor(gap_key)

    if best_t is not None:
        refined = probe_bowl_direction(best_t)
        if refined['val'] < best_anchor.metrics['val']:
            t_star = best_t
        else:
            t_star = best_anchor.param_value
    else:
        t_star = best_anchor.param_value
    ...
    print(f"    Jump: val {v0_val:.4f} → {v_new:.4f} (t*={t_star:.4f}, degree={degree})")
    print(f"    phi: {phi0}/5 → {phi_new}/5")
    print(f"    tau: {tau0:.2f} → {tau_new:.2f}")

    model_try.set_flat(w0)  # reset before deciding, jump applied only on accept
...
def main():
    ...
    # If val is still not good enough, run the anchor-probe guided finish
    # instead of a flat 400-step fixed-LR grind. REPLACES the previous
    # `run_gradient_descent(model, steps=400, lr=LR)` pattern -- same one
    # flagged repeatedly in this compiler family (compiler_topo_geom_1.py,
    # compiler_curvature.py) as the part doing the most "wandering" per
    # step. Reuses the SAME AnchorRegistry/bisect_probe/refine_with_polynomial
    # machinery already validated (synthetic test recovered a known optimum
    # to within 0.001) rather than inventing a new search here.
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        print("\n" + "=" * 70)
        print("RUNNING ANCHOR-PROBE GUIDED FINISH (replaces fixed-LR GD-400)")
        print("=" * 70)

        model = final_state.model
        registry = AnchorRegistry()
        MAX_ROUNDS = 8
        STEPS_PER_ROUND = 50  # a "round" is a short burst at one probed LR mult, not 400 fixed

        def probe_lr_mult(lr_mult, n_steps=STEPS_PER_ROUND):
            """Run n_steps of AdamW at LR*lr_mult from the CURRENT model,
            starting from a snapshot so failed probes don't corrupt state."""
            snapshot = model.flat_params().clone()
            opt = torch.optim.AdamW(model.parameters(), lr=LR * lr_mult,
                                     betas=(0.9, 0.95), weight_decay=0.1)
            ...
            phi = phi_clean(model)
            tau = gluing_defect(model, n=4)
            rm2 = 0.7 if (phi >= PHI_CLEAN_TARGET - 1) else 0.3
            metrics = {'val': v, 'phi': phi, 'tau': tau, 'rm2sigma': rm2}
            # Undo the probe -- only the WINNING lr_mult (chosen below) gets
            # actually applied for real, so probing doesn't waste the model's
            # actual trajectory on a mult that turns out bad.
            probe_state = model.flat_params().clone()
            model.set_flat(snapshot)
            return metrics, probe_state

        # Cache each probe's resulting parameters so we can jump straight to
        # the winning one instead of re-running it.
        probe_cache = {}

        def probe_and_cache(lr_mult):
            metrics, probe_state = probe_lr_mult(lr_mult)
            probe_cache[lr_mult] = probe_state
            return metrics

        val_ref0 = final_state.val
        for rnd in range(1, MAX_ROUNDS + 1):
            state_key = (int(eval_val(model, n=4) / 0.1), phi_clean(model),
                         int(gluing_defect(model, n=4) / 0.5), 'finish')
            created = registry.bisect_probe(state_key, probe_and_cache,
                                             lo=0.3, hi=15.0, max_probes=6,
                                             val_ref=val_ref0)
            for a in created:
                print(f"    round {rnd} probe lr_mult={a.param_value:.3f}: "
                      f"val={a.metrics['val']:.4f} phi={a.metrics['phi']}/5 "
                      f"cls={a.classification}")

            best_pv, degree = registry.refine_with_polynomial(state_key, prefer_cubic=True)
            best_anchor = registry.best_anchor(state_key)
            if best_pv is not None and best_pv not in probe_cache:
                refined_metrics = probe_and_cache(best_pv)
                print(f"    round {rnd} guarded refine (degree={degree}): "
                      f"lr_mult≈{best_pv:.3f} val={refined_metrics['val']:.4f}")
                if refined_metrics['val'] < best_anchor.metrics['val']:
                    winner = best_pv
                else:
                    winner = best_anchor.param_value
            else:
                winner = best_anchor.param_value

            model.set_flat(probe_cache[winner])
            v = eval_val(model, n=8); phi = phi_clean(model); tau = gluing_defect(model, n=4)
            print(f"    round {rnd} committed: lr_mult={winner:.3f} -> "
                  f"val={v:.4f} phi={phi}/5 tau={tau:.2f}")

            if v < VAL_FLOOR and phi == PHI_CLEAN_TARGET:
                print(f"    ✓ Reached target after round {rnd}")
                break
            if v >= val_ref0 * 0.999:
                print(f"    ~ No meaningful improvement this round, stopping")
                break
            val_ref0 = v
        ...
        phi_final = phi_clean(model)
        tau_final = gluing_defect(model, n=6)
        print(f"    Anchor-probe finish final: val={v_final:.4f}, phi={phi_final}/5, tau={tau_final:.2f}")

        print("\n" + "=" * 70)
        print("FINAL RESULTS AFTER ANCHOR-PROBE GUIDED FINISH")
        print("=" * 70)
        print(f"  Final val:     {v_final:.4f}")
        print(f"  Final phi:     {phi_final}/5")
        print(f"  Final tau:     {tau_final:.2f}")
        print(f"  Path:          {' → '.join(final_state.action_history)} → anchor_probe_finish")
