    ...
    if donor is not None:
        ...
        with torch.no_grad():
            for n_,p_ in snamed:
                ...
                if n_ not in S["W"]: continue
                _dw=S["W"][n_]-p_.data
                _sg=torch.sign(_dw); _al=float(_dw.abs().mean())
                _eps=_dw-_al*_sg
                _as+=int((torch.sign(_eps)==_sg).sum()); _an+=_dw.numel()
        if _an: print(f"    agree(sgn(eps), sgn(dW)) = {_as/_an:.4f}  "
                      f"(construction: = frac with |dW|>alpha)",flush=True)
        for _k in S["W"]:
            if _k in S["m"]:
        ...
        with torch.no_grad():
            for n,p in snamed:
                for bi in range(NB):
                    ...
                    if crit in ("lowrank","resid","sign","signN","gsign","msign","randN","lrN","lrT","supT","supN","rowT","rowN","resT","resN","magT","magN","predN","rowscale","colscale","spectrum"):
                        ...
                        if crit=="sign":
                        ...
                        elif crit in ("supT","supN"):
                            # support+sign: keep sign on the top-frac |dW|
                            # coordinates, zero elsewhere. NULL keeps the same
                            # NUMBER of coordinates chosen at random.
                            ...
                            M=torch.zeros(dW.numel()); M[idx]=1.0; M=M.view_as(dW)
                            sg=torch.sign(dW)*M
                            p.data.add_(sg*(dW.norm()/max(float(sg.norm()),1e-30)))
                        elif crit in ("rowT","rowN"):
                            # row-shared sign: one sign per ROW (majority),
                            # broadcast. NULL randomises which row gets which sign.
                            if dW.dim()!=2: continue
                            rs=torch.sign(torch.sign(dW).sum(1,keepdim=True))
                            rs[rs==0]=1.0
                            if crit=="rowN":
                                g_=torch.Generator().manual_seed(RSEED+hash(n)%10000)
                                rs=rs[torch.randperm(rs.shape[0],generator=g_)]
                            sg=rs.expand_as(dW)
                            p.data.add_(sg*(dW.norm()/max(float(sg.norm()),1e-30)))
                        elif crit in ("magT","magN","predN"):
                            # MAGNITUDE-ONLY: keep |dW| but strip orientation by
                            # imposing a FIXED canonical sign (all +1), so no
                            # information from sgn(dW) can leak in.
                            # NULL: same magnitude multiset, coordinates shuffled.
                            mag=dW.abs()
                            if crit=="magN":
                                g_=torch.Generator().manual_seed(RSEED+hash(n)%10000)
                                mag=mag.reshape(-1)[torch.randperm(mag.numel(),generator=g_)].view_as(mag)
                            p.data.add_(mag*(dW.norm()/max(float(mag.norm()),1e-30)))
                        elif crit in ("resT","resN","magT","magN","predN"):
                            # residual: dW minus its sign component, i.e. what
                            # orientation does NOT explain. NULL randomises signs
                            # before subtracting.
                            sg=torch.sign(dW)
                            a=float((dW*sg).sum()/max(float((sg*sg).sum()),1e-30))
                            res=dW-a*sg
                            if crit=="resN":
                                g_=torch.Generator().manual_seed(RSEED+hash(n)%10000)
                                res=res.reshape(-1)[torch.randperm(res.numel(),generator=g_)].view_as(res)
                            p.data.add_(res*(dW.norm()/max(float(res.norm()),1e-30)))
                        elif crit=="lrN":
                            # NULL for low-rank: project onto a RANDOM rank-r
                            # subspace, norm-matched. Same dose, same rank, no
                            # teacher-specific directions.
                            r=max(1,int(frac))
                            g_=torch.Generator().manual_seed(RSEED+hash(n)%10000)
                            A=torch.randn(dW.shape[0],r,generator=g_)
                            B=torch.randn(r,dW.shape[1],generator=g_)
                            Qa,_=torch.linalg.qr(A); Qb,_=torch.linalg.qr(B.T)
                            LR=Qa@(Qa.T@dW@Qb)@Qb.T
                            p.data.add_(LR*(dW.norm()/max(float(LR.norm()),1e-30)))
                        elif crit=="lrT":
                            # teacher low-rank, norm-matched to the same dose
                            r=max(1,int(frac))
                            ...
                            # prospective predictor: sgn(-m) at the donor step,
                            # norm-matched exactly like signN. Its null is randN.
                            mm=S["m"].get(src)
