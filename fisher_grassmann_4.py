...
# =====================================================================
# 2. ADAPTIVE KRYLOV-NEWTON OPTIMIZER (SOFT-FLOOR NORM SCHEDULER)
# =====================================================================
class AdaptiveKrylovSubspaceSignAdam:
    def __init__(
        ...
        lr_ortho=2e-3,         # Unchoked complement scale
        beta_ema=0.75,         # H* = 4 EMA gradient filter
        beta1_ortho=0.95,      # High-smoothing momentum for g_perp chatter
        beta2_ortho=0.999, 
        eps=1e-8, 
        k_dim=3,
        soft_floor_ratio=0.50  # Soft floor trigger: clip ONLY if R_norm < 0.50
    ):
    ...
    def step(self, Q_K, T_K=None):
        ...
        # Dynamic Soft-Floor Norm Scheduler:
        # Unchoke complement unless R_norm falls below soft_floor_ratio (0.50)
        ...
        return total_update, g_raw, kn_vec, ortho_vec, final_norm_ratio
...
def main():
    ...
    # Config: lr_kn = 0.50, beta1_ortho = 0.95, soft_floor_ratio = 0.50
    ...
    print("=" * 120)
    print(f" INTEGRATED FISHER-GRASSMANN OPTIMIZER (P={P}, k={K_DIM}) | DYNAMIC SOFT-FLOOR NORM SCHEDULER")
    print(" Config: lr_kn = 0.50 | lr_ortho = 2e-3 (Unchoked down to floor=0.50) | beta1_ortho = 0.95")
    print("=" * 120)
