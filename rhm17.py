...
ap.add_argument("--construct-emb", action="store_true",
                help="compute the order coordinate from CORPUS COUNTS and write "
                     "it into te.weight column 0; that column is then frozen")
ap.add_argument("--aux-levels", default="",
                help="comma-separated grammar levels to predict as an auxiliary "
                     "task from h2[t], e.g. 3,4. Labels come from the grammar.")
ap.add_argument("--aux-weight", type=float, default=1.0)
ap.add_argument("--fresh-sample", action="store_true",
                help="draw training batches from the grammar instead of the "
                     "fixed train_ids file. --aux-levels needs this (the latent "
                     "parse is not in the file), so the CONTROL arm must use it "
                     "too or the aux arm gets unlimited data for free.")
ap.add_argument("--freeze-emb", action="store_true",
                help="freeze the whole embedding matrix, not just column 0")
...
AUX = [int(x) for x in a.aux_levels.split(",") if x.strip()] if a.aux_levels else []
FRESH = a.fresh_sample or bool(AUX)
if FRESH:
    _R = {int(l): {int(i): [tuple(t) for t in v] for i, v in d.items()}
          for l, d in META["rules_all"].items()}
    _LV, _VS = META["depth"], META["nsym"]

    def sample_with_parse(nb):
        """Draw sequences from the grammar directly, recording the level-l
        symbol above every leaf. Needed because the training stream is a flat
        list of ids with the latent parse discarded."""
        X = torch.zeros(nb, SEQ, dtype=torch.long)
        Ynode = {l: torch.zeros(nb, SEQ, dtype=torch.long) for l in AUX}
        for b in range(nb):
            def go(sym, l, off):
                if l in Ynode:
                    for t in range(off, off + 2 ** l): Ynode[l][b, t] = sym
                if l == 0: return [sym]
                pr = _R[l][sym]; x, y = pr[torch.randint(len(pr), (1,)).item()]
                return go(x, l - 1, off) + go(y, l - 1, off + 2 ** (l - 1))
            leaves = go(torch.randint(_VS, (1,)).item(), _LV, 0)
            X[b] = torch.tensor(leaves)
        return X, Ynode
...
class Block(nn.Module):
    ...
    def forward(s, x, need_attn=False, taps=None):
        o, att = s.at(s.ln(x), need_attn)
        x = x + o
        if taps is not None: taps.append(x)          # post-attention
        if s.mlp is not None:
            x = x + s.mlp(s.ln2(x))
            if taps is not None: taps.append(x)      # post-MLP
        return x, att
...
class LM2(nn.Module):
    ...
    def forward(s, idx, targets=None, need_attn=False, need_hidden=False):
        ...
        hs, atts, taps = [], [], []
        for blk in s.blocks:
            h, at = blk(h, need_attn, taps if need_hidden else None)
        ...
        if need_hidden:
            ...
            for i, t in enumerate(taps):
                out[f"tap{i}"] = t                   # attn0, mlp0, attn1, mlp1
        return out
...
@torch.no_grad()
def sublayer_localisation(model):
    """WHERE is the order distinction built?

    The frozen-embedding arm showed the structure appears at full strength with
    te.weight never updated, so it is constructed in the blocks. This reads the
    residual stream at every sublayer boundary instead of only after each
    block, and reports D_ord - D_pos at each. The sublayer where the gap first
    jumps is where the distinction is introduced.

    taps, in order: attn0, mlp0, attn1, mlp1
    Reported against the embedding itself (tap "emb"), which for a frozen
    random matrix should show nothing."""
    names = ["emb", "attn0", "mlp0", "attn1", "mlp1"]
    f, r = _probe_batch(META["C_ord"])
    fp, rp = _probe_batch(META["C_unord"])
    of, orv = model(f, need_hidden=True), model(r, need_hidden=True)
    op, orp = model(fp, need_hidden=True), model(rp, need_hidden=True)
    def emb(idx):
        return model.te(idx) + model.pe(torch.arange(idx.shape[1]))[None]
    print("\n  SUBLAYER LOCALISATION  (D_ord - D_pos, cosine, at each tap)")
    prev = None
    for i, nm in enumerate(names):
        if nm == "emb":
            A, B = emb(f)[:, -1], emb(r)[:, -1]
            C, D_ = emb(fp)[:, -1], emb(rp)[:, -1]
        else:
            k = f"tap{i-1}"
            if k not in of: continue
            A, B = of[k][:, -1], orv[k][:, -1]
            C, D_ = op[k][:, -1], orp[k][:, -1]
        do = (1 - F.cosine_similarity(A, B, dim=-1)).numpy()
        dp = (1 - F.cosine_similarity(C, D_, dim=-1)).numpy()
        mo, so = _pairstats(do); mp, sp = _pairstats(dp)
        gap = mo - mp; sem = math.sqrt(so**2 + sp**2)
        inc = "" if prev is None else f"   step {gap-prev:+.4f}"
        prev = gap
        print(f"    {nm:<7} D_ord={mo:.4f}+/-{so:.4f}  D_pos={mp:.4f}+/-{sp:.4f}"
              f"  gap={gap:+.4f}+/-{sem:.4f} ({gap/sem if sem>0 else float('nan'):.1f} SEM)"
              + inc)
    print("    the tap where `step` first jumps is where the order distinction")
    print("    is introduced; a flat `emb` row means the input basis carries none")
...
opt = torch.optim.AdamW(model.parameters(), lr=a.lr, betas=(0.9, 0.95),
                        weight_decay=0.01)
def construct_order_coordinate():
    """Build the order coordinate from the CORPUS, not from the grammar and not
    by gradient descent.

      1. count adjacent (even -> odd) leaf pairs: these are the level-1
         productions, observable without knowing the rules
      2. keep pairs seen often enough to be real edges
      3. transitive closure (Warshall)
      4. topological rank = number of tokens below you in the closure

    The closure is what the trained model takes 48k steps to place in te.weight
    as a single linearly-decodable direction. Here it costs one pass over the
    training ids. If the constructed coordinate matches the trained model's
    loss, those 48k steps of embedding learning were redundant."""
    import collections, time
    t0 = time.time()
    ids = json.load(open(f"{a.data}/train_ids.json"))
    cnt = collections.Counter()
    for i in range(0, len(ids) - 1, 2):            # level-1 sibling pairs
        cnt[(ids[i], ids[i+1])] += 1
    thr = max(2, int(np.percentile(list(cnt.values()), 50)))
    E = np.zeros((NLEAF, NLEAF), dtype=bool)
    for (x, y), c in cnt.items():
        if c >= thr and x != y: E[x, y] = True
    E &= ~E.T                                       # drop symmetric (unordered) pairs
    C = E.copy()
    for k in range(NLEAF): C |= np.outer(C[:, k], C[k, :])
    np.fill_diagonal(C, False)
    rank = C.sum(0).astype(float)                   # how many tokens precede you
    if rank.max() > rank.min():
        rank = (rank - rank.mean()) / (rank.std() + 1e-8)
    print(f"  [construct] {int(E.sum())} edges (threshold {thr}), closure "
          f"{int(C.sum())} pairs, built in {time.time()-t0:.2f}s")
    if META.get("prec"):
        P = np.array(META["prec"], dtype=bool)
        tp = int((C & P).sum()); fp = int((C & ~P).sum()); fn = int((~C & P).sum())
        print(f"  [construct] vs ground truth: precision={tp/max(tp+fp,1):.3f} "
              f"recall={tp/max(tp+fn,1):.3f}")
    return torch.tensor(rank, dtype=torch.float32)

if a.construct_emb:
    z = construct_order_coordinate()
    with torch.no_grad():
        model.te.weight[:NLEAF, 0] = z * model.te.weight[:, 1:].std()
    col = torch.ones_like(model.te.weight); col[:, 0] = 0.0
    model.te.weight.register_hook(lambda g, m=col: g * m)   # column 0 frozen
    print("  [construct] order coordinate written to te.weight[:,0] and frozen")
if a.freeze_emb:
    model.te.weight.requires_grad_(False)
    print("  [construct] whole embedding frozen")

if AUX:
    for l in AUX:
        model.aux[str(l)] = nn.Linear(a.d_model, META["nsym"])
    print(f"  [aux] predicting grammar levels {AUX} from h2[t], "
          f"weight {a.aux_weight}")
...
for step in range(1, a.steps + 1):
    if FRESH:
        x, ynode = sample_with_parse(a.batch)
        yt = torch.cat([x[:, 1:], torch.full((x.shape[0], 1), IGNORE)], 1)
        out = model(x, yt, need_hidden=bool(AUX))
        loss = out["loss"]
        h = out.get("h2")
        aux_tot = 0.0
        for l in AUX:
            lg = model.aux[str(l)](h)                 # B x T x nsym
            aux_tot = aux_tot + F.cross_entropy(
                lg.reshape(-1, lg.shape[-1]), ynode[l].reshape(-1))
        if AUX: loss = loss + a.aux_weight * aux_tot
    else:
        x, y = get_batch()
        loss = model(x, y)["loss"]
