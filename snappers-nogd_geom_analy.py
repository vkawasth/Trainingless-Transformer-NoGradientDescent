#!/usr/bin/env python3
"""
Direction Analysis Compiler
===========================
Analyzes whether GD, Hessian, and Rare token directions align.
Determines if a direct jump is geometrically possible.

Total CE: ~5 (DP) + direction_analysis (~100 CE) = ~105 CE
"""
...
# DIRECTION ANALYSIS
# ============================================================================
def direction_analysis(model, run_gd=True):
    """
    Analyze whether GD, Hessian, and Rare token directions align.
    Determines if a direct jump is geometrically possible.
    """
    print("\n" + "="*70)
    ...
    # 1. GD direction (run 400 steps and track the total change)
    print("\n[1] Running GD for 400 steps (LR=0.003)...")
    model_gd = copy.deepcopy(model)
    w0 = model_gd.flat_params()
    opt = torch.optim.AdamW(model_gd.parameters(), lr=0.003,
                            betas=(0.9, 0.95), weight_decay=0.1)
    ...
    for step in range(1, 401):
        ...
        if step % 50 == 0:
            ...
            tau = gluing_defect(model_gd, n=4)
            phi = phi_clean(model_gd)
            gd_val_history.append(v)
            gd_tau_history.append(tau)
            gd_phi_history.append(phi)
            print(f"      GD step {step:3d}: val={v:.4f}, τ={tau:.2f}, φ={phi}/5")
    
    w_gd = model_gd.flat_params()
    gd_direction = w_gd - w0
    gd_norm = gd_direction.norm()
    gd_direction = gd_direction / max(gd_norm, 1e-10)
    print(f"\n    GD direction norm: {gd_norm:.4f}")
    print(f"    GD final val: {gd_val_history[-1]:.4f}")
    ce_used += 400 * 4  # Each step has eval
    
    # 2. Hessian direction (smallest eigenvector)
    print("\n[2] Computing Hessian direction (smallest eigenvector)...")
    n_p = sum(p.numel() for p in model.parameters())
    ...
    hessian_direction = v / max(v.norm(), 1e-10)
    print(f"    Hessian direction norm: {hessian_direction.norm():.4f}")
    ce_used += 10 * 4
    
    # 3. Rare token direction
    print("\n[3] Computing Rare token direction...")
    token_counts = {}
    ...
    print(f"      Rare tokens: {len(rare_tokens)} / {VOCAB}")
    ...
    direction_emb = torch.zeros_like(model.te.weight)
    for t in rare_tokens:
        direction_emb[t] = emb_grad[t] * weights[t]
    ...
    for t in active_tokens:
        if t not in rare_tokens:
            direction_emb[t] = emb_grad[t] * 0.5
    
    rare_direction = torch.zeros_like(model.flat_params())
    idx = 0
    for name, param in model.named_parameters():
        if name == 'te.weight':
            rare_direction[idx:idx + direction_emb.numel()] = direction_emb.flatten()
        idx += param.numel()
    rare_direction = rare_direction / max(rare_direction.norm(), 1e-10)
    print(f"    Rare direction norm: {rare_direction.norm():.4f}")
    
    # 4. Compare directions
    print("\n[4] Direction comparison...")
    
    cos_gd_hessian = float((gd_direction * hessian_direction).sum())
    cos_gd_rare = float((gd_direction * rare_direction).sum())
    cos_hessian_rare = float((hessian_direction * rare_direction).sum())
    
    print(f"    cos(GD, Hessian) = {cos_gd_hessian:.4f}")
    print(f"    cos(GD, Rare)    = {cos_gd_rare:.4f}")
    print(f"    cos(Hessian, Rare) = {cos_hessian_rare:.4f}")
    
    # 5. Project GD direction onto Hessian and Rare
    print("\n[5] Direction alignment...")
    gd_proj_hessian = cos_gd_hessian * hessian_direction
    gd_proj_rare = cos_gd_rare * rare_direction
    gd_orthogonal = gd_direction - gd_proj_hessian - gd_proj_rare
    
    print(f"    GD projection onto Hessian: {gd_proj_hessian.norm():.4f}")
    print(f"    GD projection onto Rare:    {gd_proj_rare.norm():.4f}")
    print(f"    GD orthogonal component:    {gd_orthogonal.norm():.4f}")
    
    # 6. Interpret
    print("\n[6] Interpretation:")
    if cos_gd_hessian > 0.5:
        print("    ✓ GD follows Hessian direction - Hessian jump would work")
        hessian_jump_likely = True
    else:
        print(f"    ✗ GD does NOT follow Hessian (cos={cos_gd_hessian:.3f})")
        hessian_jump_likely = False
    
    if cos_gd_rare > 0.5:
        print("    ✓ GD follows Rare token direction")
        rare_jump_likely = True
    else:
        print(f"    ✗ GD does NOT follow Rare direction (cos={cos_gd_rare:.3f})")
        rare_jump_likely = False
    
    if cos_hessian_rare > 0.5:
        print("    ✓ Hessian and Rare directions are aligned")
    else:
        print(f"    ✗ Hessian and Rare directions are NOT aligned (cos={cos_hessian_rare:.3f})")
    
    # 7. What drives the descent?
    print("\n[7] What drives the descent?")
    if cos_gd_hessian > 0.7:
        print("    → Hessian direction is the primary driver")
        print("    → A single Hessian jump should work")
        best_direction = 'hessian'
    elif cos_gd_rare > 0.7:
        print("    → Rare token direction is the primary driver")
        print("    → Rare token jump should work")
        best_direction = 'rare'
    elif cos_gd_hessian > 0.4 and cos_gd_rare > 0.4:
        print("    → Both Hessian and Rare directions contribute")
        print(f"    → cos(GD, Hessian)={cos_gd_hessian:.3f}, cos(GD, Rare)={cos_gd_rare:.3f}")
        print("    → A combined jump may work")
        best_direction = 'combined'
    else:
        print("    → Neither Hessian nor Rare direction dominates")
        print("    → GD works because it accumulates many small corrections")
        print(f"    → The orthogonal component is {gd_orthogonal.norm():.4f}")
        if gd_orthogonal.norm() > 1.0:
            print("    → The orthogonal component is large - GD is NOT following a single direction")
            print("    → Direct jump is unlikely to work")
        best_direction = 'none'
    
    return {
        'cos_gd_hessian': cos_gd_hessian,
        'cos_gd_rare': cos_gd_rare,
        'cos_hessian_rare': cos_hessian_rare,
        'gd_orthogonal': gd_orthogonal.norm(),
        'best_direction': best_direction,
        'hessian_jump_likely': hessian_jump_likely,
        'rare_jump_likely': rare_jump_likely,
        'gd_val_history': gd_val_history,
        'gd_tau_history': gd_tau_history,
        'gd_phi_history': gd_phi_history,
        'ce_used': ce_used
    }
...
def get_smart_actions(state, analyzer):
    ...
    # DIRECTION ANALYSIS: when near floor and phi >= 4
    if near_floor and phi >= 4:
        actions.append(('direction_analysis', {}))
        print(f"    → direction_analysis: val={val:.4f}, phi={phi}/5")
...
# ============================================================================
def action_direction_analysis(state):
    """
    Direction analysis action - runs the analysis and returns results.
    """
    model = state.model
    v0_val = state.val
    phi0 = state.phi
    ...
    results = direction_analysis(model)
    ce_used = results['ce_used']
    
    print(f"\n    Direction analysis complete:")
    print(f"      cos(GD, Hessian) = {results['cos_gd_hessian']:.4f}")
    print(f"      cos(GD, Rare)    = {results['cos_gd_rare']:.4f}")
    print(f"      cos(Hessian, Rare) = {results['cos_hessian_rare']:.4f}")
    print(f"      Best direction: {results['best_direction']}")
    print(f"      CE used: {ce_used}")
    
    # Store results in state for later use
    state.direction_results = results
    
    return CompilerState(state.model, state.step + 1, 'direction_analysis',
                        state.val, state.phi, state.tau,
                        state.action_history + ['direction_analysis'],
                        parent=state)
...
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'direction_analysis':
            return action_direction_analysis(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("SMART DP COMPILER WITH DIRECTION ANALYSIS")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'compiler_direction_analysis_gd_final.pt')
        print(f"\n  Saved: compiler_direction_analysis_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'compiler_direction_analysis_final.pt')
        print(f"\n  Saved: compiler_direction_analysis_final.pt")
