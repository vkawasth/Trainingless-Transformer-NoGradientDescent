#!/usr/bin/env python3
"""ESTIMATOR5 -- stagewise fit: soft counts, vertical pass, per-level confidence.

WHAT CHANGED AND WHY
--------------------
estimator4 at 20k sequences recovered 79% of the available block dependence at
level 1, 1.4% at level 2, 3.9% at level 3 (V7). Two causes, both in the INIT:

  (a) the hand-off was soft in transit and HARD on arrival. The posterior was
      propagated up correctly, then collapsed with argmax before counting:
          hard = soft.argmax(-1); pid = hard[0::2]*c + hard[1::2]
      Harmless at level 1 (children are observed leaves, argmax exact), fatal
      above it (the posterior is diffuse). Now the pair counts are the expected
      counts  C[x,y] += sum_nodes q_lo(x) q_hi(y)  -- no argmax anywhere.

  (b) levels with few nodes per sequence (2 at level 3, 1 at level 4) had no
      horizontal context, so they fell back to child identity. Those nodes ARE
      each other's context, through the root. The VERTICAL pass re-clusters
      such a level using the full posterior (inside x outside) from the levels
      already fixed below it.

STAGEWISE, NO BACK-CORRECTION: level l is fitted with levels < l frozen, and
is itself frozen before level l+1 starts. A level is only committed if its
posterior is determined enough:  mean H(q_l) < --freeze-h  (against ln V).
Otherwise the climb stops -- an undetermined level is not committed to noise.

DISJOINTNESS INVARIANT: every statistic compares functions of DISJOINT leaf
sets. Outside messages may inform the FIT (they are the exact posterior), but
the V7 decoder stays inside-only, or neighbouring blocks would share evidence
and look dependent under any model.
"""
...
ap.add_argument("--stagewise", action="store_true",
                help="fit level by level with lower levels frozen (no back-correction)")
ap.add_argument("--stage-sweeps", type=int, default=8, help="EM sweeps per stage")
ap.add_argument("--freeze-h", type=float, default=0.0,
                help="commit a level only if mean posterior entropy < this "
                     "(nats; 0 = always commit, just report)")
ap.add_argument("--vertical", action="store_true",
                help="re-cluster thin levels using inside x outside (the levels "
                     "below are already frozen, so this adds no back-correction)")
...
def soft_pair_counts(q):
    """expected counts over child pairs from the POSTERIORS, no argmax:
       C[x,y] = sum_nodes q_lo(x) q_hi(y)"""
    lo, hi = q[:, 0::2], q[:, 1::2]
    B, n, cx = lo.shape; cy = hi.shape[2]
    lo2, hi2 = lo.reshape(-1, cx), hi.reshape(-1, cy)
    acc = np.zeros((cx, cy))
    for st in range(0, lo2.shape[0], a.chunk):
        acc += lo2[st:st+a.chunk].T @ hi2[st:st+a.chunk]
    return acc

for l in ([] if a.load_P else range(1, L+1)):
    c = card(l)
    lo_s, hi_s = soft[:, 0::2], soft[:, 1::2]
    nn_ = lo_s.shape[1]
    Cpair = soft_pair_counts(soft)                    # c x c expected pair mass
    hard = soft.argmax(-1)                            # ONLY for the context map
    ...
    # SOFT counting: each pair cell carries its expected mass, not a hard tally
    tab = np.zeros((V, c, c))
    for p_ in pairs:
        x_, y_ = p_ // c, p_ % c
        tab[lab[idx[p_]] % V, x_, y_] += Cpair[x_, y_]
    tab += 1e-6
    P[l] = tab / tab.sum((1, 2), keepdims=True)
    ent = float((-(soft * np.log(np.maximum(soft, 1e-300))).sum(-1)).mean())
    print(f"    init level {l}: {len(pairs)} distinct pairs "
          f"[{'context' if use_ctx else 'child-identity'}]  "
          f"mean H(q_{l-1}) = {ent:.3f} nats (max {math.log(card(l)):.3f})")
...
# true-grammar rule tables, needed by the stagewise readout and by SCORES
PT = {}
for _l in range(1, L+1):
    _t = np.zeros((V, card(_l), card(_l)))
    for _s, _rules in M["rules_all"][str(_l)].items():
        for (_x, _y) in _rules: _t[int(_s), _x, _y] += 1.0 / len(_rules)
    PT[_l] = _t

# ============================================================ stagewise fit
def rule_info(Pm, l):
    """I(parent ; child pair) at level l, from the rule table alone -- needs no
    ground truth. This is how much a level's symbols actually CONSTRAIN what
    they produce. Posterior entropy says how sure the model is of its own
    labels, which is a different thing and can be low while this is ~0."""
    pri = np.full(V, 1.0 / V)
    for ll in range(L, l, -1):                      # prior on level-l symbols
        j = Pm[ll].sum(2) + Pm[ll].sum(1)           # marginal over both children
        pri = (pri @ j) / 2.0
        pri = pri / pri.sum()
    joint = Pm[l] * pri[:, None, None]              # P(parent, x, y)
    pair = joint.sum(0)
    Hp = -(pair[pair > 0] * np.log(pair[pair > 0])).sum()
    Hc = -(joint[joint > 0] * np.log(joint[joint > 0])).sum() + \
         (pri[pri > 0] * np.log(pri[pri > 0])).sum()
    return float(Hp + Hc)

def post_at(Pm, X, l):
    """exact posterior at every level-l node: inside x outside, normalised.
    Uses information from ABOVE as well as below -- legitimate for fitting."""
    tabs = inside(onehot(X), Pm); outs = outside(tabs, Pm)
    q = tabs[l] * outs[l]
    return q / np.maximum(q.sum(-1, keepdims=True), 1e-300)

def stage_em(Pm, X, upto, sweeps):
    """EM sweeps that re-estimate ONLY level `upto`; levels below are frozen."""
    for _ in range(sweeps):
        Cc, _ = estep(X, Pm)
        Pn = mstep(Cc)
        Pm = {l: (Pn[l] if l == upto else Pm[l]) for l in Pm}
    return Pm

def vertical_recluster(Pm, X, l):
    """re-cluster level l using the full posterior of its own nodes. With 2 or
    1 nodes per sequence there is no horizontal context, but the nodes are each
    other's context through the root, and outside carries exactly that."""
    q = post_at(Pm, X, l)                       # B x nodes x V
    ql = q.reshape(-1, V)
    prof = ql.T @ ql / max(len(ql), 1)          # V x V co-occurrence profile
    rs = prof.sum(1, keepdims=True); rs[rs == 0] = 1
    U, S, _ = np.linalg.svd(prof / rs, full_matrices=False)
    emb = U[:, :min(V, 8)] * S[:min(V, 8)]
    g_ = np.random.RandomState(a.seed)
    cen = emb[g_.choice(V, V, replace=False)]
    for _ in range(25):
        lab_ = ((emb[:, None, :] - cen[None]) ** 2).sum(-1).argmin(1)
        for j in range(V):
            m_ = lab_ == j
            if m_.any(): cen[j] = emb[m_].mean(0)
    Cc, _ = estep(X, Pm)
    pooled = np.zeros_like(Cc[l])
    for s_ in range(V): pooled[lab_[s_]] += Cc[l][s_]
    tab = pooled[lab_] + 1e-9
    Pm = dict(Pm); Pm[l] = tab / tab.sum((1, 2), keepdims=True)
    return Pm, len(set(lab_.tolist()))

if a.stagewise and not a.load_P:
    print("\n  STAGEWISE FIT (levels below are frozen; no back-correction)")
    print(f"    {'level':>6}{'nodes/seq':>11}{'H(q_l)':>10}{'I(par;kids)':>13}"
          f"{'true I':>8}{'train':>11}{'held-out':>11}   action")
    for l in range(1, L+1):
        if a.vertical and (SEQ >> l) < 4:
            P, ncl = vertical_recluster(P, TR, l)
            note = f"vertical re-cluster -> {ncl} classes"
        else:
            note = "horizontal"
        P = stage_em(P, TR, l, a.stage_sweeps)
        q = post_at(P, TR, l)
        H = float((-(q * np.log(np.maximum(q, 1e-300))).sum(-1)).mean())
        tr_ll = loglik(TR, P); va_ll = loglik(VA, P)
        act = note
        if a.freeze_h > 0 and H > a.freeze_h:
            act += "  |  UNDETERMINED: stopping the climb"
        print(f"    {l:>6}{SEQ >> l:>11}{H:>10.3f}{rule_info(P, l):>13.3f}"
              f"{rule_info(PT, l):>8.3f}{tr_ll:>11.4f}{va_ll:>11.4f}   {act}")
        if a.freeze_h > 0 and H > a.freeze_h: break
    print("    H(q_l)      : how sure the model is of its own labels (low = sure)")
    print("    I(par;kids) : how much a level's symbols constrain what they")
    print("                  produce. THIS is the confidence that matters: it can")
    print("                  be ~0 while H(q_l) is ~0, i.e. the model is certain")
    print("                  about labels that carry no structure.")
...
for it in ([] if (a.load_P or a.stagewise) else range(a.polish)):
    C, ll_tr = estep(TR, P)
    P = mstep(C)
    hist.append(ll_tr)
    va_ = loglik(VA, P)
...
if not (a.load_P or a.stagewise):
...
if not (a.load_P or a.stagewise): verdict["V0 EM ascent"] = worst > -1e-8
if not (a.load_P or a.stagewise):
