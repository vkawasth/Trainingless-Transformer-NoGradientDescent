...
# LIGHT MF PUMP. On the decorrelated corpus the pump at ETA_MF=0.01 raised val
# 5.717 -> 11.358 -> 19.118, and Phase 3 then reached only 6.476 in 32 steps --
# above where Phase 1 had already put it, and far above plain Adam's 4.77 at
# step 50. The pump was calibrated for a corpus that is one string repeated 270
# times; on a corpus with real bigram structure it overshoots.
...
for step in range(1, 300+1):
    ...
    if step % 8 == 0:
        ...
        if step % 20 == 0 or step == 1:
            import numpy as _np
            _prev = globals().get("_PREVW")
            _cur = {n: p.data.clone() for n, p in model.named_parameters()
                    if p.requires_grad and (".WQ." in n or ".WK." in n)}
            if _prev is not None:
                _H = 4
                _rg = []; _vt = []; _prm = []; _prq = []
                for _nq in sorted(k for k in _cur if ".WQ." in k):
                    _nk = _nq.replace("WQ", "WK")
                    if _nk not in _cur: continue
                    Q = _prev[_nq].double(); K = _prev[_nk].double()
                    dQ = (_cur[_nq]-_prev[_nq]).double()
                    dK = (_cur[_nk]-_prev[_nk]).double()
                    D = Q.shape[0]; dh = D // _H; num = 0.0
                    for h in range(_H):
                        ...
                        A_ = q.T@q; B_ = k.T@k
                        R_ = q.T@dq - (k.T@dk).T
                        av, U = torch.linalg.eigh(A_); bv, Vv = torch.linalg.eigh(B_)
                        X = U@((U.T@R_@Vv)/(av[:, None]+bv[None, :]+1e-12))@Vv.T
                        num += float(((q@X)**2).sum() + ((k@X.T)**2).sum())
                    den = float((dQ**2).sum() + (dK**2).sum())
                    _rg.append(num/max(den, 1e-30)); _vt.append(den-num)
                    # gauge-INVARIANT object: M = W_Q W_K^T  (row convention)
                    M0 = (Q@K.T); M1 = ((Q+dQ)@(K+dK).T); dM = M1-M0
                    for _tgt, _acc in ((dM, _prm), (dQ, _prq)):
                        _sv = torch.linalg.svdvals(_tgt); _e = _sv**2
                        _acc.append(float(_e.sum()**2/max(float((_e**2).sum()), 1e-30)))
                print(f"  [git] t={step:>3}  R_gauge={_np.mean(_rg):.4f}  "
                      f"V_trans={_np.mean(_vt):.3e}  PR(dM)={_np.mean(_prm):6.2f}  "
                      f"PR(dWQ)={_np.mean(_prq):6.2f}", flush=True)
            globals()["_PREVW"] = _cur
        ...
        # STOPPING RULES RECALIBRATED. Measured on the decorrelated corpus,
        # the per-probe descent is 0.047,0.017,0.028,0.018,0.025,0.009,0.014,
        # 0.011,0.018,0.014 with median 0.017 -- and the run stopped at step 88
        # on a single probe of 0.0003 where val had gone 3.9453 -> 3.9477, an
        # INCREASE within evaluation noise. The absolute threshold 0.003 was set
        # for a corpus where val fell by whole nats per probe.
        #   - P3MIN forces a minimum number of steps before any stop is honoured
        #   - the plateau now needs TWO consecutive low-delta probes, so one
        #     noisy evaluation cannot end the phase
        #   - the val<0.15 test is disabled: the bigram-conditional floor here
        #     is 2.238 nats, so it could never fire and was a memorisation test
        _small = delta < P3PLAT
        if step >= P3MIN and _small and _prev_small:
            print(f"  ✓ Plateau (loss, 2 consecutive < {P3PLAT})"); break
        _prev_small = _small
...
print("  [zoneadam] SNAPPER DISABLED -- no convex basin on this corpus.")
print("  Measured over 1200 Adam steps (val 4.81 -> 2.92, floor 2.238) the top")
print("  Hessian direction is UPHILL at every distance 0.25-4.0 and every")
print("  checkpoint: at step 1200 the samples run 2.916 3.018 3.224 4.081")
print("  8.407, and the quadratic coefficient decays 0.445 -> 0.168 rather")
print("  than turning positive. On the degenerate corpus it was +15.5.")
SNAPPER_DISABLED = True
