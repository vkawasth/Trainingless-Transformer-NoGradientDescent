...
def run_gradient_descent(model, steps=400, lr=LR):
    """Run standard gradient descent as fallback."""
...
def main():
    ...
    # If val is not good enough, run gradient descent
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        # Save final model
        torch.save(model.state_dict(), 'compiler_smart_gd_final.pt')
        print(f"\n  Saved: compiler_smart_gd_final.pt")
    else:
        # Save final model
