...
PSTAR=None
try:
    _meta=json.load(open("/tmp/corpus_meta.json"))
    if "cond" in _meta:
        PSTAR=torch.zeros(len(CTX),V,dtype=torch.float64)
        for _i,_x in enumerate(CTX):
            _s=_meta["succ"][_x]; _c=_meta["cond"][_x]
            if _s: PSTAR[_i,torch.tensor(_s)]=torch.tensor(_c,dtype=torch.float64)
        print(f"  true conditionals loaded: floor E[H*]={_meta['H_true']:.4f} "
              f"vs uniform-support {_meta['H_uniform_support']:.4f}")
except Exception as _e:
    print("  no true conditionals in meta; shape_true unavailable")
...
print(f"  {'step':>5}{'val':>9}{'KL':>9}{'prec':>8}{'margin':>9}{'rho':>8}{'cover':>8}{'rev':>8}{'shp_u':>8}{'shp_true':>9}{'leak':>8}{'b1':>5}  stop")
for t in range(1,701):
    ...
    if t%20==0:
        ...
        # HONEST in-support divergence against the TRUE conditional, not
        # against uniform. On a corpus with non-uniform successors these differ
        # by E[ln b] - E[H*] = 0.458 nats even for a perfect model.
        _shape_true=float((PSTAR*(PSTAR.clamp_min(1e-30).log()
                     -_pS.clamp_min(1e-30).log())).sum(1).mean()) \
                    if PSTAR is not None else float('nan')
        ...
        print(f"  {t:>5}{vl():>9.4f}{kl:>9.4f}{pr:>8.4f}{mg:>+9.4f}{rho:>+8.3f}"
              ...
              f"{_shape:>8.3f}{_shape_true:>9.3f}{_leak:>8.3f}{_b1:>5d}  {_why}",
              flush=True)
