#!/usr/bin/env python3
"""
Constant Geodesic Flow Compiler
===============================
Follows the geodesic with constant step size (no adaptation, no backtracking).
This is the same as GD-400 but with fewer steps and no optimizer state.
...
# Constant geodesic flow parameters
GEODESIC_STEPS = 50
GEODESIC_LR = 0.001  # Constant step size (same as GD-400 but fewer steps)
...
# ============================================================================
# CONSTANT GEODESIC FLOW (NO ADAPTATION, NO BACKTRACKING)
# ============================================================================
def constant_geodesic_flow(model, steps=50, lr=0.001):
    """
    Follow the geodesic with constant step size.
    No adaptation, no backtracking - just follow the gradient.
    This is the same as GD but with a fixed step size.
    """
    print("\n━━━ CONSTANT GEODESIC FLOW ━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print(f"  Following the geodesic with constant LR={lr}")
    print(f"  Steps: {steps}")
    ...
    for step in range(1, steps + 1):
        # One step of geodesic flow (same as GD but with gradient normalization)
        model.zero_grad()
        ...
        # Constant step (no adaptation)
        w0 = model.flat_params()
        model.set_flat(w0 - lr * direction)
        
        # Track direction change
        if prev_direction is not None:
            cos_change = float((direction * prev_direction).sum())
            direction_changes.append(cos_change)
        ...
        history.append((step, v, tau, phi, grad_norm))
        
        if step % 10 == 0:
            print(f"    Step {step:3d}: val={v:.4f}, τ={tau:.2f}, φ={phi}/5, ||g||={grad_norm:.4f}")
        
        # Stop if we reached the floor
        if v <= FLOOR_TARGET_VAL:
    ...
    if direction_changes:
        ...
        min_change = np.min(direction_changes)
        max_change = np.max(direction_changes)
        print(f"\n  Direction changes: avg={avg_change:.4f}, min={min_change:.4f}, max={max_change:.4f}")
    
    print(f"\n  Best val: {best_val:.4f} (from {steps} steps)")
    
    return best_model, best_val, history, direction_changes
...
def action_constant_geodesic_flow(state):
    """
    Constant geodesic flow action - follows the geodesic with constant step size.
    No adaptation, no backtracking.
    """
    model = state.model
    v0_val = state.val
    phi0 = state.phi
    ...
    # Apply constant geodesic flow
    model_try = copy.deepcopy(model)
    model_try, v_new, history, dir_changes = constant_geodesic_flow(
        model_try, steps=GEODESIC_STEPS, lr=GEODESIC_LR
    )
    
    phi_new = phi_clean(model_try)
    tau_new = gluing_defect(model_try, n=4)
    
    print(f"\n    Constant geodesic flow complete:")
    print(f"      val: {v0_val:.4f} → {v_new:.4f}")
    print(f"      phi: {phi0}/5 → {phi_new}/5")
    print(f"      tau: {tau0:.2f} → {tau_new:.2f}")
    print(f"      Steps taken: {len(history)}")
    if dir_changes:
        print(f"      Avg direction change: {np.mean(dir_changes):.4f}")
    
    if v_new < v0_val * 0.95:
        return CompilerState(model_try, state.step + 1, 'constant_geodesic_flow',
                            v_new, phi_new, tau_new,
                            state.action_history + ['constant_geodesic_flow'],
                            parent=state)
...
def get_smart_actions(state, analyzer):
    ...
    # CONSTANT GEODESIC FLOW: when near floor and in orbit
    if near_floor and phi >= 4 and val < 0.2:
        actions.append(('constant_geodesic_flow', {}))
        print(f"    → constant_geodesic_flow: val={val:.4f}, phi={phi}/5")
...
# ============================================================================
# SMART DP COMPILER WITH CONSTANT GEODESIC FLOW
# ============================================================================
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'constant_geodesic_flow':
            return action_constant_geodesic_flow(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("CONSTANT GEODESIC FLOW COMPILER")
        print("Following the geodesic with constant step size")
        print("No adaptation, no backtracking")
        print("Target: 0.0147 in ≤50 CE")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'constant_geodesic_gd_final.pt')
        print(f"\n  Saved: constant_geodesic_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'constant_geodesic_final.pt')
        print(f"\n  Saved: constant_geodesic_final.pt")
