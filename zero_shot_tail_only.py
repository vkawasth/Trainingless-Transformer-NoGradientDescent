#!/usr/bin/env python3
"""
ACTIVATION COVARIANCE ALIGNMENT WITH ANISOTROPY TAIL CORRECTION
==============================================================
Protects the model's high-rank abstract features by applying the analytical 
context cocycle deflection field exclusively to the low-variance tail dimensions.

Mathematical Formulation:
  For k < HEAD_DIM:   Sigma_edited[k] = Sigma_empirical[k] (No distortion)
  For k >= HEAD_DIM:  Sigma_edited[k] = Sigma_empirical[k] * Ratio_Cocycles[k]
"""
...
ALPHA=0.20     # Residual bleeding blend factor
HEAD_DIM=16    # Preserve the top 16 principal components exactly as-is

print(f"\n{'='*75}")
print(f"  ANISOTROPY TAIL CORRECTION ALIGNMENT ENGINE")
print(f"  Selective Subspace Spectrum Injection (Head Preservation Dim: {HEAD_DIM})")
print(f"{'='*75}\n")
...
# ── Step 4: Selective Subspace Activation Deflection Engine ──────────────────
def compute_tail_corrected_operator(cov_mat_torch, target_sv_np, head_dim=16):
    """
    Constructs a deflection matrix that leaves the top head_dim principal
    components intact, correcting only anisotropy down the tail of the spectrum.
    """
    U, Sigma, Vt = torch.linalg.svd(cov_mat_torch)
    Sigma_np = Sigma.cpu().numpy()
    
    # Scale analytical target to the baseline energy scale
    t_sv = target_sv_np * Sigma_np[0]
    ratio_vector = t_sv / (Sigma_np + 1e-8)
    ratio_vector = np.clip(ratio_vector, 0.1, 10.0)
    
    # FIX: Splice the array to insulate high-variance components from deflection
    Sigma_edited = np.copy(Sigma_np)
    Sigma_edited[head_dim:] = Sigma_np[head_dim:] * ratio_vector[head_dim:]
    
    M_edit = U @ torch.diag(torch.tensor(Sigma_edited, dtype=torch.float32)) @ Vt
    return M_edit

M_deflection = compute_tail_corrected_operator(cov_matrix, high_rank_analytical_sv, head_dim=HEAD_DIM)
