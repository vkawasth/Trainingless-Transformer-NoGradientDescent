...
def _sub_mem():
    """Optimizer-state accounting, in fp32 floats.

    Honest version, because the naive one is wrong. The subspace does NOT
    replace Adam's state here -- the patch projects the update AFTER opt_b.step(),
    so m and v are still maintained in full. The frame is state carried IN
    ADDITION.

        AdamW            2P                (m and v)
        + frame Q        sum_k n_k * K     (P*K if every block is covered)
        + coefficients   sum_k K           negligible
        + sign field     P/32 in fp32-equivalents (1 bit per coord)

    At K=8 over 7 roles the frame is ~8P, so this patch uses about 5x MORE
    optimizer memory than plain AdamW, not less. The compression result is about
    what the update FITS IN, not about what has to be stored.

    Where a real saving would come from, and none of it is implemented here:
      storing m and v only on the frame     2*K per block instead of 2*n_k
      the sign field instead of m           1 bit vs 32 bits per coordinate
      residual kept on its top 1% coords    ~12,000 instead of n_k
    Those are printed as a PROJECTION so the gap between measured and
    hypothetical is visible."""
    P = _sub_P
    adam = 2 * P
    frame = sum(f.numel() for f in _sub_frame.values()) if _sub_frame else 0
    coef = sum(c.numel() for c in _sub_coef.values()) if _sub_coef else 0
    bank = P if _SUB_ACCUM else 0
    sgn = P / 32.0
    actual = adam + frame + coef + bank + sgn
    mb = lambda x: x * 4 / 2**20
    print(f"  [mem] optimizer state, fp32 floats")
    print(f"        AdamW m,v            {adam:>12,}  {mb(adam):>8.1f} MB")
    print(f"        + frame Q            {frame:>12,}  {mb(frame):>8.1f} MB")
    print(f"        + coefficients       {coef:>12,}  {mb(coef):>8.1f} MB")
    if bank:
        print(f"        + residual bank      {bank:>12,}  {mb(bank):>8.1f} MB")
    print(f"        + sign field         {sgn:>12,.0f}  {mb(sgn):>8.1f} MB")
    print(f"        TOTAL                {actual:>12,.0f}  {mb(actual):>8.1f} MB"
          f"   = {actual/adam:.2f}x AdamW")
    # what it WOULD be if the frame replaced the ambient state
    nb = len(_sub_idx)
    proj = 2 * _SUB_K * nb + int(0.01 * P) + P / 32.0
    print(f"  [mem] PROJECTED if m,v lived on the frame and the residual were")
    print(f"        kept on its top 1% (neither is implemented):")
    print(f"        m,v on frame         {2*_SUB_K*nb:>12,}")
    print(f"        residual top-1%      {int(0.01*P):>12,}")
    print(f"        sign field           {P/32.0:>12,.0f}")
    print(f"        TOTAL                {proj:>12,.0f}  {mb(proj):>8.1f} MB"
          f"   = {proj/adam:.3f}x AdamW  ({adam/proj:.0f}x saving)")
