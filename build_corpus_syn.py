...
ap.add_argument("--synonym", action="store_true",
                help="a fraction of ORDER POSITIONS are realised by TWO leaf "
                     "tokens (c ~ d). They share a poset position but are "
                     "given DISJOINT parent contexts, so the embedding cannot "
                     "make them identical by distribution alone.")
ap.add_argument("--syn-frac", type=float, default=0.35,
                help="fraction of order positions that get a second token")
...
SYN = {}          # token -> its synonym token, for tokens in a synonym pair
CONCEPT = list(range(LEAF))     # token -> concept id
if a.synonym:
    a.poset = True
    # Build the order on CONCEPTS, then realise some concepts with two tokens.
    # Synonyms occupy the SAME order position by construction; the question is
    # whether the model transfers an order fact from one surface form to the
    # other without ever seeing it stated for the second.
    n_syn = int(round(LEAF * a.syn_frac / (1 + a.syn_frac)))
    n_con = LEAF - n_syn                      # concepts; n_syn of them doubled
    topo = list(range(n_con)); rng.shuffle(topo)
    CP = np.zeros((n_con, n_con), dtype=bool)
    for ii in range(n_con):
        for jj in range(ii + 1, n_con):
            if rng.random() < a.poset_density: CP[topo[ii], topo[jj]] = True
    for k in range(n_con): CP |= np.outer(CP[:, k], CP[k, :])
    np.fill_diagonal(CP, False)
    doubled = set(rng.sample(range(n_con), n_syn))
    CONCEPT, tok = [], 0
    con_tokens = {}
    for cid in range(n_con):
        con_tokens[cid] = [tok]; CONCEPT.append(cid); tok += 1
        if cid in doubled:
            con_tokens[cid].append(tok); CONCEPT.append(cid); tok += 1
    LEAF = a.nleaf = tok
    CONCEPT = np.array(CONCEPT)
    PREC = CP[np.ix_(CONCEPT, CONCEPT)]       # tokens inherit their concept order
    for cid, ts in con_tokens.items():
        if len(ts) == 2: SYN[ts[0]] = ts[1]; SYN[ts[1]] = ts[0]
    COMPARABLE = [(x, y) for x in range(LEAF) for y in range(LEAF) if PREC[x, y]]
...
C_trans, C_incomp, C_chain, C_syn, C_synsrc = [], [], [], [], []
if a.poset:
...
# ---------------------------------------------- synonym compatibility class
# C_syn: (a,d) where (a,c) IS a level-1 rule, d ~ c (same order position,
# different surface form), and (a,d) is NOT a rule in either direction.
# The quotient axiom says a < d must hold, since d occupies c's position.
# C_synsrc is the matched positive anchor: the (a,c) the fact came from.
if SYN:
    prod_pairs2 = set(prod_of.keys())
    seent = set()
    for (x, y) in list(prod_pairs2):
        if not PREC[x, y]:
            continue
        d = SYN.get(y)
        if d is None or (x, d) in prod_pairs2 or (d, x) in prod_pairs2:
            continue
        if (x, d) in seent:
            continue
        seent.add((x, d)); C_syn.append([x, d]); C_synsrc.append([x, y])
    _o = list(range(len(C_syn))); rng.shuffle(_o)
    C_syn = [C_syn[i] for i in _o][:96]
    C_synsrc = [C_synsrc[i] for i in _o][:96]
...
if SYN:
    print(class_stats(C_syn,    "C_syn"))
    print(class_stats(C_synsrc, "C_synsrc"))
    _pset = set(prod_of.keys())
    _par = {}
    for i in range(V):
        for (x, y) in rules[1][i]:
            _par.setdefault(x, set()).add(("L", i)); _par.setdefault(y, set()).add(("R", i))
    _ov = [len(_par.get(t, set()) & _par.get(SYN[t], set())) /
           max(len(_par.get(t, set()) | _par.get(SYN[t], set())), 1)
           for t in sorted(SYN) if t < SYN[t]]
    print(f"    SYNONYMS: {len(SYN)//2} pairs, {len(SYN)}/{LEAF} tokens doubled")
    print(f"    parent-context Jaccard overlap between synonyms: "
          f"mean={np.mean(_ov):.3f} (0 = fully disjoint contexts)")
    print(f"    C_syn    = (a,d) with (a,c) a rule, d~c, (a,d) NEVER a rule")
    print(f"    C_synsrc = the (a,c) each fact came from (positive anchor)")
    if len(C_syn) < 16:
        print(f"    WARNING C_syn too small; raise --syn-frac or --nrules")
if a.poset:
...
json.dump({"mode": "rhm", "reverse_valid": bool(a.reverse_valid), "depth": L, "nsym": V, "nrules": M,
           ...
           "C_syn": C_syn, "C_synsrc": C_synsrc,
           "synonyms": {str(k): v for k, v in SYN.items()},
           "concept": (CONCEPT.tolist() if hasattr(CONCEPT, "tolist") else CONCEPT),
           "poset": bool(a.poset),
           "prec": (PREC.astype(int).tolist() if a.poset else []),
           "rules_level2": {str(i): rules[2][i] for i in range(V)} if L >= 2 else {},
           "rules_all": {str(l): {str(i): rules[l][i] for i in range(V)}
                         for l in range(1, L + 1)},
