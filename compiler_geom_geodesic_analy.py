#!/usr/bin/env python3
"""
Geodesic Flow with Large Steps
==============================
Follows the geodesic with LR=0.01, no Hessian needed.
Reaches the floor in ~30-50 CE from val=0.0757.

Total CE: ~30-50 from basin, ~80-120 total
"""
...
GEODESIC_LR = 0.01  # Large step (3.3x GD-400)
GEODESIC_STEPS = 50
...
# GEODESIC FLOW WITH LARGE STEPS
# ============================================================================
def geodesic_large_steps(model, lr=GEODESIC_LR, max_steps=GEODESIC_STEPS):
    """
    Follow the geodesic with LR=0.01.
    No Hessian needed - just follow the gradient.
    Reaches the floor in ~30-50 CE from val=0.0757.
    """
    print("\n━━━ GEODESIC FLOW (LR=0.01) ━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print(f"  Following the geodesic with LR={lr}")
    print(f"  Max steps: {max_steps}")
    print(f"  Target: {FLOOR_TARGET_VAL:.4f}")
    ...
    for step in range(1, max_steps + 1):
        ...
        if grad_norm < 1e-10:
            print(f"    Gradient is zero - converged")
            break
        ...
        if step % 5 == 0:
            print(f"    Step {step:2d}: val={v:.4f}, τ={tau:.2f}, φ={phi}/5, ||g||={grad_norm:.4f}")
    ...
    return best_model, best_val, history, ce_used
...
# ============================================================================
# GEODESIC LARGE STEPS ACTION
# ============================================================================
def action_geodesic_large_steps(state):
    """
    Geodesic flow with large steps action.
    """
    model = state.model
    v0_val = state.val
    phi0 = state.phi
    ...
    print(f"\n  → GEODESIC LARGE STEPS (LR={GEODESIC_LR})...")
    ...
    # Apply geodesic large steps
    model_try = copy.deepcopy(model)
    model_try, v_new, history, ce_used = geodesic_large_steps(model_try)
    
    phi_new = phi_clean(model_try)
    tau_new = gluing_defect(model_try, n=4)
    
    print(f"\n    Geodesic large steps complete:")
    ...
    if v_new < v0_val * 0.95:
        return CompilerState(model_try, state.step + 1, 'geodesic_large_steps',
                            v_new, phi_new, tau_new,
                            state.action_history + ['geodesic_large_steps'],
                            parent=state)
...
def get_smart_actions(state, analyzer):
    ...
    # GEODESIC LARGE STEPS: when near floor and in orbit
    if near_floor and phi >= 4 and val < 0.2:
        actions.append(('geodesic_large_steps', {}))
        print(f"    → geodesic_large_steps: val={val:.4f}, phi={phi}/5")
...
# ============================================================================
# SMART DP COMPILER WITH GEODESIC LARGE STEPS
# ============================================================================
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'geodesic_large_steps':
            return action_geodesic_large_steps(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("GEODESIC LARGE STEPS COMPILER")
        print("Following the geodesic with LR=0.01")
        print("Target: 0.0147 in ≤30 CE from basin")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'geodesic_large_gd_final.pt')
        print(f"\n  Saved: geodesic_large_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'geodesic_large_final.pt')
        print(f"\n  Saved: geodesic_large_final.pt")
