#!/usr/bin/env python3
"""
CONTEXT COCYCLE HOLONOMY INJECTION WITH MATURE EMBEDDINGS
================================================================
Uses the mature algebraic coordinate space of a 24L teacher to seed 
the 2L student's embedding and normalization layers. This provides 
the structural baseline necessary for the high-rank unrolled context 
cocycle to execute without encountering initialization-scale evaporation.

Mathematical Formulation for the Analytical Spectrum:
...
print(f"\n{'='*75}")
print(f"  MATURE SEEDING + UNROLLED CONTEXT COCYCLE ENGINE")
print(f"  High-Rank Zero-Shot Spectrum Injection (\u03b3 = {GAMMA})")
print(f"{'='*75}\n")
...
def compute_unrolled_cocycle_spectrum(train_ids, vocab_size, seq_len=64, proj_dim=48, gamma=0.85):
    ...
    # Target scaling matched to mature teacher dimensions (~12.5)
    Analytical_Lambda = (Analytical_Lambda / (Analytical_Lambda[0] + 1e-8)) * 12.5
...
# FIX: Seed the input/positional embeddings and final normalization layer
...
print(f"  Extracted Mature 2L Spectrum  (Top 4 \u039b): {sv_2l[:4].round(4)}")

# ── Step 4: Cartan Subalgebra Lie Deflection & Calibration Pass ───────────────
