...
for step in range(1, 300+1):
    ...
    if step % 8 == 0:
        ...
        if step in (40,200):
            _SNAP[step]={n:p.data.clone() for n,p in model.named_parameters()
                         if p.requires_grad}
            if step==200:
                import numpy as _np, torch as _t
                _ch={n:(_SNAP[200][n]-_SNAP[40][n]) for n in _SNAP[200]}
                _f=_t.cat([v.reshape(-1) for v in _ch.values()])
                print(f"  [wq] ||chord|| {float(_f.norm()):.3f}")
                for _nm in ("attn.WQ.weight","attn.WK.weight","attn.WV.weight",
                            "ff.g.weight","te.weight"):
                    _ks=[x for x in _ch if x.endswith(_nm)]
                    if not _ks: continue
                    _M=_ch[_ks[0]]
                    _sv=_t.linalg.svdvals(_M); _e=_sv**2
                    _pr=float(_e.sum()**2/max(float((_e**2).sum()),1e-30))
                    _t2=float(_e[:2].sum()/_e.sum())
                    print(f"  [wq] {_nm:<16} shape {tuple(_M.shape)}  PR {_pr:6.2f}"
                          f"  top2-energy {_t2:.3f}  ||.|| {float(_M.norm()):.3f}")
                _t.save({"ch":{n:_ch[n] for n in _ch if n.endswith("WQ.weight")},
                         "w40":{n:_SNAP[40][n] for n in _SNAP[40]}},"wq_snap.pt")
                print("  [wq] saved wq_snap.pt")
