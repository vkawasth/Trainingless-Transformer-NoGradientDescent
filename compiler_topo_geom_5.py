#!/usr/bin/env python3
"""
Fast Basin Settle Compiler
==========================
Based on compiler_topo_geom_1.py but with faster basin settle.

Changes:
  1. Basin settle LR×10: 25 steps (was 40)
  2. Basin settle LR×5: 20 steps (was 40) - SKIP if phi=5/5 after LR×10
  3. Basin settle LR×3: 15 steps (was 40) - SKIP if val < 0.1
  4. Basin settle LR×1: 15 steps (was 30-40)

Expected: ~60-80 CE total for basin settle (vs 150-160)
Total compiler CE: ~100-120 (vs 276)
"""
...
# ============================================================================
# SMART ACTION SELECTION (from working compiler)
# ============================================================================
def get_smart_actions(state, analyzer):
    ...
    # 1. Saddle exit: only if negative curvature
    ...
    # 3. Basin settle: multiple LR options with fewer steps
    if not near_floor:
        if val > 1.0:
            actions.append(('basin_settle', {'lr_mult': 10, 'max_steps': 25}))
            actions.append(('basin_settle', {'lr_mult': 5, 'max_steps': 20}))
        elif val > 0.3:
            actions.append(('basin_settle', {'lr_mult': 5, 'max_steps': 20}))
            actions.append(('basin_settle', {'lr_mult': 3, 'max_steps': 15}))
        else:
            actions.append(('basin_settle', {'lr_mult': 3, 'max_steps': 15}))
            actions.append(('basin_settle', {'lr_mult': 2, 'max_steps': 15}))
    
    # 4. Small-step basin settle (LR×1) for near floor
    if near_floor or val < 0.15:
        actions.append(('basin_settle', {'lr_mult': 1, 'max_steps': 15}))
        print(f"    → basin_settle: LR×1 (15 steps)")
    ...
    # 9. Fallback
    if not actions:
        print(f"    ⚠ No viable actions. Using fallback.")
        actions.append(('basin_settle', {'lr_mult': 2, 'max_steps': 20}))
...
def action_saddle_exit(state, params):
    """Saddle exit with alpha parameter."""
    model = state.model
    alpha = params['alpha']
    ...
    if phi_clean(model_try) < PHI_CLEAN_TARGET:
        apply_orbit_correction(model_try)
    ...
    if v_try < state.val * 0.95:
        return CompilerState(model_try, state.step + 1, 'saddle_exit',
                            ...
                            state.action_history + [f'saddle_exit(α={alpha:.3f}, {ce_used}CE)'],
                            parent=state, ce_used=ce_used)
    return None


def apply_orbit_correction(model):
    """Apply algebraic orbit correction to restore clean phases."""
    WKs = [model.blocks[l].attn.WK.weight.data.float().cpu().numpy()
           for l in range(N_STU)]
    phases = sheet_angles(model)
    
    for k in range(N_STU - 1):
        if k >= len(phases) or np.isnan(phases[k]):
            continue
        if abs(phases[k]) < ORBIT_TOLERANCE or abs(abs(phases[k]) - math.pi) < ORBIT_TOLERANCE:
            continue
...
def action_basin_settle(state, params):
    ...
    for step in range(1, max_steps + 1):
        ...
        if step % 5 == 0 or step == max_steps:
            ...
            # Track best
            if phi > best_phi or (phi == best_phi and v < best_val):
                best_phi = phi
                best_val = v
                best_model = copy.deepcopy(model_try)
            ...
            if v > v0 * 1.5:
                return None
            # If phi=5/5 and val is low enough, stop early
            if phi == PHI_CLEAN_TARGET and v < 0.15 and lr_mult <= 3:
                break
    ...
    if v_try < v0 * 0.95 or phi_try > phi_clean(model):
        return CompilerState(best_model, state.step + 1, 'basin_settle',
...
def action_restore_orbit(state, params):
    """Restore orbit by correcting off-wall phases."""
    ...
    if phi_try > state.phi and v_try < state.val * 1.1:
        return CompilerState(model_try, state.step + 1, 'restore_orbit',
                            ...
                            state.action_history + [f'restore_orbit(step={step_size:.2f}, {ce_used}CE)'],
                            parent=state, ce_used=ce_used)
    return None


def action_k0_split(state, params):
    """K0 split with clamped w_FF."""
    model = state.model
    ...
    if v_try < state.val * 0.95:
        return CompilerState(m_out, state.step + 1, 'k0_split',
                            ...
                            state.action_history + [f'k0_split(w_FF={w_ff:.2f}, {ce_used}CE)'],
                            parent=state, ce_used=ce_used)
    return None


def action_lm_step(state, params):
    """LM step with mu parameter."""
    mu = params['mu']
    model = state.model
    ce_used = 0
    ...
    if v_try < state.val * 0.95:
        return CompilerState(model_try, state.step + 1, 'lm_step',
                            v_try, phi_clean(model_try), gluing_defect(model_try),
                            state.action_history + [f'lm_step(μ={mu:.2f}, {ce_used}CE)'],
                            parent=state, ce_used=ce_used)
    return None
...
def action_lanczos(state, params):
    ...
    if accepted and v0 < state.val * 0.95:
        return CompilerState(model_try, state.step + 1, 'lanczos',
                            ...
                            state.action_history + [f'lanczos(k=8, {ce_used}CE)'],
                            parent=state, ce_used=ce_used)
    return None


# ============================================================================
# DP COMPILER (from working compiler)
# ============================================================================
class SmartDPCompiler:
    ...
    def run(self):
        print("=" * 70)
        print("FAST BASIN SETTLE COMPILER")
        print("=" * 70)
        print("  Based on working compiler with faster basin settle")
        print("=" * 70)
        print()
...
# ============================================================================
# GD FALLBACK (Reduced to 200 steps)
# ============================================================================
def run_gd_fallback(model, steps=200):
    """Reduced GD fallback (200 steps instead of 400)."""
    print("\n" + "=" * 70)
    ...
    return best_model, v_final, phi_final, tau_final, ce_used, history
...
def main():
    ...
    # Run reduced GD fallback (200 steps)
    if compiler_state.val > 0.02:
        model_final, v_final, phi_final, tau_final, gd_ce, gd_history = run_gd_fallback(
    ...
    torch.save(model_final.state_dict(), 'fast_basin_compiler.pt')
    print(f"\n  Saved: fast_basin_compiler.pt")
