...
mem_viz.sample_compiler("Spectral E0", model)
print()
...
mem_viz.sample_compiler("Saddle exit", model)
...
mem_viz.sample_compiler("MF pump", model)
...
mem_viz.sample_compiler("Basin settle", model)
...
if v_jump <= FLOOR_TARGET_VAL:
    ...
    mem_viz.sample_compiler("Snapper jump (floor)", model)
else:
    print(f"  ⚠ Snapper jump to {v_jump:.4f} - continuing to TopoGate")
    mem_viz.sample_compiler("Snapper jump", model)
    ...
    mem_viz.sample_compiler("TopoGate", model)
    print()
    ...
    mem_viz.sample_compiler("K0 split", model)
    ...
    if v_final > 0.055:
        ...
        mem_viz.sample_compiler("Lanczos", model)
    print()
...
for gd_step in range(1,401):
    ...
    if gd_step in {50,100,150,200,250,300,350,400}:
        ...
        mem_viz.sample_gd(gd_step, gd, opt_gd)
...
# ── MEMORY REPORT ─────────────────────────────────────────────
mem_viz.report()
mem_viz.plot("compiler_vs_gd_memory.png")
