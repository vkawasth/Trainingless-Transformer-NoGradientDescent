#!/usr/bin/env python3
"""
Geometric Geodesic Compiler (Batched CE)
========================================
Follows the geodesic with batched CE (forward + backward = 2 CE per step).
No separate val/tau computation - all from the same pass.

Total CE: ~120 to reach below floor (0.0557)
"""
...
GEODESIC_LR = 0.01
GEODESIC_STEPS = 50
EVAL_INTERVAL = 5  # Only evaluate every 5 steps to save CE
...
# ============================================================================
# BATCHED GEODESIC FLOW (2 CE per step)
# ============================================================================
def batched_geodesic_flow(model, lr=GEODESIC_LR, steps=GEODESIC_STEPS, eval_interval=EVAL_INTERVAL):
    """
    Follow the geodesic with batched CE.
    Each step: 1 forward + 1 backward = 2 CE.
    No gluing_defect calls during the flow.
    """
    print("\n━━━ BATCHED GEODESIC FLOW ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print(f"  LR={lr}, Steps={steps}, Eval interval={eval_interval}")
    print(f"  CE per step: 2 (forward + backward)")
    ...
    for step in range(1, steps + 1):
        # One batched step: forward + backward = 2 CE
        model.zero_grad()
        ls = [model(*get_batch())[1] for _ in range(6)]
        loss = torch.stack(ls).mean()
        loss.backward()
        CE.add(2)  # Forward + backward
        ...
        if grad_norm < 1e-10:
            print(f"    Gradient is zero - converged at step {step}")
            break
        ...
        CE.add(1)  # Step (just a parameter update)
        
        # Evaluate periodically - ONLY val, no tau
        if step % eval_interval == 0 or step == steps:
            ...
            history.append((step, v, phi, grad_norm))
            print(f"    Step {step:3d}: val={v:.4f}, φ={phi}/5, ||g||={grad_norm:.4f}")
            
            if v <= FLOOR_TARGET_VAL:
                print(f"      ✓ REACHED FLOOR!")
                break
            
            # Early stop if val is not decreasing
            if step > eval_interval and v > prev_val * 1.05:
                print(f"      val increased - stopping early")
                break
            prev_val = v
    
    # Final tau computation (only once, at the end)
    tau_final = gluing_defect(best_model, n=4)
    CE.add(4)
...
# ============================================================================
# BATCHED GEODESIC ACTION
# ============================================================================
def action_batched_geodesic(state):
    """
    Batched geodesic action.
    """
    model = state.model
    ...
    print(f"\n  → BATCHED GEODESIC (LR={GEODESIC_LR})...")
    ...
    # Apply batched geodesic
    model_try = copy.deepcopy(model)
    model_try, v_new, history = batched_geodesic_flow(model_try)
    
    phi_new = phi_clean(model_try)
    tau_new = gluing_defect(model_try, n=4)
    CE.add(4)
    
    print(f"\n    Batched geodesic complete:")
    ...
    if v_new < v0_val * 0.95:
        return CompilerState(model_try, state.step + 1, 'batched_geodesic',
                            v_new, phi_new, tau_new,
                            state.action_history + ['batched_geodesic'],
                            parent=state)
...
def get_smart_actions(state, analyzer):
    ...
    # BATCHED GEODESIC: when near floor and in orbit
    if near_floor and phi >= 4 and val < 0.2:
        actions.append(('batched_geodesic', {}))
        print(f"    → batched_geodesic: val={val:.4f}, phi={phi}/5")
...
# ============================================================================
# SMART DP COMPILER WITH BATCHED GEODESIC
# ============================================================================
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'batched_geodesic':
            return action_batched_geodesic(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("BATCHED GEODESIC COMPILER")
        print("2 CE per step (forward + backward)")
        print("Target: 0.0147 in ≤120 CE")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'batched_geodesic_gd_final.pt')
        print(f"\n  Saved: batched_geodesic_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'batched_geodesic_final.pt')
        print(f"\n  Saved: batched_geodesic_final.pt")
