...
# ══════════════════════════════════════════════════════════════════════
# PHASE-3 GRAPH-CULLED OPTIMISER  (measured V/O cull; core untouched)
# ----------------------------------------------------------------------
# Validated on this model: as the block gradient-profile stops drifting, the
# attention value/output projections' AdamW preconditioner flattens (CV→~0.24),
# so W_V/W_O can run on scalar SGD (lr×median D) at ~free val cost — culling the
# second-moment state on ~18% of params. Emb/Q/K/FF are the irreducible core and
# are LEFT ON AdamW. Q/K frozen-D is deliberately NOT enabled (pending the
# seed-variance test). Exposes .param_groups so the existing LR schedule is
# unchanged; the Φ_cl/τ/rm2 geo-stop is left exactly as-is.
_P3_GROUPS = ["Emb", "W_Q", "W_K", "W_V", "W_O", "FF", "LayerNorm"]
...
def _g3_profile(model):
    acc = {}
    for nm, p in model.named_parameters():
        if p.grad is not None:
            g = _g3_group(nm); acc[g] = acc.get(g, 0.0) + float(p.grad.pow(2).sum())
    v = torch.tensor([math.sqrt(acc.get(k, 0.0)) for k in _P3_GROUPS])
    return v / (v.sum() + 1e-12)

class GraphCulledAdamW:
    """AdamW that, once graph drift settles, migrates W_V/W_O to scalar SGD
    (no anisotropic turn) and W_Q/W_K to a FROZEN diagonal preconditioner
    (static turn, v-update culled). Emb/FF/LayerNorm stay on live AdamW.
    ~36% of params leave the second-moment update at no measured cost
    (frozen-Q/K validated free within noise, 5 seeds). Drop-in for
    torch.optim.AdamW: exposes .param_groups (core AdamW), .zero_grad(),
    .step(), and .maybe_cull() (call each step after backward())."""
    VO = ("W_V", "W_O"); QK = ("W_Q", "W_K")
    any_culled = False   # class flag: did any stage cull?

    def __init__(self, model, lr, cull_from_start=False, vo_scale=None, qk_D=None,
                 freeze_qk=True, thr=0.03, warm=20, hold=15, wd=0.1, b1=0.9, verbose=True):
        self.model = model; self.b1 = b1; self.wd = wd; self.verbose = verbose
        self.freeze_qk = freeze_qk
        named = list(model.named_parameters())
        cull_grp = self.VO + (self.QK if freeze_qk else ())
        self.core = [p for nm, p in named if _g3_group(nm) not in cull_grp]   # Emb/FF/LN(+QK if not freezing)
        self.vo   = [p for nm, p in named if _g3_group(nm) in self.VO]
        self.qk   = [p for nm, p in named if _g3_group(nm) in self.QK] if freeze_qk else []
        self.adam = torch.optim.AdamW(self.core, lr=lr, betas=(0.9, 0.95), weight_decay=wd)
        self.vo_adam = (torch.optim.AdamW(self.vo, lr=lr, betas=(0.9, 0.95),
                                          weight_decay=wd) if self.vo else None)
        self.qk_adam = (torch.optim.AdamW(self.qk, lr=lr, betas=(0.9, 0.95),
                                          weight_decay=wd) if self.qk else None)
        self.thr = thr
        self.ph = collections.deque(maxlen=warm); self.dr = collections.deque(maxlen=hold)
        self.vo_m = [torch.zeros_like(p) for p in self.vo]; self.vo_t = 0
        self.qk_m = [torch.zeros_like(p) for p in self.qk]; self.qk_t = 0
        self.culled = False; self.vo_scale = vo_scale; self.qk_D = qk_D
        if cull_from_start and (self.vo or self.qk):
            self._do_cull(from_start=True)
    ...
    def _diag_D(self, opt, params):
        """per-param diagonal D = 1/(sqrt(v_hat)+eps) from an AdamW's state."""
        st = opt.state; b2 = 0.95; eps = 1e-8; out = []
        for p in params:
            if p in st and "exp_avg_sq" in st[p]:
                t = st[p]["step"]; t = float(t.item()) if torch.is_tensor(t) else float(t)
                vhat = st[p]["exp_avg_sq"] / (1 - b2 ** max(t, 1))
                out.append(1.0 / (vhat.sqrt() + eps))
            else:
                out.append(torch.ones_like(p))
        return out

    def _vo_median_D(self):
        if self.vo_adam is None: return 1.0
        Ds = [d.flatten() for d in self._diag_D(self.vo_adam, self.vo)]
        if not Ds: return 1.0
        return min(max(float(torch.cat(Ds).median()), 0.05), 40.0)   # clamp: no blow-up

    def current_vo_scale(self):
        return self.vo_scale if self.vo_scale is not None else self._vo_median_D()

    def current_qk_D(self):
        if not self.freeze_qk: return None
        return self.qk_D if self.qk_D is not None else self._diag_D(self.qk_adam, self.qk)

    def _do_cull(self, from_start=False):
        if self.vo and self.vo_scale is None:
            self.vo_scale = self._vo_median_D()
        if self.qk and self.qk_D is None:
            self.qk_D = self._diag_D(self.qk_adam, self.qk)
        self.culled = True; GraphCulledAdamW.any_culled = True
        if self.verbose:
            n_vo = sum(p.numel() for p in self.vo)
            n_qk = sum(p.numel() for p in self.qk)
            n_all = sum(p.numel() for p in self.core) + n_vo + n_qk
            src = "post-geo-stop" if from_start else "drift settled"
            msg = f"  \u27d0 cull ({src}): V/O\u2192scalar-SGD"
            if self.qk: msg += " + Q/K\u2192frozen-D"
            msg += (f"  [{100*(n_vo+n_qk)/max(n_all,1):.0f}% of params off "
                    f"live 2nd-moment; slr=lr\u00d7{self.vo_scale:.1f}]")
            print(msg)

    def zero_grad(self):
        self.adam.zero_grad()
        if not self.culled:
            if self.vo_adam is not None: self.vo_adam.zero_grad()
            if self.qk_adam is not None: self.qk_adam.zero_grad()
        else:
            for p in self.vo + self.qk:
                if p.grad is not None: p.grad = None

    def maybe_cull(self):
        if self.culled or not (self.vo or self.qk): return
        self.ph.append(_g3_profile(self.model))
        if len(self.ph) == self.ph.maxlen:
            self.dr.append(float((self.ph[-1] - self.ph[0]).norm()))
            if len(self.dr) == self.dr.maxlen and (sum(self.dr) / len(self.dr)) < self.thr:
                self._do_cull()

    def step(self):
        lr = self._cur_lr()
        self.adam.step()                                   # Emb/FF/LN (+ Q/K if not freezing)
        if not self.culled:
            for opt in (self.vo_adam, self.qk_adam):
                if opt is not None:
                    for pg in opt.param_groups: pg["lr"] = lr
                    opt.step()
        else:
            self.vo_t += 1                                 # V/O: scalar SGD (no turn)
            for p, m in zip(self.vo, self.vo_m):
                if p.grad is None: continue
                m.mul_(self.b1).add_(p.grad, alpha=1 - self.b1)
                mhat = m / (1 - self.b1 ** self.vo_t)
                p.data.add_(mhat, alpha=-lr * self.vo_scale).add_(p.data, alpha=-lr * self.wd)
            if self.qk:                                    # Q/K: frozen diagonal D (static turn)
                self.qk_t += 1
                for p, m, D in zip(self.qk, self.qk_m, self.qk_D):
                    if p.grad is None: continue
                    m.mul_(self.b1).add_(p.grad, alpha=1 - self.b1)
                    mhat = m / (1 - self.b1 ** self.qk_t)
                    p.data.add_(mhat * D, alpha=-lr).add_(p.data, alpha=-lr * self.wd)
...
opt_b = GraphCulledAdamW(model, LR*5)   # AdamW + drift-triggered V/O cull
val_history = [v_mf]
step = 0
geo_stop_count = 0
geo_stopped = False
...
for step in range(1, 151):
    ...
    opt_b.maybe_cull()          # migrate V/O to scalar SGD once graph drift settles
    opt_b.step()
...
if geo_stopped:
    ...
    n_fast = 30
    opt_fast = GraphCulledAdamW(model, LR*10, cull_from_start=True,
                                vo_scale=opt_b.current_vo_scale(),
                                qk_D=opt_b.current_qk_D())
    ...
    opt_retry = GraphCulledAdamW(model, LR*2, cull_from_start=True,
                                 vo_scale=opt_b.current_vo_scale(),
                                 qk_D=opt_b.current_qk_D())
...
_p3_vo = sum(p.numel() for nm, p in model.named_parameters()
             if _g3_group(nm) in ("W_V", "W_O"))
_p3_all = sum(p.numel() for p in model.parameters())
_p3_qk = sum(p.numel() for nm, p in model.named_parameters() if _g3_group(nm) in ("W_Q","W_K"))
_p3_off = (_p3_vo+_p3_qk) if GraphCulledAdamW.any_culled else 0
print(f"  Cull: {'FIRED' if GraphCulledAdamW.any_culled else 'not triggered'}"
      f"  (V/O→SGD + Q/K→frozen-D; live 2nd-moment footprint "
      f"{100*(1-_p3_off/_p3_all):.0f}%)")
