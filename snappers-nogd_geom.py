#!/usr/bin/env python3
"""
Smart DP Compiler with Rare Token Jump + 10-Step LR=0.003 Descent
==================================================================
1. DP compiler reaches flat basin (val ~0.075)
2. Rare token jump gets close to floor
3. 10 steps of LR=0.003 finishes the job

Total CE: ~5 (DP) + 113 (rare jump) + 10 (LR=0.003) = ~128 CE
"""
...
# RARE TOKEN JUMP
# ============================================================================
def rare_token_jump(model):
    """
    Rare token jump - gets close to floor.
    """
    print(f"\n  → RARE TOKEN JUMP")
    ...
    # Full direction
    full_direction = torch.zeros_like(model.flat_params())
    idx = 0
    for name, param in model.named_parameters():
        if name == 'te.weight':
            full_direction[idx:idx + direction_emb.numel()] = direction_emb.flatten()
        idx += param.numel()
    
    full_direction = full_direction / max(full_direction.norm(), 1e-10)
    
    # Find best step
    print("    [4] Finding best step...")
    w0 = model.flat_params()
    val0 = eval_val(model, n=4)
    tau0 = gluing_defect(model, n=4)
    phi0 = phi_clean(model)
    ce_used += 4 + 4
    print(f"      Initial: val={val0:.4f}, τ={tau0:.2f}, φ={phi0}/5")
    
    step_sizes = [0.001, 0.003, 0.01, 0.03, 0.06, 0.1, 0.15, 0.2]
    geom_vals = []
    
    for s in step_sizes:
        model.set_flat(w0 + s * full_direction)
        v = eval_val(model, n=4)
        tau = gluing_defect(model, n=4)
        phi = phi_clean(model)
        geom_vals.append((tau, phi, v, s))
        ce_used += 4 + 4
    
    model.set_flat(w0)
    
    # Select best step
    phi_5 = [(tau, phi, v, s) for tau, phi, v, s in geom_vals if phi == 5]
    if phi_5:
        best_step = min(phi_5, key=lambda x: abs(x[0] - TAU_TARGET))
    else:
        best_step = min(geom_vals, key=lambda x: (5 - x[1]) * 2.0 + abs(x[0] - TAU_TARGET))
    
    best_tau, best_phi, best_val, best_s = best_step
    print(f"      Selected: s={best_s:.4f}, val={best_val:.4f}, τ={best_tau:.2f}, φ={best_phi}/5")
    
    # Jump
    print("    [5] Jumping...")
    model.set_flat(w0 + best_s * full_direction)
...
def action_rare_token_jump(state):
    ...
    model_try = copy.deepcopy(model)
    model_try, v_new, tau_new, phi_new, ce_used = rare_token_jump(model_try)
    
    print(f"\n    Rare token jump complete:")
...
# ============================================================================
# 10-STEP LR=0.003 DESCENT
# ============================================================================
def action_lr_003_descent(state):
    """
    10 steps of LR=0.003 descent to reach the floor.
    """
    model = state.model
    v0_val = state.val
    phi0 = state.phi
    ...
    print(f"\n  → LR=0.003 DESCENT (10 steps)...")
    print(f"    val={v0_val:.4f}, phi={phi0}/5, tau={tau0:.2f}")
    
    if v0_val > 0.1:
        print(f"    ℹ Skipping: val={v0_val:.4f} > 0.1 (descent only works near floor)")
        return None
    
    model_try = copy.deepcopy(model)
    opt = torch.optim.AdamW(model_try.parameters(), lr=DESCENT_LR,
                            betas=(0.9, 0.95), weight_decay=0.1)
    ...
    for step in range(1, DESCENT_STEPS + 1):
        ...
        print(f"      Step {step:2d}: val={v:.4f}, τ={tau:.2f}, φ={phi}/5")
    ...
    v_new = best_v
    phi_new = phi_clean(best_model)
    tau_new = gluing_defect(best_model, n=4)
    
    print(f"\n    LR=0.003 descent complete:")
    print(f"      val: {v0_val:.4f} → {v_new:.4f}")
    print(f"      phi: {phi0}/5 → {phi_new}/5")
    print(f"      tau: {tau0:.2f} → {tau_new:.2f}")
    
    if v_new < v0_val * 0.95:
        return CompilerState(best_model, state.step + 1, 'lr_003_descent',
                            v_new, phi_new, tau_new,
                            state.action_history + ['lr_003_descent'],
                            parent=state)
...
def get_smart_actions(state, analyzer):
    ...
    # LR=0.003 DESCENT: when very close to floor (val < 0.08)
    if near_floor and val < 0.08:
        actions.append(('lr_003_descent', {}))
        print(f"    → lr_003_descent: val={val:.4f}")
...
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'lr_003_descent':
            return action_lr_003_descent(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("SMART DP COMPILER WITH RARE TOKEN JUMP + 10-STEP DESCENT")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'compiler_rare_10step_gd_final.pt')
        print(f"\n  Saved: compiler_rare_10step_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'compiler_rare_10step_final.pt')
        print(f"\n  Saved: compiler_rare_10step_final.pt")
