...
# AdamW preconditioner D = 1/(sqrt(vhat)+eps), flattened
st=opt.state; b2=0.95; eps=1e-8; Dl=[]
for p in params:
    if p in st and "exp_avg_sq" in st[p]:
        t=st[p]["step"]; t=float(t.item()) if torch.is_tensor(t) else float(t)
        vh=st[p]["exp_avg_sq"]/(1-b2**max(t,1))
        Dl.append((1.0/(vh.sqrt()+eps)).flatten())
    else:
        Dl.append(torch.ones(p.numel()))
D=torch.cat(Dl)
Hv = D*Hv                     # preconditioned:  D H v
v  = D*v; v = v/(v.norm()+1e-12)   # velocity in the preconditioned metric
print("preconditioned", flush=True)

alpha_meas={"Emb":-0.13,"LayerNorm":-0.16,"W_Q":-0.21,"W_K":-0.21,
            "W_V":-0.25,"W_O":-0.25,"FF":-0.27}
print("\n"+"="*78); print("  DOES PRECONDITIONED CURVATURE (D*H) DECIDE ROTATION?"); print("="*78)
...
json.dump(rows, open("curv_rot_D.json","w"), indent=1, default=float)
