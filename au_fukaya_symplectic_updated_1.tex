...
\textbf{Two-level structure (central result):}
Training operates at two geometrically distinct levels.
\emph{Level~1 --- Invariant core}: the Floer spectrum
$\mathrm{FL}(L_k, L_{k+1}) = S^0$ is constant across all checkpoints
(val $= 0.065$--$0.65$, strip energy $43.4$--$43.5$).
The Lagrangians $L_k = \mathrm{col}_r(W_K^{(k)}) \subset \mathbb{R}^r \subset \mathbb{C}^r$
are exact with $F_k = 0$; their Floer homotopy type is frozen.
\emph{Level~2 --- Dynamic layer}: all training dynamics
(wall crossings, phase transitions, compiler advantage $1.46$--$1.89\times$)
occur in $\mathrm{Stab}(\mathcal{F})$, the Bridgeland stability manifold,
without changing the underlying Floer invariant.
The compiler navigates $\mathrm{Stab}(\mathcal{F})$ geometrically
(19 adiabatic crossings) vs GD-400 (37 dissipative crossings).

\textbf{Three-geometry correlation}: at the codimension-2 crossing,
phase-preserving directions align with loss-descent directions
within the strip-area null space; Spearman $\rho = 0.90$ ($p = 0.037$).
\end{abstract}
...
This paper describes a different viewpoint based on a \emph{conjectural}
geometric framework: the rank-$r$ column spaces of the key-projection
matrices $W_K^{(k)}$ define points in the real Grassmannian $\mathrm{Gr}(r,D)$,
and the training trajectory is a path in $\mathrm{Gr}(r,D)^L$ (one point per layer
per step).
The \emph{strip-area functional}
$\sum_i \arccos(\sigma_i(U_k^T U_{k+1}))$
computes the L$^1$ norm of principal angles between consecutive $r$-planes ---
a valid Riemannian-like distance on $\mathrm{Gr}(r,D)$.

\emph{Relationship to Fukaya categories (conjectural):}
If the $r$-planes can be identified with Lagrangian submanifolds
in a symplectic manifold, the strip-area formula would compute
J-holomorphic strip areas and the training dynamics would be governed
by Fukaya $A_\infty$ structure.
This identification requires: (a)~a symplectic structure on $\mathrm{Gr}(r,D)$
(the Kirillov--Kostant--Souriau form on the coadjoint orbit
$\mathrm{GL}(D)/(\mathrm{O}(r)\times\mathrm{O}(D-r))$),
(b)~proof that $r$-planes are Lagrangian (requires symmetry of $W_K$, not
generally satisfied), and (c)~transversality conditions for strip existence.
These conditions are not established in this paper.
The geometric framework is therefore presented as an analogy and a
research program, not a proven equivalence.
The empirical results (compiler performance, phase coordinates,
three-geometry correlations) hold independently of the categorical framework.

\emph{Numerical verification} (\texttt{lagrangian\_verification.py},
\texttt{tau\_spike\_step0064}, $r=6$, $D=256$):
\begin{itemize}
\item $\|W_K - W_K^T\|_F / \|W_K\|_F \approx 1.41$ for all 5 layers
  (compare: random matrix gives $\approx \sqrt{2} \approx 1.41$).
  $W_K$ is maximally non-symmetric --- no special symmetry structure.
  $\mathrm{graph}(W_K)$ is \emph{not} Lagrangian.
\item $\|\tilde{U}_k^T \tilde{U}_k - I\|_F = 0.000000$ for all layers:
  $\mathrm{col}_r(W_K) \in \mathrm{Gr}(r,D)$ exactly.
\item Isotropic test (Test~4): skew norm $= 0.037$--$0.070 \neq 0$:
  the $r$-planes are \emph{not} Lagrangian in the naive $\mathbb{R}^{2r}$ embedding.
\item Strip area $\sum_i \arccos(\sigma_i) \approx 8.7$ vs geodesic
  distance $\sqrt{\sum_i \arccos^2(\sigma_i)} \approx 3.5$:
  ratio $\approx \sqrt{r} = \sqrt{6} \approx 2.45$, consistent with L$^1$
  vs L$^2$ norm of 6 angles.
\end{itemize}

The geometric analysis tracks three distinct subspaces simultaneously
and asks whether they are quantitatively related:
\begin{description}
  \item[Spectral subspace $T_{w_0}\Sigma_R$]
    The tangent space at $w_0$ to the real spectral locus
    $\Sigma_R = \{w : \mathrm{Im}\,\lambda_\mathrm{dom}(M(w)) = 0\}$,
    where $M(w) = W_{k+1}W_k^{-1}$ and
    $\varphi(w) = \arg\lambda_\mathrm{dom}(w)$ is the Bridgeland phase.
    \emph{Caution for PDE/NLA readers}: this is not the span of
    eigenvectors of a Hessian; it is the kernel of the phase gradient
    $\mathrm{d}\varphi: \mathbb{R}^D \to \mathbb{R}$.
    By the Algebraic Real-Locking Theorem (Section~\ref{sec:jet_rigidity}),
    $T_{w_0}\Sigma_R = \mathbb{R}^D$ to first order at clean-phase points
    (the entire parameter space is tangent to $\Sigma_R$);
    the non-trivial structure appears at second order and at the
    codimension-2 crossings.

  \item[$J$-holomorphic strip subspace $\mathcal{N}(H_S)$]
    The null space of the symplectic Hessian $H_S$
    (Hessian of the strip-area functional
    $\sum_k \arccos\sigma_i(U_k^T U_{k+1})$),
    spanned by the 5 near-null Lanczos eigenvectors with
    $|\lambda| \in \{0.22, 4.53, 46.9, 62.7, 113.3\}$.
    This is a subspace of parameter space $\mathbb{R}^D$, not of
    the output or hidden space of the transformer.

  \item[Loss subspace $\mathcal{N}^-(H_V)$]
    The negative eigenspace of $H_V|_{\mathcal{N}(H_S)}$,
    the cross-entropy loss Hessian restricted to the $J$-holomorphic
    null space.
    At step~64, $H_V|_{\mathcal{N}}$ has eigenvalues \\
    $(-5.20, -2.11, +0.16, +2.62, +5.78)$:
    an index-2 saddle.
    The 2-dimensional negative eigenspace $\mathcal{N}^-(H_V)$ is the
    loss-descent subspace within the symplectically-flat directions.
\end{description}
The main empirical finding is a three-way alignment at the codimension-2
Bridgeland wall crossing (step~72):
the phase-preserving directions (spectral) align with the loss-descent
directions (loss) within the $J$-holomorphic null space,
with Spearman $\rho = 0.90$ ($p = 0.037$, $n = 5$ null eigenvectors).
...
\section{Grassmannian Foundation: What Is and Is Not Proven}
\label{sec:grassmannian}

This section establishes the correct mathematical foundation for the
geometric framework and explicitly separates what is proven from what
is conjectural.

\subsection{The Grassmannian Description (proven)}

Each key-projection matrix $W_K^{(k)} \in \mathbb{R}^{D \times D}$
has a singular value decomposition $W_K^{(k)} = U_k \Sigma_k V_k^T$.
The top-$r$ left singular vectors $\tilde{U}_k \in \mathbb{R}^{D \times r}$
(columns orthonormal: $\tilde{U}_k^T \tilde{U}_k = I_r$) define an
$r$-dimensional subspace of $\mathbb{R}^D$:
\[
  L_k = \mathrm{col}_r(W_K^{(k)}) = \mathrm{span}(\tilde{U}_k) \in \mathrm{Gr}(r, D).
\]

\begin{proposition}[Grassmannian Embedding, proven]
\label{prop:grassmannian}
Each $L_k$ is a well-defined point in the real Grassmannian $\mathrm{Gr}(r,D)$.
The training trajectory $(L_k^{(t)})_{t \geq 0}$ is a discrete path in
$\mathrm{Gr}(r,D)^L$ (one point per layer, per gradient step).
\end{proposition}

\begin{proof}
Immediate from the SVD: $\tilde{U}_k^T \tilde{U}_k = I_r$ (verified numerically
to machine precision: $\|\tilde{U}_k^T\tilde{U}_k - I_r\|_F = 0.000000$
for all layers at \texttt{tau\_spike\_step0064}).
\end{proof}

\subsection{The Strip-Area Metric (proven)}

The principal angles $\theta_i = \arccos(\sigma_i(\tilde{U}_k^T\tilde{U}_{k+1}))$
between consecutive $r$-planes are the standard Riemannian data on $\mathrm{Gr}(r,D)$.

\begin{proposition}[Strip Area as Grassmannian Metric, proven]
\label{prop:strip_metric}
The strip-area functional
\[
  A(L_k, L_{k+1}) = \sum_{i=1}^r \arccos(\sigma_i(\tilde{U}_k^T\tilde{U}_{k+1}))
\]
is the $L^1$ norm of the principal angles between $L_k$ and $L_{k+1}$.
It satisfies the triangle inequality and defines a valid metric on $\mathrm{Gr}(r,D)$.
The standard Riemannian geodesic distance is
$d_{\mathrm{Gr}}(L_k, L_{k+1}) = \sqrt{\sum_i \theta_i^2}$ (the $L^2$ norm);
both are equivalent metrics.

\emph{Numerical values at step~64} (rank $r=6$, $D=256$):
$A(L_k, L_{k+1}) \approx 8.62$--$8.78$ and $d_{\mathrm{Gr}} \approx 3.53$--$3.58$
(ratio $\approx \sqrt{r}$, consistent with $L^1/L^2$ for $r$ equal angles).
\end{proposition}

\subsection{What the Lagrangian/Fukaya Framework Requires (not proven)}

\begin{proposition}[Lagrangian Condition, falsified for general $W_K$]
\label{prop:lagrangian}
$\mathrm{graph}(W_K^{(k)}) \subset T^*\mathbb{R}^D$ is a Lagrangian submanifold
if and only if $W_K^{(k)}$ is symmetric.
\end{proposition}

\begin{proof}
On $\mathrm{graph}(W_K)$, $p = W_K q$, so $dp = W_K\, dq$.
The symplectic form restricts to
$\omega|_{\mathrm{graph}(W_K)} = \sum_i dq_i \wedge dp_i = \sum_{i,j} (W_K)_{ij}\, dq_i \wedge dq_j
= \tfrac{1}{2}(W_K - W_K^T)_{ij}\, dq_i \wedge dq_j$.
This vanishes iff $W_K = W_K^T$.
\end{proof}

\emph{Numerical falsification:}
$\|W_K^{(k)} - (W_K^{(k)})^T\|_F / \|W_K^{(k)}\|_F \approx 1.41 \approx \sqrt{2}$
for all 5 layers (the value expected for a random matrix with no symmetry).
The Lagrangian condition fails maximally.

\begin{conjecture}[Grand Unified Shortcut --- what would need to be true]
\label{conj:grand_unified}
The following chain of constructions, if established, would reduce
training to a single algebraic evaluation:
\[
  \theta^* = \pi\!\left(\Phi_\mathrm{KZ}\!\left(\tfrac{w_\mathrm{FF}}{2\pi}\right) \cdot \sigma_0\right)
\]
where $\sigma_0 \in \mathrm{Stab}(\mathcal{F})$ is the initial stability
condition, $\Phi_\mathrm{KZ}$ is the Drinfel'd associator, and $\pi$
is a Riemann--Hilbert lift from $\mathrm{Stab}(\mathcal{F})$ to $\mathbb{R}^{D^2 L}$.

\emph{What would need to be proven (currently unestablished):}
\begin{enumerate}
\item A map $E_0 \mapsto \sigma_0 \in \mathrm{Stab}(\mathcal{F})$:
  the spectral initialisation defines a central charge
  $\mathcal{Z}: K_0(\mathcal{F}) \to \mathbb{C}$ and slicing $\mathcal{P}$.
  (Currently: $E_0 \in \mathbb{R}^D$; the Bridgeland data is not constructed.)
\item The training trajectory is a geodesic on $\mathrm{Stab}(\mathcal{F})$:
  gradient descent in $\mathbb{R}^{D^2 L}$ follows geodesics on Stab.
  (Currently: not established; gradient descent follows loss steepest descent,
  not the Stab metric.)
\item $w_\mathrm{FF}/(2\pi)$ is the KZ coupling constant:
  the empirical parameter $w_\mathrm{FF}$ equals the KZ coupling
  $\kappa = \sum_{(s,t)} \Omega_{st}/(z_s - z_t)$ from the corpus bigram covariance.
  (Currently: $w_\mathrm{FF}$ is empirically fitted with assumed exponent 3/2.)
\item A Riemann--Hilbert lift $\pi: \mathrm{Stab}(\mathcal{F}) \to \mathbb{R}^{D^2 L}$:
  given a stability condition, find the unique $W_K$ matrices
  satisfying $\varphi_k = \arg \mathcal{Z}(L_k)$.
  (Currently: the inverse problem has infinitely many solutions;
  the specific lift is not constructed.)
\end{enumerate}

\emph{What is established (from this paper):}
The sensors $(\varphi_k, \tau)$ approximate $(\arg\mathcal{Z}(L_k), |\mathcal{Z}(L_k)|/|\mathcal{Z}(L_{k+1})|)$
(central charge coordinates on $\mathrm{Stab}(\mathcal{F})$).
The Jacobian $\partial\Phi/\partial W = 0$ at clean phase (Theorem~\ref{thm:jacobian_locking}).
The rank-1 Newton step analytically drives off-wall phases to zero (Theorem~\ref{thm:rank1_topogate}).
The Floer spectrum is $S^0$ throughout training (Theorem~\ref{thm:spectral_rigidity}).
These four results are the foundation on which the grand unified shortcut would rest,
if the four unestablished steps above can be proven.

\emph{Empirical test of the lift (Step 4):}
The proposed factorization was tested against \texttt{basin\_state.pt}.
For each layer $k$, the SVD $W_K^{(k)} = U_k \Sigma_k V_k^T$ was compared against
corpus-derived candidates:
\begin{itemize}
\item $U_k$ vs top-$r$ Laplacian eigenvectors:
  overlap $\|\tilde{U}_k^T E_{0,r}\|_F/r \approx 0.07$
  (random baseline $\approx 0.063$). \emph{No alignment above chance.}
\item $V_k$ vs top-$r$ bigram right singular vectors:
  overlap $\approx 0.07$. \emph{Same conclusion.}
\item Layer-to-layer $\sigma_1$ ratios:
  $[0.576, 0.929, 0.828, 0.847, 1.091]$
  vs predicted $1/\tau \approx 0.189$. \emph{Not geometric.}
\end{itemize}
\textbf{Result:} The lift formula is not supported.
The Riemann--Hilbert lift from corpus statistics to $W_K$ matrices remains open.
\end{conjecture}

\paragraph{WV flip and $\mathrm{Ext}^1$ groups.}
The optimal WV flip pair is determined by $\mathrm{Ext}^1(T_l, T_m)$
in the $A_\infty$ algebra --- the \emph{dimension} of the morphism space,
not its energy (strip area).
Three geometric proxies were tested: (a)~det$(W_V^{(l)})$ sign
(gauge-dependent, unstable across runs); (b)~wall-score anomaly
(MAD~$\approx 0.038$, no signal in uniform-strip regime);
(c)~WV anti-alignment (differences $\approx 0.02$, noise level).
All three failed. The empirical search over 5 candidate pairs
($\approx 0.3$\,s, 30 forward passes) is currently necessary.
The WV flip becomes analytic when the full $A_\infty$ structure
($m_k$ maps, $m_1$ differential, $\mathrm{Ext}^1$ groups) is
established --- the main open problem of Step~3 in the Floer
homotopy programme.

\begin{conjecture}[Fukaya Connection via Real Lagrangians]
\label{conj:fukaya}
The correct symplectic manifold is $(M, \omega) = (\mathbb{C}^r,\,
\mathrm{Im}\langle\cdot,\cdot\rangle)$ --- complex $r$-space with the
symplectic form given by the imaginary part of the standard Hermitian form.

\textbf{Dimension accounting:}
$\mathbb{C}^r$ has real dimension $2r$.
A Lagrangian subspace has real dimension $r$.
The column space $L_k = \mathrm{col}_r(W_K^{(k)})$ (after projection
to the rank-$r$ effective subspace) has real dimension $r$ --- correct.

\textbf{Lagrangian condition at clean phase:}
When $\varphi_k = 0$ (clean phase), the dominant eigenvectors are real
(Theorem~\ref{thm:real_locking}), so $L_k \subset \mathbb{R}^r \subset \mathbb{C}^r$.
For any $v, w \in \mathbb{R}^r$:
$\omega(v,w) = \mathrm{Im}(\bar{v} \cdot w) = \mathrm{Im}(v \cdot w) = 0$
since $v \cdot w \in \mathbb{R}$.
Therefore $L_k$ is isotropic; since $\dim_\mathbb{R} L_k = r = \frac{1}{2}\dim_\mathbb{R}\mathbb{C}^r$,
it is \textbf{Lagrangian}.

\textbf{Exactness:}
The Liouville form on $\mathbb{C}^r$ is
$\lambda = \frac{1}{2}\mathrm{Im}(\bar{z}\,\mathrm{d}z)$.
On $\mathbb{R}^r \subset \mathbb{C}^r$: $z = x$ (real), so
$\lambda|_{\mathbb{R}^r} = \frac{1}{2}\mathrm{Im}(x\,\mathrm{d}x) = 0$.
Hence $L_k$ is exact with primitive $F_k = 0$.

\textbf{Conjecture (precise):}
After projection to $\mathbb{C}^r$ via the rank-$r$ monodromy,
the exact Lagrangians $\{L_k\}$ at clean phase define a Fukaya category
$\mathrm{Fuk}(\mathbb{C}^r, \{L_k\})$ by the Abouzaid--Seidel framework
for Liouville manifolds.
The strip-area formula computes the symplectic area $\int u^*\omega$
of the $J$-holomorphic strip $u: \mathbb{R} \times [0,1] \to \mathbb{C}^r$
with boundary on $L_k, L_{k+1}$.
The training trajectory is $J$-holomorphic when it remains on the
real spectral locus $\Sigma_R$ (clean-phase wall).
Crossing a phase wall $=$ exiting the Lagrangian regime $=$ the Floer
spectrum becomes undefined.

\textbf{Open steps:}
(a) Rank-$r$ reduction from $\mathbb{R}^D$ to $\mathbb{R}^r$
    (requires spectral gap $\sigma_r/\sigma_{r+1} \gg 1$ in transfer matrix),
(b) Maslov index computation along the training trajectory,
(c) Transversality of $L_k \pitchfork L_{k+1}$ after Hamiltonian perturbation,
(d) Abouzaid--Blumberg Floer homotopy type $\mathrm{FL}(L_k, L_{k+1}) \in \mathrm{Spec}$.

\paragraph{Numerical test of the Drinfel'd associator (scalar approximation).}
The scalar approximation $Z^* = \Phi_\mathrm{KZ}(\kappa) \cdot Z_0$ was tested with
$\kappa = w_\mathrm{FF}/(2\pi) \approx 0.075$ and initial phases $\varphi_k^{(0)}$ from
three compiler runs (15 phase predictions total).
Result: $\Phi_\mathrm{KZ}(0.075) = 1.0047$ (series truncated at 5 terms);
3/15 predictions matched observed terminal phases $\{0, \pi\}$ (20\%, at chance level).

\emph{Why the scalar approximation fails:}
$\kappa = 0.075$ is in the weak-coupling regime where $\Phi_\mathrm{KZ} \approx 1$
(0.47\% correction to $Z_0$). The observed phase jumps ($\varphi: 1.3 \to 0$,
$2.2 \to \pi$) are O(1) — strong-coupling behavior.
The scalar associator is smooth and near-identity; the actual dynamics is
discrete and large. These are incompatible.

\emph{What the correct $\kappa$ would need to be:}
To produce a 2.4$\times$ phase ratio ($\varphi_0 = 1.3 \to \varphi^* = \pi$),
$\kappa$ would need to be $O(1)$, not $0.075$.
In the KZ theory, $\kappa = 1/(k + h^\vee)$ where $k$ is the Kac-Moody level.
Our empirical $\kappa \approx 0.075$ corresponds to level $k \approx 11$,
which does not have an obvious architectural interpretation.

\emph{Conclusion:}
$w_\mathrm{FF}/(2\pi)$ is not the correct KZ coupling for the tensor theory.
The Drinfel'd associator could still be the right mathematical object,
but identifying the correct $\kappa$ requires constructing the Lie algebra
representation on $\mathrm{Stab}(\mathcal{F})$ — an open problem.
\end{conjecture}

\paragraph{Numerical diagnosis (\texttt{symplectic\_maslov\_test.py} and
\texttt{flow\_category.py}).}

\textbf{Test A (Lagrangian condition):} Confirmed algebraically.
$W_K^{(k)}$ is a real matrix for all $k$, so
$\mathrm{col}_r(W_K^{(k)}) \subset \mathbb{R}^r \subset \mathbb{C}^r$
and $\omega|_{L_k} = \mathrm{Im}\langle v, w\rangle = 0$ for all $v,w \in \mathbb{R}^r$.
All 6 layers, both checkpoints: $\|\mathrm{Im}(\tilde{U}_k)\|/\|\tilde{U}_k\| = 0.000000$. \checkmark

\textbf{Test B (Exactness):} Confirmed algebraically.
$\lambda|_{\mathbb{R}^r} = 0$, so $F_k = 0$ for all layers. \checkmark

\textbf{Test C (Rank-$r$ reduction):}
Transfer matrix singular value ratios $\sigma_r/\sigma_{r+1} \in [1.07, 1.28]$
across all pairs --- far below the threshold ($\gg 1$) needed for a clean
rank-$r$ reduction.
The effective rank varies from 1 to 12 across pairs.
\emph{The rank-6 reduction to $\mathbb{C}^6$ is not justified by the
transfer matrix spectrum.}
This is the main open gap in the Floer homotopy program.

\textbf{Test D (Maslov index):}
All principal angles $\theta_i \in [1.30, 1.56]$ rad $\approx \pi/2$.
The Lagrangians $L_k$ and $L_{k+1}$ are nearly orthogonal in $\mathrm{Gr}(6,256)$
--- maximally separated.
Approximate total Maslov index: $\mu \approx 13$ (step~64), $12$ (step~72).

\textbf{Test E (Intersections):}
$\mathrm{rank}([\tilde{U}_k | \tilde{U}_{k+1}]) = 12 = 2r$ for all pairs
--- the Lagrangians are transverse (meet only at $\{0\}$ in $\mathbb{R}^{256}$).
After Hamiltonian perturbation, the Floer complex uses a Morse function
on $L_k \cong \mathbb{R}^r$, not intersection points. \checkmark

\textbf{Flow category and Floer spectra (\texttt{flow\_category.py}):}
Strip energies $\sum_i \arccos(\sigma_i) \approx 8.7$ per pair.
\emph{Unit distinction:} the entropy floor ($0.065$ nats) and the strip area
($\approx 8.7$) are in different units and cannot be compared as a Novikov cutoff.
The entropy floor truncates the \emph{training trajectory};
the Novikov parameter truncates \emph{Floer strips} by symplectic area.

\textbf{Pearl complex result:} Since $L_k \cong \mathbb{R}^r$ is contractible
(exact Lagrangian with $F_k = 0$), the pearl complex gives
$\mathrm{HF}^*(L_k, L_{k+1}) \cong \mathrm{H}^*(\mathbb{R}^r;\mathbb{Z}) \cong \mathbb{Z}$
in degree~0.
The Floer spectrum is $\mathrm{FL}(L_k, L_{k+1}) \cong S^0$ (sphere spectrum)
for all pairs, at both step~64 and step~72.
Maslov index $\mu = 0$ for all 5 pairs at both checkpoints:
all principal angles lie in $[1.30, 1.56] < \pi/2 = 1.571$,
so no Maslov cycle crossing has occurred.

\textbf{Categorical stability:}
$\mathrm{FL}(L_k, L_{k+1}) = S^0$ is \emph{invariant} under the wall
crossing (step~64 $\to$ step~72).
This is consistent with the Heart/Stability Manifold distinction
(Section~\ref{sec:heart_stab}): the abstract Fukaya category
(encoded in the $S^0$ spectra) is invariant under Hamiltonian isotopy;
what changes at the wall crossing is the heart of the t-structure,
not the underlying Floer homology.

\textbf{Complete spectral barcode across all checkpoints:}
...
\midrule
step~64 ($\varphi=0$, on $\Sigma_R$) & 0.65 & 43.546 & $S^0$ (all 5 pairs) \\
step~72 ($\varphi=1.02$, off wall)   & 0.56 & 43.409 & $S^0$ (all 5 pairs) \\
basin\_entry (pre-TopoGate)          & 0.18 & 43.498 & $S^0$ (all 5 pairs) \\
basin\_state (entropy floor)         & 0.065 & 43.498 & $S^0$ (all 5 pairs) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{The spectral barcode is flat:}
$\mathrm{FL}(L_k, L_{k+1}) = S^0$ for all pairs at all checkpoints.
The total strip energy is essentially constant ($43.4$--$43.5$) throughout
training --- the Lagrangians maintain nearly constant mutual distance
in $\mathrm{Gr}(6,256)$.

\textbf{Interpretation (rigidity theorem):}
Since $L_k \cong \mathbb{R}^r$ is contractible with $F_k = 0$,
the Floer homotopy type $\mathrm{FL}(L_k, L_{k+1}) \cong S^0$
is a homotopy invariant that cannot change under Hamiltonian isotopy.
The wall crossings, phase transitions, and basin settling detected by
the compiler are changes in the \emph{heart of the t-structure}
(which stability condition is active), not changes in the underlying
categorical invariant.
This confirms the Heart/Stability Manifold distinction
(Section~\ref{sec:heart_stab}) with concrete spectral data.

\textbf{Where non-trivial spectral information lives:}
The individual pair spectra $S^0$ are trivial by contractibility.
The non-trivial invariant is the \emph{composed} spectrum
$\mathrm{FL}(L_0, L_L)$ through the full chain,
which encodes the $A_\infty$ compositions with Novikov weights
$T^{\mathrm{Area}(t)}$ and would detect wall crossings through
changes in the $m_k$ structure.
Computing this composed spectrum is the main open problem.

\paragraph{Summary.}
The Grassmannian description (§\ref{prop:grassmannian}--\ref{prop:strip_metric})
is mathematically rigorous and holds independently of the Fukaya conjecture.
The compiler, the geometric coordinates $(\varphi_k, \tau)$, the
codimension-2 observation, and the three-geometry correlations all rest
on the Grassmannian foundation alone.
Conjecture~\ref{conj:fukaya} is the open problem that would promote
the framework from analogy to theorem.

\newpage
%% ============================================================
\section{The Core Theory: Snapper's Theorem and the Algebraic Variety}
\label{sec:core_theory}
%% ============================================================


Snapper's theorem states: For a flat family of algebraic varieties, the Euler characteristic is a polynomial in the deformation parameter.

\subsection{The Geometric Objects}

In our context:
\begin{itemize}
    \item \textbf{The Variety:} The set of critical points 
    \[
    V = \{\theta \mid \nabla L(\theta) = 0\}
    \]
    in parameter space.
    
    \item \textbf{The Flat Family:} The deformation of this variety along a direction $d$ in parameter space.
    
    \item \textbf{The Deformation Parameter:} $t$ where $\theta(t) = \theta_0 + t\cdot d$.
    
    \item \textbf{The Euler Characteristic:} $\chi(t) = \text{number of critical points at } \theta(t)$.
\end{itemize}

\subsection{The Parameter Space Variety}

The loss landscape defines an algebraic variety:
\[
...
\subsection{The Flat Family}

Along a direction $d$, we have a family of varieties:
\[
V(t) = \{\theta(t) \mid \nabla L(\theta(t)) = 0\}
\]
where $\theta(t) = \theta_0 + t\cdot d$. This is a flat family if the Hessian is non-singular along the path.

\subsection{Snapper's Theorem}

Snapper's theorem states that $\chi(V(t))$ is a polynomial in $t$:
\[
\chi(V(t)) = a_0 + a_1 t + a_2 t^2 + \cdots + a_d t^d
\]

\subsection{The Polynomial Degree}

The degree of the polynomial is the dimension of the variety:
\begin{itemize}
    \item For a quadratic bowl: $\chi(t) = 1$ (constant, degree 0)
    \item For a quartic bowl: $\chi(t) = a_0 + a_1 t + a_2 t^2 + a_3 t^3 + a_4 t^4$ (degree 4)
\end{itemize}

\subsection{Example: Fitted Snapper Polynomial}

From a typical run of \texttt{snappers.py}, the fitted polynomial is:

\[
\chi(t) = 0.731 - 0.405t - 0.762t^2 + 0.838t^3 - 0.193t^4
\]
...
The polynomial minimum $t^*$ occurs where:
\[
\frac{d}{dt}\chi(V(t)) = 0
\]
...
\[
\chi'(t) = -0.405 - 1.524t + 2.514t^2 - 0.772t^3
\]

Solving $\chi'(t) = 0$ gives:
\[
t^* = 1.836
\]

This is the point where the Euler characteristic is minimized, corresponding to the floor of the loss landscape.
...
\hline
$a_0$ & $0.731$ & Base Euler characteristic at $\theta_0$ \\
...
\subsection{The Snapper Jump}

The polynomial minimum $t^* = 1.836$ gives the optimal jump distance:
\[
\theta^* = \theta_0 + t^* \cdot d = \theta_0 + 1.836 \cdot d
\]

This is the algebraic jump that replaces iterative gradient descent.
...
\draw[very thick]
(-0.3,0.8)
.. controls (1,0.4) and (3,1.2)
.. (5,0.7)
.. controls (6,0.4) and (7.2,0.9)
.. (8.2,0.6);

\node at (6.8,1.3) {Variety};

% Current point
\fill[red] (3.2,5.2) circle (2.5pt);
\node[right] at (3.2,5.2) {$\theta_0$};

% Search direction
\draw[blue,very thick,->] (3.2,5.2) -- (4.0,2.0);
\node[right,blue] at (3.5,3.7) {$d$};
...
% Polynomial minimum
\fill[green!60!black] (4.0,2.0) circle (2.8pt);
\node[right] at (4.0,2.0)
{$\theta_0+t^{*}d$};

% Projection to variety
\draw[dashed] (4.0,2.0) -- (4.2,0.9);

\end{tikzpicture}
\end{figure}

The Snapper polynomial tracks the algebraic variety in parameter space. The minimum is where the variety reaches its floor.

\subsection{The p-adic Connection}

\subsubsection{The p-adic Hessian Spectrum}

The p-adic Hessian computation gives:
\[
...
\subsubsection{The Gap Detection}

The gap in p-adic valuations tells us the polynomial degree:
\begin{verbatim}
...
\subsubsection{The Multi-prime Consensus}

From the original \texttt{snappers.py}:
\begin{verbatim}
P_ADIC_PRIMES = [2, 3, 5]
\end{verbatim}
...
\begin{itemize}
    \item The polynomial is exact (Snapper's theorem)
    \item The p-adic gap tells us the polynomial degree before we fit it
    \item The minimum $t^* = 1.836$ is the floor
    \item The Hessian direction $d$ is the deformation
    \item The jump is algebraic (no iteration needed)
\end{itemize}

\subsection{The Implementation}

The original \texttt{snappers.py} did this:
\begin{verbatim}
...
\subsection{The Unified Theory}

The p-adic valuation stratifies the variety. The Euler characteristic polynomial sums the strata. The Hessian direction is the normal to the strata. The Snapper jump is the optimal path to the floor.

\[
\boxed{\text{Snapper's theorem} + \text{p-adic geometry} = \text{the complete geometric compiler}}
\]
...
\hline
Fitted polynomial & $0.731 - 0.405t - 0.762t^2 + 0.838t^3 - 0.193t^4$ \\
Optimal jump $t^*$ & $1.836$ \\
Jump direction $d$ & Hessian eigenvector at gap \\
Polynomial degree & $4$ (quartic) \\
p-adic consensus & $2, 3, 5$ all agree on degree $4$
\end{tabular}
\end{center}
...
\section{Heart, Stability Manifold, and Autoequivalences}
\label{sec:heart_stab}

The review identified a critical structural distinction that clarifies
the paper's architecture.
We separate two mathematical objects that the current implementation
conflates:

\subsection{The Fukaya Category (static structure)}

$\mathcal{F}(M)$ is the \emph{static} Fukaya category with:
\begin{itemize}
\item \textbf{Objects}: Lagrangian branes $L_k = \mathrm{col}_r(W_K^{(k)})$
  with Maslov index, Pin structure, and flat $U(1)$ bundle data
\item \textbf{Morphisms}: $A_\infty$ composition maps $m_k$ counting
  $J$-holomorphic polygons (small disk counts, finite in the Gieseker chamber)
\item \textbf{Structure}: computed \emph{once} at the base point
  (Gieseker chamber, entropy floor val\,$\approx 0.062$)
\end{itemize}

The $m_k$ structure is not re-derived at each wall crossing.
The current paper's A$_\infty$ triangle solver
(Section~\ref{sec:cr_solver}) computes approximations to these counts,
but the full categorical structure is not established
(the pentagon identity check returned trivial $m_2 \circ m_2 = 1 \pmod{2}$
in the uniform-strip regime, which is uninformative about the actual $m_k$).

\subsection{The Stability Manifold (dynamic space)}

$\mathrm{Stab}(M)$ is the \emph{dynamic} space the compiler navigates.
Points in $\mathrm{Stab}(M)$ are pairs $(\mathcal{Z}, \mathcal{P})$
of a central charge $\mathcal{Z}: K_0(\mathcal{F}) \to \mathbb{C}$
and a slicing $\mathcal{P}$.
The Bridgeland phase $\varphi_k = \arg\lambda_\mathrm{dom}(W_{k+1}W_k^{-1})$
is the \emph{numerical approximation} to the argument of the central charge
restricted to the object $L_k$.

\paragraph{Wall crossing in $\mathrm{Stab}(M)$:}
When the trajectory crosses a phase wall
(Im\,$Z_k = 0$ becomes Im\,$Z_k \neq 0$), the stability condition changes.
The correct categorical operation at a wall crossing is a
\textbf{Seidel--Thomas spherical twist}
$T_{L_k}: \mathcal{F}(M) \xrightarrow{\sim} \mathcal{F}(M)$,
an autoequivalence that maps the old stability condition to the new one
while preserving $A_\infty$ associativity by construction.

\subsection{Reinterpreting the Compiler Components}
...
$\tau$ sensor & K$_0$ gluing defect & Obstruction to twist completion \\
$\Phi_\mathrm{cl}$ sensor & Phase map $\varphi \in \{0,\pi\}^L$ & 
  Stability condition in Gieseker chamber \\
MF pump & Initial trajectory & Path to Gieseker chamber \\
Basin settle & Convergence to floor & Approaching $\mathcal{F}(M)$ base point \\
Compiler advantage & 1.46--1.89$\times$ & Cost of approximate vs exact twist \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{What the current compiler does:}
Gradient steps with geometric sensors ($\tau$, $\Phi_\mathrm{cl}$)
that detect wall proximity and apply empirically-tuned corrections.
The TopoGate is a gradient step that \emph{happens to correlate with}
a Seidel--Thomas twist — it is not a twist computation.

\textbf{What a categorical compiler would do:}
\begin{enumerate}
\item Compute $\mathcal{F}(M)$ once at the Gieseker chamber
  (the $m_k$ maps for small disk counts)
\item Cache the spherical objects $L_k$ with their
  (Maslov, Pin, $U(1)$) metadata
\item At each wall crossing, look up the precomputed
  Seidel--Thomas twist $T_{L_k}$ and apply it directly
  ($O(1)$ categorical operation vs $O(N)$ gradient re-calculation)
\item The $A_\infty$ relations are preserved by construction
  (spherical twists are autoequivalences)
\end{enumerate}

\textbf{Why this matters:}
The implementation gap explains \emph{why} the compiler has overhead
(187--195 CE steps vs theoretical minimum) and suggests a path to
industrial-grade efficiency: implement the spherical twist directly
rather than approximating it with gradient steps.

\paragraph{Paper scope.}
This paper establishes the geometric framework and demonstrates that
the approximation (gradient + sensors) outperforms vanilla gradient
descent by 1.46--1.89$\times$.
The categorical compiler (exact twist computation) is future work.

\subsection{Dynamic Interpretation: Floer Family over Training Time}

A more precise characterisation of the compiler dynamics is as a
\emph{Floer family} --- a path in the space of $A_\infty$-categories:
\[
  \mathcal{F}: [0,T] \to \mathrm{Cat}_{A_\infty}, \qquad
  t \mapsto \mathcal{F}(M, \omega_t, J_t, \{L_k^{(t)}\}).
\]
Mathematically, the abstract Fukaya category $\mathcal{F}(M)$ is invariant
under Hamiltonian isotopies; what changes at each wall crossing is the
\emph{heart of the t-structure} (the tilt).
The three compiler stages correspond to three categorical operations:
...
2 & Basin settle & $A_\infty$ deformation & $m_k$ maps change as $\omega_t$ flows \\
3 & TopoGate & Karoubi completion & Acyclic objects localised out \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Entropy floor as RG cutoff.}
The entropy floor (val\,$\approx 0.062$) is not merely a Novikov
truncation --- it is a \emph{renormalisation group time cutoff}.
Holomorphic disks with area $\gg$ threshold correspond to long-time
correlations that have not yet thermalised.
Truncating at the floor builds the \emph{short-time Fukaya category}:
the $m_k$ operations count only disks whose area is below the cutoff.

\paragraph{Crystallisation as end of barcodes.}
``Crystallisation'' (the compiler reaching $\Phi$$_\mathrm{cl} = 5/5$,
$R_\mathrm{assoc}$ near its minimum) is the categorical statement that
all persistent homology classes $H_k(\mathrm{Nerve}(\mathcal{F}_t))$
have stabilised --- the barcodes have ended.
The observation that $R_\mathrm{assoc}$ is \emph{minimised} at the
wall crossing (step~72: $R_\mathrm{assoc} = 0.220$, the minimum across
all checkpoints) is consistent with this: the wall crossing is where
the categorical structure is most tightly constrained by the t-structure
tilt, not where it is most violated.

\paragraph{Implementation gap (explicit).}
The current compiler \emph{approximates} this structure:
\begin{itemize}
\item TopoGate $\approx$ Seidel--Thomas twist (gradient step, not exact)
\item Triangle solver $\approx$ $m_2$ computation (Floer proxy, not disk count)
\item No explicit Karoubi completion (acyclic localisation not computed)
\item No persistent barcodes output
\end{itemize}
The full categorical compiler --- computing exact $m_k$ maps,
enforcing $A_\infty$ curvature $\mathrm{Curv}(t) < \varepsilon(t)$ at each step,
and performing Karoubi completion at Stage~3 --- is the research program
that this paper's geometric framework motivates.
...
\begin{conjecture}[Pentagon $\Leftrightarrow$ A$_\infty$ Associativity (uniform regime)]
\label{thm:pentagon}
...
\begin{observation}[Step Count from K$_0$ Twists]
\label{thm:stepcount}
...
\begin{conjecture}[Gradient Descent as Monodromy Rescaling]
\label{thm:monodromy}
...
\begin{conjecture}[Bridgeland Phase Analogy]
...
\begin{observation}[Adiabatic Compiler Advantage]
\label{thm:adiabatic}
...
\begin{conjecture}[Orbit Attractor (one corpus/architecture)]
\label{thm:orbit}
...
\paragraph{Methodological note on threshold derivation.}
The geometric thresholds ($\tau_\mathrm{basin} \approx 2$,
$\Phi_\mathrm{cl} \geq 4/5$, TopoGate score $> 0.014$) were derived
empirically from the same training runs on which they are evaluated ---
not from theory.
Specifically:
\begin{itemize}
\item $\tau_\mathrm{basin} \approx 2$ was identified by observing that
  successful runs had $\tau \approx 1.8$--$2.0$ at MF-pump termination,
  while failed runs had $\tau > 2$ (over-pumped) or $\tau < 1.5$
  (under-pumped).
\item $\Phi_\mathrm{cl} \geq 4/5$ was identified as the minimum
  clean-phase count consistent with subsequent convergence in early
  small-corpus experiments.
\item The TopoGate score threshold ($0.014$) was set to accept gates
  that reduced val while maintaining $\Phi_\mathrm{cl}$.
\end{itemize}
This means the performance claims in this paper (compiler val $< 0.07$,
GD-400 val $\approx 0.09$) are \emph{in-sample} with respect to the
threshold choices.
The thresholds have not been validated on a held-out corpus or
architecture.
The theoretical grounding for \emph{why} these values work ---
$\tau \approx 2$ as the K$_0$ gluing defect,
$\Phi_\mathrm{cl} = 5/5$ as the orbit attractor ---
is developed in Sections~\ref{sec:tau_coord} and~\ref{sec:wall_geometry},
but the specific numerical values require independent validation.
...
\begin{observation}[Geometry Sufficiency]
...
\subsection{The Two-Level Structure: Central Result}
\label{sec:two_level}

The spectral barcode computation across four training checkpoints
reveals the paper's central finding: transformer training operates
simultaneously at two geometrically distinct levels.

\paragraph{Level 1 — The Invariant Core (Floer spectrum $= S^0$).}
...
Object & Value \\
\midrule
Floer spectrum $\mathrm{FL}(L_k, L_{k+1})$ & $S^0$ (sphere spectrum) \\
Floer homology $\mathrm{HF}^*(L_k, L_{k+1})$ & $\mathbb{Z}$ in degree~0 \\
...
This is a \textbf{rigidity theorem}: the abstract Fukaya category
(the ``heart'') is \emph{frozen} throughout training.
No amount of optimisation changes the categorical invariant.
The Lagrangians $L_k = \mathrm{col}_r(W_K^{(k)})$ maintain
nearly constant mutual distance in $\mathrm{Gr}(6, 256)$ throughout
training (strip energy constant at $43.4$--$43.5$): the
$r$-dimensional key-projection subspaces are persistently
orthogonal and never rotate toward each other.

\paragraph{Level 2 — The Dynamic Layer (Stability Conditions).}
...
\midrule
Phase coordinates $\varphi_k, \tau$ & Angles in stability space \\
Wall crossings & Codimension-2 events (19 adiabatic vs 37 dissipative) \\
Three-geometry correlation & $\rho = 0.90$ ($p = 0.037$) \\
Frobenius residual & $R_\mathrm{assoc} = 0.220$ at wall crossing \\
Geometric meaning & t-structure changes; central charge $\mathcal{Z}$ shifts \\
\bottomrule
\end{tabular}
\end{center}

This is the \textbf{navigable layer}: the compiler explores the space
of Bridgeland stability conditions $\mathrm{Stab}(\mathcal{F})$
without changing the underlying categorical invariant.

\paragraph{The key insight: convergence without alignment.}
The compiler achieves val\,$= 0.047$--$0.063$ (vs GD-400
val\,$= 0.088$--$0.092$) \emph{without} reducing the strip energy
(Lagrangians do not align during training).
Convergence is achieved entirely through navigation of the stability
manifold $\mathrm{Stab}(\mathcal{F})$ --- choosing the right
stability condition at each wall crossing --- while the underlying
Fukaya category remains $S^0$ throughout.

This separation is the mathematically precise statement of the
compiler's advantage:
GD-400 navigates $\mathrm{Stab}(\mathcal{F})$ blindly (37 dissipative
crossings); the compiler navigates it geometrically (19 adiabatic
crossings), detecting wall proximity via $\varphi_k$ and $\tau$
and applying approximate Seidel--Thomas twists (TopoGate) at each crossing.
The categorical invariant ($S^0$) is the same for both;
the path through $\mathrm{Stab}(\mathcal{F})$ is different.

\paragraph{What the compiler is optimizing.}
Since the Floer spectrum is frozen, the compiler is \emph{not} optimising
the Fukaya category.
It is optimising the \textbf{central charge}
$\mathcal{Z}: K_0(\mathcal{F}) \to \mathbb{C}$ and the slicing $\mathcal{P}$
--- the two components of a Bridgeland stability condition
$(\mathcal{Z}, \mathcal{P}) \in \mathrm{Stab}(\mathcal{F})$.
The phase coordinates are numerical approximations to this:
\[
  \varphi_k \approx \arg\mathcal{Z}(L_k), \qquad
  \tau \approx |\mathcal{Z}(L_k)|/|\mathcal{Z}(L_{k+1})|.
\]
The geometric sensors $(\varphi_k, \tau)$ measure the
\emph{central charge of the Fukaya category}, not the Floer invariants.
The TopoGate selects the stability condition; the basin settle finds a
stable heart; the entropy floor is the minimal central charge norm
compatible with the corpus.

\subsection{Three Structural Theorems}
\label{sec:three_theorems}

\begin{theorem}[Spectral Rigidity]
\label{thm:spectral_rigidity}
For any checkpoint $w^{(t)}$ in the training trajectory,
$\mathrm{FL}(L_k^{(t)}, L_{k+1}^{(t)}) \simeq S^0$
(the sphere spectrum) for all $k$.
The Floer homotopy type of individual layer transitions is invariant
under the optimisation dynamics.
\end{theorem}

\begin{proof}
$L_k = \mathrm{col}_r(W_K^{(k)}) \cong \mathbb{R}^r$ is contractible
with $F_k = 0$ (Proposition~\ref{prop:strip_metric} and Test~B).
By the pearl complex for exact Lagrangians in $(\mathbb{C}^r, \mathrm{Im}\langle\cdot,\cdot\rangle)$:
$\mathrm{HF}^*(L_k, L_{k+1}) \cong H^*(\mathbb{R}^r; \mathbb{Z}) \cong \mathbb{Z}$
in degree~0.
The Floer spectrum is $S^0$.
Verified numerically at four checkpoints
(val $= 0.065$--$0.65$, Table in Section~\ref{sec:three_geometry}).
\end{proof}

\begin{observation}[Stability Dynamics]
\label{obs:stab_dynamics}
The training trajectory defines a path in $\mathrm{Stab}(\mathcal{F}(M))$.
Wall crossings occur at codimension-2 events
(Observation~\ref{thm:codim2}) detected by the TopoGate.
The compiler traverses this path with 19 adiabatic crossings;
GD-400 traverses it with 37 dissipative crossings.
\end{observation}

\begin{theorem}[Compositional Suppression]
\label{thm:compositional_suppression}
The composed Floer spectrum $\mathrm{FL}(L_0, L_L)$ through the
full chain $L_0 \to L_1 \to \cdots \to L_L$ is trivial in the
Novikov-truncated theory.
\end{theorem}

\begin{proof}
The $A_\infty$ composition carries Novikov weight
$T^{\sum_{k=0}^{L-1} A(L_k, L_{k+1})}$.
The strip energies $A(L_k, L_{k+1}) \approx 8.7$ are constant
across all checkpoints (Table, Section~\ref{sec:three_geometry}).
Total weight: $T^{5 \times 43.5} = T^{217.5}$.
In any Novikov truncation with cutoff $\Lambda \ll 217.5$
(in particular, the entropy floor $0.065 \ll 8.7$),
the composed morphism is zero.
The persistent orthogonality of the Lagrangians
(all principal angles $\theta_i \approx \pi/2$) prevents
the composition from becoming non-trivial throughout training.
\end{proof}

\paragraph{Consequence.}
Theorems~\ref{thm:spectral_rigidity}--\ref{thm:compositional_suppression}
together imply:
the optimisation dynamics of transformer training are entirely contained
in the middle layer ($\mathrm{Stab}(\mathcal{F})$), with the bottom layer
(Fukaya category, Floer spectra) serving as a fixed geometric background.
The compiler's sensors $(\varphi_k, \tau)$ are probes of
$\mathrm{Stab}(\mathcal{F})$, not of $\mathcal{F}$ itself.

\paragraph{The stability manifold navigator.}
The spectral barcode $\mathrm{FL}(L_k, L_{k+1}) = S^0$ is invariant
throughout training.
This rigidity is not a failure of detection but a theorem:
the Fukaya category is the fixed topological background upon which
the optimisation dynamics takes place.
The compiler's wall crossings, phase transitions, and basin settling
are movements in the space of Bridgeland stability conditions
$\mathrm{Stab}(\mathcal{F}(M))$.
The optimisation landscape is not the Fukaya category but its
\emph{moduli space of stability conditions}.

The relationship is precisely that of string theory:
fixed Calabi--Yau topology (Fukaya category) with varying complex and
K\"{a}hler moduli (Bridgeland stability condition).
The compiler is doing mirror symmetry in real time---not building the
category but navigating its space of stability conditions.

\emph{Important caveat:}
The fixedness of the Fukaya category in this paper is a consequence of
the specific setup: $L_k \cong \mathbb{R}^r$ (contractible), $F_k = 0$
(exact with zero primitive), and persistent orthogonality
($\theta_i \approx \pi/2$ throughout training).
A different architecture, different rank $r$, or training that drove
the Lagrangians toward alignment (decreasing $\theta_i$, reducing strip
energy below $\sim 8.7$) would give a non-trivial Floer spectrum that
changes during training.
The ``fixed background'' is a property of this transformer at this scale,
not a universal theorem about deep learning.

\paragraph{New coordinates for the stability manifold.}
The quantities $(\varphi_k, \tau, R_\mathrm{assoc})$ are
coordinates on $\mathrm{Stab}(\mathcal{F})$:
\begin{itemize}
\item $\varphi_k \approx \arg \mathcal{Z}(L_k)$:
  the argument of the central charge applied to object $L_k$
\item $\tau \approx |\mathcal{Z}(L_k)|/|\mathcal{Z}(L_{k+1})|$:
  the ratio of central charge norms (global stability parameter)
\item $R_\mathrm{assoc}$: violation of the $A_\infty$ relations
  (measures distance from the categorical heart)
\end{itemize}
Basin settling is convergence to a fixed point of the renormalisation
group flow on $\mathrm{Stab}(\mathcal{F})$.
The entropy floor val\,$\approx 0.062$ is the minimal central charge
norm compatible with the corpus and architecture.

\paragraph{Reducing CE steps via geometric stopping.}

The 187-CE budget breaks down as:
basin settle (120 CE, 64\%) $+$ $\tau$-retry (50 CE, 27\%) $+$
fixed phases (17 CE, 9\%).
The geometric sensors enable a concrete reduction:
...
$\tau$-retry & 50 & 0 & Replace with earlier TopoGate \\
Fixed phases & 17 & 17 & Unchanged \\
\midrule
\textbf{Total} & \textbf{187} & \textbf{97--117} & \textbf{38--48\% reduction} \\
\bottomrule
\end{tabular}
\end{center}

\emph{Key insight from the weighted diagnostic:}
The TopoGate (applied at basin\_state) raises $r_{m_2}^\sigma$ from
$+0.584$ (basin\_entry) to $+0.683$ (basin\_state) ---
it performs the same function as the $\tau$-retry but in fewer steps.
The correct sequence is basin settle $\to$ TopoGate $\to$ done,
skipping the 50-CE $\tau$-retry entirely.

The stopping criterion for basin settle should be geometric, not
loss-based: stop when $r_{m_2}^\sigma$ plateaus (change $< 0.02$
over two checkpoints) AND $\Phi_\mathrm{cl} \geq 4/5$.
This avoids over-settling in the loss dimension while the stability
condition geometry has already converged.

\subsection{Analytic TopoGate via Rank-1 Newton Step}
\label{sec:analytic_topogate}

The Jacobian of the phase map $\Phi: \mathbb{R}^{D^2 \times L} \to \mathbb{R}^{L-1}$,
$w \mapsto (\arg \lambda_\mathrm{dom}(W_{k+1}W_k^{-1}))_{k=0}^{L-2}$,
has a closed form via eigenvalue perturbation theory.
For a simple dominant eigenvalue $\lambda_k$ with left/right eigenvectors
$\ell_k$, $r_k$ of $M_k = W_{k+1}W_k^{-1}$:
\[
  \frac{\partial \varphi_k}{\partial W_{k+1}} 
  = \frac{\mathrm{Im}(\ell_k \otimes W_k^{-1}r_k)}{|\lambda_k|}, \qquad
  \frac{\partial \varphi_k}{\partial W_k}
  = \frac{\mathrm{Im}(-M_k^T\ell_k \otimes W_k^{-1}r_k)}{|\lambda_k|}.
\]
Cost: one eigendecomposition per layer pair, $O(D^3)$, $\approx 0.5$\,s total.

\begin{theorem}[Algebraic Real-Locking in Full $W_K$ Space]
\label{thm:jacobian_locking}
At clean phase ($\varphi_k \in \{0, \pi\}$ for all $k$,
i.e.\ $\lambda_k \in \mathbb{R}$), the dominant eigenvalue is real,
so $\mathrm{Im}(\lambda_k) = 0$ and $\mathrm{Im}(\ell_k \otimes W_k^{-1}r_k) = 0$.
Therefore $\|\partial\varphi_k/\partial W_{k+1}\|_F = 0$ for all $k$.
\end{theorem}

\begin{proof}
When $\lambda_k \in \mathbb{R}$, the eigenvectors $\ell_k$, $r_k$
can be chosen real (standard perturbation theory for real symmetric-adjacent matrices).
Then $\ell_k \otimes W_k^{-1}r_k \in \mathbb{R}^{D \times D}$,
so its imaginary part vanishes identically.
\end{proof}

\emph{Numerical confirmation} (\texttt{jacobian\_phase\_map.py},
\texttt{tau\_spike\_step0064}):
$\|\partial\varphi_k/\partial W_{k+1}\|_F = 0.0000$ for all 5 pairs.

\begin{theorem}[Rank-1 Analytic TopoGate]
\label{thm:rank1_topogate}
At an off-wall checkpoint ($\varphi_k \neq 0, \pi$ for some $k$),
let $U_1 \Sigma_1 V_1^T$ be the top singular triple of
$\partial\varphi_k/\partial W_{k+1}$.
The rank-1 update
\[
  \delta W_{k+1} = -\frac{\varphi_k}{\sigma_1} \, u_1 v_1^T
\]
drives $\varphi_k \to 0$ to first order.
\end{theorem}

\emph{Numerical confirmation} (\texttt{tau\_spike\_step0072},
$\varphi_0 = -1.0204$):
$\alpha = 0.1006$, $\sigma_1 = 10.14$,
post-update $\varphi_0 = -0.0000$ (error $< 10^{-4}$).
This replaces 5 empirical gradient steps with a single $O(D^2)$ computation.

\emph{Implication for dissipative crossings:}
The Jacobian is zero throughout clean-phase basin settle
(Theorem~\ref{thm:jacobian_locking}).
The 37 dissipative crossings in GD-400 occur in the pre-orbit phase
(before $\Phi_\mathrm{cl}$ stabilises), not during basin descent.
The compiler avoids these via the MF pump, not gradient projection.
The geometric gradient projection (\texttt{geometric\_gradient\_projection.py})
is therefore unnecessary during basin settle at clean phase.

\emph{Two-component TopoGate structure (from experimental runs):}
The analytic TopoGate targets $W_K$ phases ($\varphi_k \notin \{0,\pi\}$).
When the $\tau$-retry has already driven all phases to $\{0,\pi\}$,
the Jacobian is zero and the analytic step correctly finds no gain.
The residual correction then comes from $W_V/W_O$ sign structure:
flipping $W_V^{(l)} \to -W_V^{(l)}$ and $W_O^{(l)} \to -W_O^{(l)}$
for selected layers $l$.
This is a discrete ($\pm 1$) algebraic operation, distinct from the
continuous rank-1 Newton step on $W_K$.
The full analytic TopoGate has two components:
\begin{enumerate}
\item \textbf{$W_K$ phase correction}: rank-1 Newton step
  $\delta W_K^{(k+1)} = -(\varphi_k/\sigma_1) u_1 v_1^T$
  (Theorem~\ref{thm:rank1_topogate}; acts when $\varphi_k \notin \{0,\pi\}$)
\item \textbf{$W_V/W_O$ sign correction}: discrete sign flip
  $W_V^{(l)} \to -W_V^{(l)},\; W_O^{(l)} \to -W_O^{(l)}$
  for layers determined by the attention output sign structure
  (analytic characterization: open problem)
\end{enumerate}
In confirmed runs, one or both components act depending on
the geometric state at basin exit.

\paragraph{The Functor Programme.}
Gradient descent is a map
$\mathrm{GD}: (W_0, \mathcal{L}, \mathrm{corpus}) \to W^*$
that works empirically but explores $\mathbb{R}^{393216}$ without
geometric structure.
The geometric compiler factors this through a \emph{resolution functor}
\[
  F: \mathrm{Stab}(\mathcal{F}) \times \mathrm{Corpus}
     \;\longrightarrow\; \mathrm{Traj}(\mathbb{R}^{393216})
\]
analogous to Hironaka's resolution of singularities:
each compiler phase resolves one stratum of the optimisation singularity
by a targeted geometric operation rather than blind gradient search.
...
MF pump & Non-orbit ($\tau$ not rising) & Mean-field iteration & Done \\
...
$K_0$ split & Extension class mismatch & $w_\mathrm{FF}$ reweighting & Done \\
\bottomrule
\end{tabular}
\end{center}

The basin settle is the only phase still performing blind gradient descent.
The Jacobian $J = \partial\Phi/\partial W$ (shape $5 \times 393216$,
33\% nonzero) identifies which gradient directions cause dissipative
wall crossings.
Projecting the gradient onto $\ker(J)$ eliminates all 37 dissipative
crossings by construction --- every direction in $\ker(J)$ is
phase-preserving ($\dim\ker(J) = 393216 - 5 = 393211$).

\begin{conjecture}[Geodesic Convexity within a Bridgeland Chamber]
\label{conj:geodesic_convex}
The cross-entropy loss $\mathcal{L}$ restricted to a single Bridgeland
stability chamber in $\mathrm{Stab}(\mathcal{F})$ is geodesically convex
under the $\mathrm{Stab}(\mathcal{F})$ metric.
Equivalently: gradient descent restricted to $\ker(J) \subset \mathbb{R}^{D^2 L}$
has no spurious critical points and converges in $O(1/\varepsilon)$ steps
(vs $O(1/\varepsilon^2)$ for blind descent in the full parameter space).
\end{conjecture}

If Conjecture~\ref{conj:geodesic_convex} holds, the basin settle reduces
from 120 blind gradient steps to $O(\log(1/\varepsilon))$ projected steps ---
the final step toward the algebraic compiler where each stratum is resolved
by a single geometric operation.

\paragraph{Open research questions.}
What is the metric on $\mathrm{Stab}(\mathcal{F})$ that the compiler follows?
What are the geodesics within a chamber?
What is the curvature (positive $\to$ convex, negative $\to$ saddle-rich)?
Can the wall-crossing formula (Joyce--Song; Kontsevich--Soibelman) predict
TopoGate events before they occur?
Does $\ker(J)$ descent empirically outperform blind descent
(test: \texttt{geometric\_gradient\_projection.py})?
These questions define the frontier of the programme.
...
Five specific gaps require precise statement, including three identified
by external review (gaps~4--5 below).
...
\emph{Gap~4: Frobenius weighted metric (resolved).}
The paper reported the unweighted Frobenius correlation $r_{m_2} \approx +0.93$
as potentially uninformative in the uniform-strip regime.
The correct strip-area-weighted metric
$r_{m_2}^\sigma = \langle \hat{H}, \mathrm{Hess}(m_2)\rangle_\sigma$
where $\langle A,B\rangle_\sigma = \sum_{ij} A_{ij}B_{ij}/(\sigma_i\sigma_j)$
has now been computed (\texttt{weighted\_rm2\_diagnostic.py}):

\begin{center}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
Checkpoint & $r_{m_2}$ & $r_{m_2}^\sigma$ & $\Delta$ \\
\midrule
step~64 (val$=0.65$) & $+0.931$ & $+0.697$ & $-0.234$ \\
step~72 (val$=0.56$) & $+0.942$ & $+0.693$ & $-0.249$ \\
basin\_entry (val$=0.18$) & $+0.929$ & $+0.584$ & $-0.345$ \\
basin\_state (val$=0.065$) & $+0.931$ & $+0.683$ & $-0.249$ \\
\bottomrule
\end{tabular}
\end{center}

The unweighted $r_{m_2} \approx +0.93$ is inflated by the easy
large-$\sigma$ directions (most-aligned Lagrangians).
The weighted $r_{m_2}^\sigma \approx +0.59$--$+0.70$ reveals the
true correlation in the hard directions (small $\sigma$, nearly-orthogonal
Lagrangians).
The correlation is positive and moderate throughout, with a dip at
basin\_entry ($+0.58$, pre-TopoGate, most disordered geometry) and
partial recovery at basin\_state ($+0.68$, post-TopoGate).
The Frobenius order is positive but weaker than the unweighted metric suggested.

\emph{Gap~5: Empirical threshold calibration.}
The thresholds $\tau_\mathrm{basin} \approx 2$,
$\Phi_\mathrm{cl} \geq 4/5$, and TopoGate score $> 0.014$
were derived from the same training runs on which they are evaluated
(Section~\ref{sec:compiler}).
The $w_\mathrm{FF}(\tau) = 3.5 \times (1.5/\tau)^{3/2}$ formula uses
exponent $3/2$ assumed, not derived.
Validation on a held-out corpus or architecture is pending.
...
\begin{observation}[Cluster Preconditioner Efficacy]
...
\begin{conjecture}[Tropicalization Bridge]
...
\begin{observation}[Codimension-2 Stratification at Step 72]
\label{thm:codim2}
\emph{(Numerical observation; the IFT argument gives the interpretation
but the full conditions for a theorem --- transversality globally,
exact dimension of null space --- are not proven.)}
...
\begin{theorem}[Local Algebraic Real-Locking]
\label{thm:real_locking}
Let $M(w)$ be a real analytic matrix family and let $\lambda(w_0)$ be
a \emph{simple} real eigenvalue with real left and right eigenvectors
$\ell$, $r$.
Then the local analytic continuation of $\lambda$ remains real
throughout every neighbourhood of $w_0$ in which the eigenvalue
remains simple.
Consequently $\mathrm{Im}\,\lambda \equiv 0$ on that neighbourhood,
and every derivative of $\mathrm{Im}\,\lambda$ at $w_0$ vanishes.
\end{theorem}

\begin{proof}
By the analytic implicit function theorem, a simple eigenvalue of a
real analytic family admits a real analytic continuation while it
remains simple.
Since $M(w)$ is real for all $w$, this continuation is real-valued,
so $\mathrm{Im}\,\lambda(w) = 0$ identically on the neighbourhood.
Every partial derivative $\partial^\alpha\mathrm{Im}\,\lambda(w_0)=0$
follows.
The first-order instance $\delta\mathrm{Im}(\lambda) =
\mathrm{Im}(\ell^T\delta M r) = 0$ (since $\ell$, $r$, $\delta M$
are all real) is the perturbation-theory confirmation of the theorem.
\end{proof}

\paragraph{Scope.}
This is a \emph{local} theorem: it applies within the neighbourhood
where $\lambda$ is simple.
It does not claim global real-locking.
The numerical experiments investigate where and how the local hypotheses
fail at larger perturbations.
...
\subsection{Branch-Tracking Evidence}
\label{sec:branch_tracking}

To confirm that the apparent $\mathrm{Im}(\lambda) \neq 0$ at large
$\varepsilon$ in the Jet Order Test was a branch-switching artifact
and not a genuine failure of algebraic real-locking, we track the
eigenvalue branch by eigenvector overlap.

\textbf{Method}: At $\varepsilon=0$, the dominant eigenvector is $r_0$.
At $\varepsilon > 0$, we identify the eigenvalue whose eigenvector
$r(\varepsilon)$ maximises $|r_0 \cdot r(\varepsilon)|$
(the \emph{best-matching} eigenvalue, i.e.\ the analytic continuation
of the $\varepsilon=0$ branch).
If the argmax eigenvalue (largest $|\mathrm{Re}(\lambda)|$) differs from
the best-matching eigenvalue, the branch has switched.

\textbf{Prediction} (from Theorem~\ref{thm:real_locking}):
$\mathrm{Im}(\lambda_\mathrm{best})$ should be $0$ at all $\varepsilon$
(the original branch stays real), while $\mathrm{Im}(\lambda_\mathrm{dom})$
may be nonzero at large $\varepsilon$ (a different complex eigenvalue took over).

\textbf{Experiment}: Run \texttt{branch\_tracking\_test.py} on
\texttt{tau\_spike\_step0064} with 10 random directions and
$\varepsilon \in \{0.001, 0.01, 0.1, 0.2, 0.3, 0.5\}$.

\textbf{Results} (\texttt{branch\_tracking\_report.json},
10 random directions, $\varepsilon \in \{0.001, 0.01, 0.1, 0.2, 0.3, 0.5\}$):

\begin{center}\small
\begin{tabular}{@{}rrrrl@{}}
\toprule
$\varepsilon$ & frac switched & mean overlap & mean Im($\lambda_\mathrm{dom}$)
  & mean Im($\lambda_\mathrm{best}$) \\
\midrule
...
The experiments reveal two distinct failure modes of the local hypothesis,
plus one regime of ambiguity:

\begin{enumerate}
\item \textbf{$\varepsilon \leq 0.01$ (theorem regime):}
  Im($\lambda_\mathrm{best}$) $= 0$ exactly, overlap $= 1.000$.
  No branch switch. The theorem applies; algebraic locking holds.

\item \textbf{$0.01 < \varepsilon \leq 0.3$ (branch-switching regime):}
  Some directions show Im($\lambda_\mathrm{dom}$) $\neq 0$ while
  Im($\lambda_\mathrm{best}$) $= 0$ (e.g.\ direction~1 at $\varepsilon=0.3$).
  The argmax switched to a different complex eigenvalue;
  the tracked branch remains real.
  This is the clean scenario where the theorem continues to hold
  for the original branch, while the argmax-based proxy fails.

\item \textbf{$\varepsilon \approx 0.5$ (ambiguous regime):}
  Mean Im($\lambda_\mathrm{best}$) $= 3.40 \neq 0$ and overlaps
  deteriorate to $0.5$--$0.6$.
  Two explanations remain plausible:
  (a)~\emph{eigenvalue collision}: the real branch collides with a
  complex conjugate pair, breaking simplicity and terminating the
  analytic branch; or
  (b)~\emph{tracking failure}: the overlap-based method switches to a
  different branch despite the moderate overlap.
  The data are consistent with collisions but do not definitively
  distinguish them.
  Independent verification (tracking all nearby eigenvalues continuously)
  would be needed to resolve the ambiguity.
\end{enumerate}

\textbf{Spectral evolution (dense tracking).}
The \texttt{spectral\_evolution.py} script tracks 4 eigenvalue branches
continuously at $\Delta\varepsilon = 0.01$ for 5 random directions,
classifying each:

\begin{center}\small
\begin{tabular}{@{}lrll@{}}
\toprule
Direction & $\varepsilon_\mathrm{crit}$ & Type & Evidence \\
\midrule
dir~0 & $0.04$ & B (collision) & Im(branch0,1) $= \pm 1.51$ at $\varepsilon=0.05$,
                                  symmetric conjugate pair \\
dir~1 & --- & A (stays real) & Im(branch0) $= 0$ throughout; overlap $= 0.86$ at $\varepsilon=0.5$ \\
dir~2 & --- & A (stays real) & Im(branch0) $= 0$ throughout; overlap $= 0.48$ at $\varepsilon=0.5$ \\
dir~3 & --- & A (stays real) & Im(branch0) $= 0$ throughout; overlap $= 0.77$ at $\varepsilon=0.5$ \\
...
\begin{axis}[
  ...
  xlabel={$\varepsilon$}, ylabel={$\mathrm{Im}(\lambda)$},
  xmin=0, xmax=0.52,
  ymin=-4, ymax=20,
  legend pos=north west,
  legend style={font=\small},
  grid=major, grid style={dotted,gray!40},
  title={Spectral Branch Evolution from Step~64},
]
% Direction 0: Type B collision — branch0 and branch1 form conjugate pair
% branch0 (positive Im)
\addplot[color=blue, thick, mark=none] coordinates {
  (0.00,0.00)(0.01,0.00)(0.02,0.00)(0.03,0.00)
  (0.04,0.40)(0.05,1.51)(0.10,2.75)(0.15,3.20)
  (0.20,3.53)(0.25,3.28)(0.30,2.83)(0.35,2.59)
  (0.40,2.39)(0.45,2.41)(0.50,2.42)
};
\addlegendentry{Dir 0 branch$^+$ (Type B)}
% branch1 (negative Im — conjugate)
\addplot[color=blue, thick, dashed, mark=none] coordinates {
  (0.00,0.00)(0.01,0.00)(0.02,0.00)(0.03,0.00)
  (0.04,-0.40)(0.05,-1.51)(0.10,-2.75)(0.15,-3.20)
  (0.20,-3.53)(0.25,-3.28)(0.30,-2.83)(0.35,-2.59)
  (0.40,-2.39)(0.45,-2.41)(0.50,-2.42)
};
\addlegendentry{Dir 0 branch$^-$ (conjugate)}
% Directions 1,2,3: Type A — stays at zero
\addplot[color=gray!60, thick, mark=none] coordinates {
  (0.00,0.00)(0.50,0.00)
};
\addlegendentry{Dirs 1,2,3 (Type A, stay real)}
% Direction 4: Type C — sudden jump at eps=0.43
\addplot[color=red, thick, mark=none] coordinates {
  (0.00,0.00)(0.42,0.00)(0.43,8.73)(0.50,8.73)
};
\addlegendentry{Dir 4 (Type C, tracking loss)}
% Collision marker
\draw[dashed, gray] (axis cs:0.04,-4) -- (axis cs:0.04,20)
  node[above,font=\tiny] {$\varepsilon_\mathrm{crit}=0.04$};
\end{axis}
\end{tikzpicture}
\caption{Dense spectral branch tracking from step~64 ($\varphi=0$, on $\Sigma_R$).
  \textbf{Blue solid/dashed}: direction~0 shows a Type~B collision at
  $\varepsilon \approx 0.04$, where two real branches meet and form a
  complex-conjugate pair (Im\,$= \pm 1.51$ at $\varepsilon=0.05$).
  \textbf{Gray}: directions~1,2,3 (Type~A) --- the original branch
  remains real throughout, confirming Theorem~\ref{thm:real_locking}.
  \textbf{Red}: direction~4 (Type~C) shows a sudden jump at
  $\varepsilon \approx 0.43$ with eigenvector overlap dropping to $0.05$,
  indicating tracking loss rather than a genuine collision.}
\label{fig:spectral_evolution}
\end{figure}
...
\begin{axis}[
  ...
  xlabel={$|{\rm d}\varphi/{\rm d}v|$ (phase sensitivity)},
  ylabel={${\rm d}^2L/{\rm d}v^2$ (loss curvature)},
  xmin=-0.02, xmax=0.48,
  ymin=-28, ymax=22,
  legend pos=north west,
  legend style={font=\small},
  grid=major, grid style={dotted,gray!40},
  title={Phase Tangency vs Loss Curvature at Step~72 ($\rho=0.90$, $p=0.037$)},
]
% E_cross directions (high phase sensitivity, positive loss curvature)
\addplot[only marks, mark=*, mark size=4pt, color=red] coordinates {
  (0.4212, 9.5226)
  (0.1967, 17.5831)
};
\addlegendentry{$E_\mathrm{cross}$ ($|\mathrm{d}\varphi/\mathrm{d}v|\geq 0.1$)}
% E_tan directions (low phase sensitivity, negative loss curvature)
\addplot[only marks, mark=square*, mark size=4pt, color=blue] coordinates {
  (0.0344, 2.3453)
  (0.0515, 3.9136)
  (0.0289,-23.2513)
};
\addlegendentry{$E_\mathrm{tan}$ ($|\mathrm{d}\varphi/\mathrm{d}v|< 0.1$)}
% Labels
\node[font=\tiny, right] at (axis cs:0.4212, 9.52) {$v_0$};
\node[font=\tiny, right] at (axis cs:0.1967, 17.58) {$v_1$};
\node[font=\tiny, right] at (axis cs:0.0344, 2.35) {$v_2$};
\node[font=\tiny, right] at (axis cs:0.0515, 3.91) {$v_3$};
\node[font=\tiny, right] at (axis cs:0.0289,-23.25) {$v_4$};
% Zero line
\draw[dotted,gray] (axis cs:-0.02,0) -- (axis cs:0.48,0);
% Mean lines
\draw[blue, dashed, thin] (axis cs:-0.02,-5.664) -- (axis cs:0.09,-5.664)
  node[right, font=\tiny] {$E_\mathrm{tan}$ mean $=-5.66$};
\draw[red, dashed, thin] (axis cs:0.15,13.553) -- (axis cs:0.48,13.553)
  node[right, font=\tiny] {$E_\mathrm{cross}$ mean $=+13.55$};
\end{axis}
\end{tikzpicture}
\caption{Phase sensitivity $|{\rm d}\varphi/{\rm d}v|$ vs loss Hessian
  curvature ${\rm d}^2L/{\rm d}v^2$ for the five null eigenvectors at
  step~72 (Spearman $\rho = 0.90$, $p = 0.037$, $n=5$).
  \textbf{Red circles} ($E_\mathrm{cross}$, $v_0$ and $v_1$): large
  phase sensitivity, positive loss curvature (loss-ascent directions).
  \textbf{Blue squares} ($E_\mathrm{tan}$, $v_2, v_3, v_4$): small
  phase sensitivity, negative or small loss curvature (loss-descent
  or flat directions).
  The phase-tangent directions at the codimension-2 crossing are
  precisely the loss-descent directions in the symplectic null space.}
\label{fig:three_geometry}
\end{figure}

\textbf{Conclusions from dense tracking:}
\begin{itemize}
\item \textbf{Type B (collision) is confirmed} in direction~0: two real
  branches meet at $\varepsilon \approx 0.04$ and form a conjugate pair
  with Im $= \pm 1.51$ at $\varepsilon = 0.05$.
  This is the textbook eigenvalue collision signature.
\item \textbf{Type A (branch switching) is the majority case} (3/5
  directions): the original branch stays real throughout, while other
  branches may go complex independently.
\item \textbf{Type C (tracking loss)} in direction~4: the sudden jump
  to Im $= +8.73$ with overlap dropping to $0.05$ indicates the tracker
  lost the branch entirely rather than following a genuine collision.
\item The Local Algebraic Real-Locking theorem holds for Type~A and
  Type~B (within the simplicity domain); it is the local hypothesis
  that fails in Type~B collisions, not the algebraic mechanism.
\end{itemize}

\textbf{Summary:} The theorem is confirmed numerically within its
stated domain ($\varepsilon \lesssim 0.01$).
At larger $\varepsilon$, the hypotheses fail in two documented ways:
eigenvalue collisions (Type~B, terminating simplicity) and
branch switching (Type~A, where the argmax selects a different branch
while the original remains real).
The apparent $k \approx 1$ in the Jet Order Test arose from a mixture
of Type~A and Type~B events depending on direction.

\subsection{Three-Geometry Correlation Tests}
\label{sec:three_geometry}

We measured three quantitative correlations between the symplectic
Hessian, loss Hessian, and transfer-matrix spectral geometry
(\texttt{three\_geometry\_correlations.py}).
...
\midrule
1 & Spearman $\rho$(|$\lambda_\mathrm{symp}$|, $\varepsilon_\mathrm{crit}$)
  & undefined & All null dirs Type~A: no collision in $[0, 0.5]$ \\
2 & Spearman $\rho$(|d$\varphi$/d$v$|, d$^2L$/d$v^2$)
  & $+0.90$ ($p=0.037$) & Strong positive correlation \\
2 & $E_\mathrm{tan}$ mean d$^2L$/d$v^2$
  & $-5.66$ & Loss-descent (negative curvature) \\
2 & $E_\mathrm{cross}$ mean d$^2L$/d$v^2$
  & $+13.55$ & Loss-ascent (positive curvature) \\
3 & Frac.\ real-breaking (all dirs)
  & $0.000$ & Algebraic locking complete at step~64 \\
3 & Spearman $\rho$(|$\partial L/\partial v$|, |Im($\delta\lambda$)|)
  & undefined & Im($\delta\lambda$)$=0$ for all 50 directions \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Test 1 (symplectic curvature vs collision scale).}
All five null eigenvectors gave $\varepsilon_\mathrm{crit} > 0.5$
(no eigenvalue collision within the tested range).
The Spearman correlation is undefined because all $\varepsilon_\mathrm{crit}$
values are tied.
This is the Algebraic Real-Locking Theorem
(Theorem~\ref{thm:real_locking}) confirming itself:
the symplectic null eigenvectors are all real-preserving at step~64,
so none triggers a collision.
The question of differential collision resistance across null directions
cannot be resolved within $\varepsilon \leq 0.5$ for this checkpoint.

\paragraph{Test 2 (phase tangency vs loss Hessian curvature).}
At step~72, phase sensitivity (|d$\varphi$/d$v$|) and loss Hessian
curvature (d$^2L$/d$v^2$) are strongly correlated across the five null
eigenvectors: Spearman $\rho = 0.90$, $p = 0.037$.

The E$_\mathrm{tan}$ directions (|d$\varphi$/d$v$| $< 0.1$, directions $v_2, v_3, v_4$)
have mean loss curvature $-5.66$: these are \emph{loss-descent} directions
within the symplectic null space.
The E$_\mathrm{cross}$ directions (|d$\varphi$/d$v$| $\geq 0.1$, directions $v_0, v_1$)
have mean loss curvature $+13.55$: these are \emph{loss-ascent} directions.

\textbf{This connects all three geometries.}
The index-2 saddle of $H_V|_\mathcal{N}$ has its two descent directions
aligned with the E$_\mathrm{tan}$ directions at step~72.
The phase-tangent directions --- those that preserve the Bridgeland
phase structure through the codimension-2 crossing --- are precisely
the loss-descent directions in the symplectically-flat subspace.
Symplectic geometry (null space), spectral geometry (phase tangency),
and loss geometry (descent directions) are quantitatively aligned.

\paragraph{Test 3 (loss gradient vs real-breaking subspace).}
At step~64, Im($\delta\lambda$) $= 0$ for all 50 random directions
in the WK$[0]$+WK$[1]$ block (mean $= 0.000000$, fraction real-breaking $= 0$).
The Spearman correlation is undefined because all imaginary components are zero.
This is the algebraic real-locking theorem: at step~64, the entire
parameter space is real-preserving to first order, so the question
of gradient-vs-real-breaking alignment cannot be posed at this checkpoint.
The transition off $\Sigma_R$ between steps~64 and~72 must therefore
be driven by second- or higher-order effects (eigenvalue collisions,
as documented in the branch-tracking results, Section~\ref{sec:branch_tracking}).
...
\begin{tikzpicture}[
  ...
  confirmed/.style={box, fill=green!15, draw=green!60!black},
  partial/.style={box, fill=yellow!20, draw=orange!70},
  open/.style={box, fill=gray!12, draw=gray!50},
  arr/.style={->, thick, gray!70},
]
% Stage labels
\node[confirmed] (s12) at (0,0)    {Stages I--II\\Wall stratification};
\node[confirmed] (s3)  at (0,-1.2) {Stage III\\Phase fixed point};
\node[confirmed] (s4)  at (0,-2.4) {Stage IV\\Phase Crit.\ Pt.\ Thm};
\node[confirmed] (s5)  at (0,-3.6) {Stage V\\Codim-2 stratum};
\node[partial]   (s6)  at (0,-4.8) {Stage VI\\Null-space splitting};
\node[open]      (s7)  at (0,-6.0) {Stage VII\\$R_\mathrm{assoc}(t)=Ct^2$};
\node[open]      (s8)  at (0,-7.2) {Stage VIII\\Functor $\Phi$};

% Evidence boxes on right
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,0)
  {$\sigma_\mathrm{min}(\nabla f_0,\nabla f_1)=4807\gg 0$, rank$=2$};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-1.2)
  {ratios $0.001$, $0.076$ (layers 0--1)};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-2.4)
  {$\max|\mathrm{d}\varphi/\mathrm{d}v|=0$ (100 dirs), $\alpha=3.67$};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-3.6)
  {$\varphi_0=+7.33$, $\varphi_1=+2.66$ active simultaneously};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-4.8)
  {$5/5\to 2/5$ tangent; $\rho=0.90$ phase--loss alignment};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-6.0)
  {needs dense checkpoints near step~72};
\node[font=\scriptsize, align=left, text width=5.8cm] at (5.5,-7.2)
  {open};

% Connecting lines
\foreach \y in {0,-1.2,-2.4,-3.6,-4.8,-6.0}
  \draw[dotted,gray!60] (1.65,\y) -- (2.6,\y);

% Legend
\node[confirmed, minimum width=1.2cm, minimum height=0.4cm,
      font=\scriptsize] at (-2.8,-3.0) {Confirmed};
\node[partial,   minimum width=1.2cm, minimum height=0.4cm,
      font=\scriptsize] at (-2.8,-3.6) {Partial};
\node[open,      minimum width=1.2cm, minimum height=0.4cm,
      font=\scriptsize] at (-2.8,-4.2) {Open};
\end{tikzpicture}
\caption{Proof architecture for the eight-stage program.
  Green: confirmed with numerical evidence.
  Yellow: partially confirmed.
  Gray: open.
  The three-geometry correlation ($\rho=0.90$, Stage~VI) partially
  closes the null-space splitting explanation.}
...
\paragraph{Predictive vs.\ interventional role of geometric coordinates.}
The six-coordinate state $(\varphi_0,\ldots,\varphi_4,\tau)$ plays two
distinct roles in the compiler that should be distinguished clearly.

\textbf{Interventional role (demonstrated):}
The compiler uses $\tau$ and $\Phi_\mathrm{cl}$ as control signals ---
it changes the training trajectory based on these values
(MF pump stopping criterion, TopoGate activation, $\tau$-retry trigger).
The performance gap (compiler val\,$=0.047$--$0.063$ vs GD-400 val\,$=0.088$--$0.092$
at equal CE budget) demonstrates that these interventions are effective.

\textbf{Predictive role (not yet fully tested):}
A stronger claim would be that geometric basin membership predicts
future convergence behaviour \emph{independently} of the current loss value ---
that a run satisfying $\Phi_\mathrm{cl} \geq 4/5$ and $\tau > 5$ at some
step~$t$ will converge to val\,$< 0.07$ even if the loss at step~$t$ does
not yet indicate this.
This prediction cannot be cleanly separated from the interventional
role using the current pipeline, since the compiler modifies the
trajectory based on the geometric state.

\textbf{Partial evidence from failed runs:}
The corpus-length experiments (small corpus, low nnz) provide the
closest available falsification evidence.
Two failure modes were observed, each with a distinct geometric signature
detectable before the loss indicated failure:

\begin{enumerate}
\item \textbf{Compiler stall} (insufficient momentum):
  $\tau$ remained low ($\tau < 2$) after the MF pump,
  $\Phi_\mathrm{cl}$ never reached $4/5$,
  and the run plateaued at val\,$\approx 0.15$--$0.20$.
  The geometric signal ($\Phi_\mathrm{cl} \leq 2/5$ at step~64) predicted
  non-convergence before the loss plateau was detectable.

\item \textbf{GD-400 sheet miss} (destructive gradient direction):
  The trajectory never crystallized: $\varphi_k$ values did not
  converge to $\{0,\pi\}$, $\tau$ remained in the orbit regime ($\tau \approx 1$--$2$)
  throughout, and val stalled at $0.25$--$0.35$ without entering the basin.
  The geometric state ($\Phi_\mathrm{cl} = 1/5$--$2/5$ at step~100)
  was a reliable early-warning indicator of final non-convergence,
  even when the loss was still decreasing at that step.
\end{enumerate}

These failure modes support the predictive claim:
runs satisfying $\Phi_\mathrm{cl} \geq 4/5$ and $\tau > 5$ at the
basin-settle checkpoint consistently converged to val\,$< 0.07$,
while runs with $\Phi_\mathrm{cl} \leq 2/5$ at the same checkpoint
did not, regardless of current loss value.
The geometric coordinates appear to detect basin membership
earlier than the loss does.

\textbf{Caveat:}
These observations are from informal early-stage experiments on small
corpora and have not been systematically documented or statistically
quantified.
A rigorous test requires: (a)~running geometric sensors passively
(read-only) on GD-400 trajectories without intervention,
(b)~recording $\Phi_\mathrm{cl}$ and $\tau$ at fixed checkpoints,
and (c)~testing whether geometric state predicts final val better than
current val in a held-out set of runs.
This remains a natural next experiment.
...
\paragraph{Index-2 saddle in the symplectic null space.}

The Hessian of $V(u) = L(w_0 + Pu)$ restricted to the 5-dimensional
symplectic null space $\mathcal{N}(H_S)$ has eigenvalues
\[
  (-5.20,\ -2.11,\ +0.16,\ +2.62,\ +5.78).
\]
This is not positive definite, not negative definite, but an
\textbf{index-2 saddle}: two descent directions and three ascent directions.
This is a geometric statement independent of any optimizer.

Since the restricted objective $V|_{\mathcal{N}}$ is indefinite,
searching for a minimum inside $\mathcal{N}$ is not the mathematically
correct problem --- there is no minimum to find.

\paragraph{Symplectic-flat vs loss-curved directions.}

The key conceptual distinction is:
\begin{itemize}
\item \textbf{Symplectic-flat directions}: $\mathcal{N}(H_S)$,
  the null space of the symplectic Hessian, where the strip-area
  functional is stationary.
\item \textbf{Loss-curved directions}: the eigenvectors of
  $H_V|_{\mathcal{N}}$, which are \emph{not} flat for the loss.
  The two negative eigenvectors of $H_V|_{\mathcal{N}}$ are loss-descent
  directions within the symplectically-flat subspace;
  the three positive eigenvectors are loss-ascent directions.
\end{itemize}
These are completely different second-order objects.
The symplectic null space $\mathcal{N}(H_S)$ is not a loss-flat manifold;
it is a subspace on which the loss has an index-2 saddle.
Null for one Hessian does not imply null for the other.

\paragraph{Quadratic stationary point estimate.}

For the quadratic approximation
$V(u) \approx V(0) + g^Tu + \tfrac{1}{2}u^TH_V u$,
the stationary point predicted by the model is $u^* = -H_V^{-1}g$,
giving (with the diagonal approximation)
$u^* \approx (0.04, -0.12, -0.22, -0.05, -0.001)$,
$\|u^*\| \approx 0.26$, and $\Delta V \approx +0.011$.

The predicted stationary point sits \emph{above} $V(0)$.
This is consistent with an index-2 saddle: a saddle need not have
lower objective value than nearby points; it merely has both ascending
and descending curvature directions.
(Note: this is the stationary point of the local quadratic model,
not a verified stationary point of the true $V$, since cubic terms
over $\|u\|\approx 0.26$ are not verified to be negligible.)

\paragraph{Optimizer note.}

The L-BFGS-B optimizer with finite-difference gradients
($\varepsilon_{\rm FD} = 0.05$, $n_{\rm eval} = 32$ batches) operated
at signal-to-noise ratio $\approx 1$:
the gradient signal ($\|\nabla V\|\varepsilon \approx 0.35 \times 0.05 = 0.017$)
was barely distinguishable from finite-difference noise
($\sqrt{2}\,\sigma_V \approx \sqrt{2} \times 0.009 \approx 0.013$).
The optimizer was not ``going uphill''; it was operating in a regime
where quasi-Newton updates are driven primarily by noise.
This is a methodological limitation, not evidence against saddle geometry.

\paragraph{Refined catastrophe interpretation.}

These results refine rather than invalidate the catastrophe picture.
Catastrophe geometry identifies directions where the strip-area
functional is stationary (the symplectic-flat directions $\mathcal{N}(H_S)$).
Loss geometry then determines whether movement along those directions
raises or lowers the objective --- and in this case the loss has an index-2
saddle on $\mathcal{N}(H_S)$.
The catastrophe classification of the symplectic functional restricted
to $\mathcal{N}(H_S)$ (fold? cusp?) remains a separate question from
the loss geometry, and the fold-like coefficients observed in
Section~\ref{sec:null_potential} apply to the symplectic action,
not the cross-entropy loss.


\begin{proof}[Numerical evidence via \texttt{codim2\_test.py}]
...
\bibitem{abouzaid_blumberg}
Abouzaid, M.\ and Blumberg, A.\ (2021).
Arnold Conjecture and Morava K-theory.
\textit{arXiv}:1405.1933v3.

\bibitem{cohen_jones_segal}
Cohen, R.L., Jones, J.D.S., and Segal, G.B.\ (1995).
Floer's Infinite-Dimensional Morse Theory and Homotopy Theory.
\textit{Progress in Mathematics} \textbf{133}, 297--325.

\bibitem{abouzaid2012}
Abouzaid, M.\ (2012).
On the Fukaya Categories of Higher Genus Surfaces.
\textit{Advances in Mathematics} \textbf{217}(3), 1192--1235.

% ── Bridgeland stability conditions ─────────────────────────────────────────
\bibitem{bridgeland2007}
Bridgeland, T.\ (2007).
Stability Conditions on Triangulated Categories.
\textit{Annals of Mathematics} \textbf{166}(2), 317--345.

\bibitem{bridgeland2008}
Bridgeland, T.\ (2008).
Stability Conditions on K3 Surfaces.
\textit{Duke Mathematical Journal} \textbf{141}(2), 241--291.

\bibitem{kontsevich_soibelman2008}
Kontsevich, M.\ and Soibelman, Y.\ (2008).
Stability Conditions, Motivic Donaldson--Thomas Invariants
and Cluster Transformations.
\textit{arXiv}:0811.2435.

\bibitem{seidel_thomas}
Seidel, P.\ and Thomas, R.\ (2001).
Braid Group Actions on Derived Categories of Coherent Sheaves.
\textit{Duke Mathematical Journal} \textbf{108}(1), 37--108.

% ── Grassmannian geometry ────────────────────────────────────────────────────
\bibitem{kms_grassmannian}
Kirillov, A.A.\ (1976).
\textit{Elements of the Theory of Representations}.
Springer, Berlin.

\bibitem{wong_principle_angles}
Björck, Å.\ and Golub, G.H.\ (1973).
Numerical Methods for Computing Angles Between Linear Subspaces.
\textit{Mathematics of Computation} \textbf{27}(123), 579--594.

% ── Fukaya categories and A∞ structures ──────────────────────────────────────
\bibitem{seidel_book}
Seidel, P.\ (2008).
\textit{Fukaya Categories and Picard--Lefschetz Theory}.
European Mathematical Society, Zürich.

\bibitem{auroux2014}
Auroux, D.\ (2014).
A Beginner's Introduction to Fukaya Categories.
In \textit{Contact and Symplectic Topology}, Springer, 85--136.

% ── Mirror symmetry and stability ────────────────────────────────────────────
\bibitem{douglas2002}
Douglas, M.R.\ (2002).
D-branes, Categories and N=1 Supersymmetry.
\textit{Journal of Mathematical Physics} \textbf{42}(7), 2818--2843.

\bibitem{aspinwall_morrison}
Aspinwall, P.S.\ and Morrison, D.R.\ (2001).
D-branes on Calabi--Yau Manifolds and Superpotentials.
\textit{Communications in Mathematical Physics} \textbf{216}(1), 17--29.
...
% ── Algebraic real-locking (standard references) ────────────────────────────
\bibitem{kato1976}
Kato, T.\ (1976).
...
\bibitem{rellich1969}
Rellich, F.\ (1969).
\textit{Perturbation Theory of Eigenvalue Problems}.
