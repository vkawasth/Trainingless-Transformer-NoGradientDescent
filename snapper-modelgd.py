#!/usr/bin/env python3
"""
Geodesic Submanifold Structure Analysis
========================================
1. Run the FULL compiler chain from snappers.py to reach val=0.0757
2. THEN instrument the GD-400 fallback (LR=0.003) EXACTLY as it runs
3. Track: gradients, Hessian, active layers, orbit alignment, geodesic bending

CE tracking: each phase reports CE used.
"""
...
# ============================================================================
# MODEL DEFINITION (from snappers.py)
# ============================================================================
...
# ============================================================================
# UTILITY FUNCTIONS (from snappers.py)
# ============================================================================
...
# ============================================================================
# ACTION FUNCTIONS (from snappers.py)
# ============================================================================
...
# ============================================================================
# COMPILER STATE (from snappers.py)
# ============================================================================
class CompilerState:
...
# ============================================================================
# SMART ACTION SELECTION (from snappers.py)
# ============================================================================
...
# ============================================================================
# INSTRUMENTED GD-400 (EXACTLY AS IN snappers.py FALLBACK)
# ============================================================================
def instrumented_run_gradient_descent(model, steps=400, lr=0.003):
    """
    EXACTLY the same as run_gradient_descent in snappers.py, but with instrumentation.
    """
    print(f"\n  → INSTRUMENTED GD-400 (LR={lr})")
    print(f"    Tracking: gradients, Hessian, active layers, orbit alignment")
    ...
    # Initial Hessian and gradient for analysis
    print("    Computing initial Hessian direction...")
    hessian_dir0 = hessian_smallest_eigenvector(model)
    ...
    layer_contrib0, _ = layer_contributions(model)
    active0 = get_active_layers(layer_contrib0)
    
    analysis_data['steps'].append(0)
    analysis_data['vals'].append(v0)
    analysis_data['taus'].append(tau0)
    analysis_data['phis'].append(phi0)
    analysis_data['cos_grad_hessian'].append(float((grad_dir0 * hessian_dir0).sum()))
    analysis_data['grad_norms'].append(grad0.norm().item())
    analysis_data['dtheta_norms'].append(0.0)
    analysis_data['cos_dtheta_hessian'].append(0.0)
    analysis_data['active_layers'].append(active0)
    analysis_data['layer_contributions'].append(layer_contrib0)
    
    print(f"      cos(grad, Hessian) = {analysis_data['cos_grad_hessian'][0]:.4f}")
    print(f"      Active layers: {len(active0)}")
    
    # Run GD (exactly as in snappers.py)
    print("\n    Running GD-400...")
    prev_w = w0.clone()
    ...
    for step in range(1, steps + 1):
        ...
        if step % 50 == 0:
            ...
            hessian_dir = hessian_smallest_eigenvector(model)
            ...
            layer_contrib, _ = layer_contributions(model)
            ...
            analysis_data['steps'].append(step)
            analysis_data['vals'].append(v)
            analysis_data['taus'].append(tau)
            analysis_data['phis'].append(phi)
            analysis_data['cos_grad_hessian'].append(cos_grad_hess)
            analysis_data['cos_dtheta_hessian'].append(cos_dtheta_hess)
            analysis_data['grad_norms'].append(grad.norm().item())
            analysis_data['dtheta_norms'].append(dtheta.norm().item())
            analysis_data['active_layers'].append(active)
            analysis_data['layer_contributions'].append(layer_contrib)
    ...
    return model, v_final, phi_final, tau_final, history, analysis_data


# ============================================================================
# SMART DP COMPILER (from snappers.py, with instrumented GD)
# ============================================================================
class SmartDPCompiler:
    ...
    def run(self):
        print("=" * 70)
        print("SMART DP COMPILER WITH INSTRUMENTED GD-400")
        print("=" * 70)
        print()
        ...
        for step_counter in range(MAX_DP_STEPS):
            ...
            if not untried:
                ...
                if back_state is None:
                    ...
                    return self.best_state, None, None
...
# GEODESIC SUMMARY
# ============================================================================
def print_geodesic_summary(analysis_data):
    """Print a summary of the geodesic path analysis."""
    print("\n" + "=" * 70)
    print("GEODESIC PATH SUMMARY")
    print("=" * 70)
    
    if not analysis_data or len(analysis_data['steps']) == 0:
        print("  No analysis data collected.")
        return
    
    steps = analysis_data['steps']
    vals = analysis_data['vals']
    cos_grad_hess = analysis_data['cos_grad_hessian']
    cos_dtheta_hess = analysis_data['cos_dtheta_hessian']
    phis = analysis_data['phis']
    taus = analysis_data['taus']
    
    print("\n  Step |   val   |  τ   | φ  | cos(g,H) | cos(dθ,H)")
    print("  -----|---------|------|----|----------|----------")
    for i, step in enumerate(steps):
        cos_dtheta = cos_dtheta_hess[i] if i < len(cos_dtheta_hess) else 0.0
        phi_val = phis[i] if i < len(phis) else 0
        tau_val = taus[i] if i < len(taus) else 0.0
        print(f"  {step:5d} | {vals[i]:.4f} | {tau_val:.2f} | {phi_val:d}  | {cos_grad_hess[i]:+.4f}   | {cos_dtheta:+.4f}")
    
    # Trends
    if len(cos_grad_hess) >= 2:
        start_cos = cos_grad_hess[0]
        end_cos = cos_grad_hess[-1]
    ...
    for i, step in enumerate(steps):
        active = analysis_data['active_layers'][i] if i < len(analysis_data['active_layers']) else {}
        if active:
...
def main():
    ...
    print("=" * 70)
    print("FULL COMPILER CHAIN + INSTRUMENTED GD-400 (from snappers.py)")
    print("=" * 70)
    print(f"\n  CE tracking enabled. Initial CE: {CE.ce}")
    
    # Run the compiler (this replicates the snappers.py flow)
    compiler = SmartDPCompiler()
    final_state, _, _ = compiler.run()
    
    # Check if we reached the basin and need GD fallback
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        print("\n" + "=" * 70)
        print("RUNNING INSTRUMENTED GD FALLBACK")
        print("=" * 70)
        
        model = final_state.model
        v_start = final_state.val
        phi_start = final_state.phi
        tau_start = final_state.tau
        
        print(f"\n  Starting instrumented GD-400 from val={v_start:.4f}")
        print(f"  phi={phi_start}/5, tau={tau_start:.2f}")
        print(f"  CE before GD: {CE.ce}")
        
        # Run instrumented GD
        model, v_final, phi_final, tau_final, history, analysis_data = instrumented_run_gradient_descent(
            model, steps=400, lr=0.003
        )
        ...
        print(f"  Path:          {' → '.join(final_state.action_history)} → instrumented_gradient_descent(400)")
        
        # Geodesic summary
        print_geodesic_summary(analysis_data)
        ...
        }, 'geodesic_analysis_snappers.pt')
        print("\n  Saved: geodesic_analysis_snappers.pt")
        
    else:
        print("\n  ✓ Already at floor! val={:.4f}".format(final_state.val))
        torch.save(final_state.model.state_dict(), 'compiler_success.pt')
        print("\n  Saved: compiler_success.pt")
