...
# =====================================================================
# 2. ADAPTIVE KRYLOV-NEWTON OPTIMIZER (Configurable Clamping Floor)
# =====================================================================
class AdaptiveKrylovSubspaceSignAdam:
    def __init__(
        self, 
        params, 
        lr_kn=0.35,               # Fixed at lr_kn = 0.35
        lr_ortho_max=2e-3, 
        lr_ortho_min=8e-4, 
        beta_ema=0.75, 
        beta1_ortho=0.95, 
        beta2_ortho=0.999, 
        eps=1e-8, 
        k_dim=4,                  # Fixed at k = 4
        soft_floor_ratio=0.50, 
        eig_clamp_min=1e-4        # Parameterized threshold
    ):
    ...
    def step(self, Q_K, T_K=None, total_steps=120):
        ...
        # 2. Subspace Newton Step with Hard Eigenvalue Clamping Sweep
        if T_K is not None:
            rhs = Q_K.T @ g_seeded
...
# =====================================================================
# 4. EIGENVALUE CLAMP SWEEP EXECUTION LOOP
# =====================================================================
def run_trial(eig_clamp_min, total_steps=120):
    torch.manual_seed(0)
...
def main():
    clamp_thresholds = [1e-3, 5e-4, 1e-4]
    results = {}

    print("=" * 105)
    print(f" EIGENVALUE CLAMP THRESHOLD SWEEP (k=4, lr_kn=0.35) | lr_ortho: 2e-3 -> 8e-4")
    print("=" * 105)
    print(f"{'min_eig':>10} | {'Step 120 Loss':>15} | {'Rho_F (ρ)':>12} | {'Gamma (γ)':>12} | {'Torsion τ_L':>15}")
    print("-" * 105)

    for thresh in clamp_thresholds:
        diag, final_loss = run_trial(thresh)
        results[thresh] = (final_loss, diag)
        print(f"{thresh:10.1e} | {final_loss:15.4f} | {diag['rho_F']:12.4f} | {diag['gamma']:12.4f} | {diag['tau_L']:15.4e}")
