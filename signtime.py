...
DONOR_FOR_G=120
DONORS=[40,60,80,100,120]; EVAL=[0,1,2,5,10,20,30]; LAYER=3
        ...
        with torch.no_grad():
            for n,p in snamed:
                for bi in range(NB):
                    ...
                    if crit in ("lowrank","resid","sign","signN","gsign","rowscale","colscale","spectrum"):
                        ...
                        if crit=="sign":
                            ...
                            p.data.add_(torch.sign(gg)*dW.abs().mean())
                        elif crit=="rowscale":
