...
class LandscapeAnalyzer:
    ...
    def _to_float(self, x):
        """Convert tensor to float, handling both torch.Tensor and other types."""
        if x is None:
            return 0.0
    ...
    def analyze(self, model):
        ...
        for _ in range(10):
            Hv = hvp_s(v_tensor)
            v_tensor = Hv / max(float(Hv.norm()), 1e-10)
...
# ============================================================================
# ============================================================================
# SMART ACTION SELECTION (FIXED - CONVERT TENSORS TO NATIVE TYPES)
# ============================================================================
def get_smart_actions(state, analyzer):
    ...
    # This is the fix - ensure everything is a Python float/int/bool
    val = float(landscape['val']) if hasattr(landscape['val'], 'item') or isinstance(landscape['val'], torch.Tensor) else float(landscape['val'])
    phi = int(landscape['phi']) if hasattr(landscape['phi'], 'item') or isinstance(landscape['phi'], torch.Tensor) else int(landscape['phi'])
    tau = float(landscape['tau']) if hasattr(landscape['tau'], 'item') or isinstance(landscape['tau'], torch.Tensor) else float(landscape['tau'])
    lambda_max = float(landscape['lambda_max']) if hasattr(landscape['lambda_max'], 'item') or isinstance(landscape['lambda_max'], torch.Tensor) else float(landscape['lambda_max'])
    lambda_min = float(landscape['lambda_min']) if hasattr(landscape['lambda_min'], 'item') or isinstance(landscape['lambda_min'], torch.Tensor) else float(landscape['lambda_min'])
    g_norm = float(landscape['g_norm']) if hasattr(landscape['g_norm'], 'item') or isinstance(landscape['g_norm'], torch.Tensor) else float(landscape['g_norm'])
    is_positive_definite = bool(landscape['is_positive_definite'])
    has_negative_curvature = bool(landscape['has_negative_curvature'])
    is_flat = bool(landscape['is_flat'])
    in_orbit = bool(landscape['in_orbit'])
    near_floor = bool(landscape['near_floor'])
    ...
    # 3. Basin settle: works almost everywhere, choose LR from val
    if not near_floor:
        if val > 1.0:
            lr_mult = 10
        elif val > 0.3:
        ...
        print(f"    → basin_settle: LR×{lr_mult} (val-driven)")
    else:
        print(f"    ℹ skipping basin_settle: near floor")
    ...
    if near_floor and is_positive_definite:
    ...
    # 8. If no actions, try basin settle as fallback
    if not actions:
...
class CompilerState:
    ...
    def __repr__(self):
        ...
        return (f"State(step={self.step}, val={self.val:.4f}, "
                f"phi={self.phi}/5, tau={self.tau:.2f} {orbit_ok})")
...
class SmartDPCompiler:
    ...
    def run(self):
        ...
        for step in range(MAX_DP_STEPS):
            ...
            if not untried:
                print(f"\n[{step}] No untried actions at {current}")
                # Backtrack
                idx, back_state, back_untried = self.find_backtrack_state(state_stack)
                if back_state is None:
                    print(f"⚠ No states with untried actions. Best val={self.best_state.val:.4f}")
            ...
            if not action_taken:
                print(f"  ⊘ No working action at {current}")
