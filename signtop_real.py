...
def compare_operators(u, grad, rank_budget=32):
    """Apply all four compressors to the SAME update and report the diagnostics.

    This reproduces the ablation quoted in the docstring. The four operators
    isolate sign+top's two ingredients:

        lowrank    two-sided rank-r projection of u, projector from grad's SVD
        signtop    top-k exact + one-bit sign field for the tail
        toponly    top-k exact, tail ZEROED -- what the sign field is worth
        signonly   the whole matrix as sign x mean -- what top-k is worth

    Returns {name: {cos, relerr, normratio, D}} where D is retained descent
    -g.uhat / -g.u. Note cos and normratio coincide for every operator here:
    all four satisfy <u, uhat> = |uhat|^2, so both reduce to |uhat|/|u|.
    """
    def _dot(a, b):
        return sum(float((a[n] * b[n]).sum()) for n in a)
    def _nrm(a):
        return math.sqrt(sum(float((a[n] * a[n]).sum()) for n in a))

    def _toponly(uu):
        o = {}
        for n, t in uu.items():
            k = signtop_budget(tuple(t.shape), rank_budget)
            fl = t.reshape(-1); z = torch.zeros_like(fl)
            if k > 0:
    ...
    def _signonly(uu):
        return {n: (torch.sign(t.reshape(-1))
                    * float(t.abs().mean())).view_as(t) for n, t in uu.items()}

    ops = {"lowrank": lambda z: lowrank_compress(z, grad, rank_budget),
           "signtop": lambda z: signtop_compress(z, rank_budget=rank_budget),
           "toponly": _toponly,
           "signonly": _signonly}
    Df = -_dot(grad, u); un = _nrm(u)
    out = {}
    for name, fn in ops.items():
        uh = fn(u)
        out[name] = dict(
            cos=_dot(u, uh) / max(un * _nrm(uh), 1e-30),
            relerr=_nrm({n: uh[n] - u[n] for n in u}) / max(un, 1e-30),
            normratio=_nrm(uh) / max(un, 1e-30),
            D=-_dot(grad, uh) / Df if abs(Df) > 1e-12 else float("nan"))
    return out


def _ablation():
    """Run compare_operators on a throwaway net, so the file reproduces its own
    ablation. The numbers in the docstring come from the real pipeline
    (D=128 transformer, 140 steps); this demo only checks the ORDERING, which
    is what the argument rests on."""
    torch.manual_seed(0)
    ...
    for _ in range(30):                      # let Adam's moments warm up
        loss = ((net(x) - y) ** 2).mean()
        opt.zero_grad(); loss.backward(); opt.step()
    before = {n: p.data.clone() for n, p in net.named_parameters() if p.dim() == 2}
    loss = ((net(x) - y) ** 2).mean()
    opt.zero_grad(); loss.backward()
    grad = {n: p.grad.detach().clone() for n, p in net.named_parameters()
            if n in before}
    opt.step()
    u = {n: (p.data - before[n]) for n, p in net.named_parameters() if n in before}
    with torch.no_grad():
        for n, p in net.named_parameters():
            if n in before: p.data.copy_(before[n])
    res = compare_operators(u, grad, rank_budget=32)
    print("\n  ablation at the r=32 budget, all four applied to the SAME step\n")
    print(f"  {'op':>10}{'cos':>8}{'relerr':>9}{'|uh|/|u|':>10}{'D retained':>12}")
    for k in ("lowrank", "signtop", "toponly", "signonly"):
        r = res[k]
        print(f"  {k:>10}{r['cos']:>8.3f}{r['relerr']:>9.3f}"
              f"{r['normratio']:>10.3f}{r['D']:>12.3f}")
    print("\n  on the real pipeline this reads:")
    print("     lowrank  0.568  0.821  0.568  0.858")
    print("     signtop  0.949  0.314  0.949  0.892")
    print("     toponly  0.798  0.601  0.798  0.632")
    print("     signonly 0.820  0.571  0.820  0.632")
    print("  Both ingredients are needed: top-k alone and sign alone each retain")
    print("  0.632, together 0.892. And low rank retains nearly the same LOCAL")
    print("  descent as sign+top (0.858 vs 0.892) while its direction is far")
    print("  worse (0.568 vs 0.949) -- that is what compounds over a trajectory.")
