"""
RIEMANNIAN SUBSPACE ADAM-LESS TRACKER (smallgps_riemannian_adam.py)

Tracks first and second moments strictly inside the K-dimensional subspace coordinate
frame alpha_t in R^K. Eliminates the non-commutativity frame-mismatch error and restores
full optimization performance without invoking torch.optim.
"""
...
# Riemannian Subspace Hyperparameters
LR = 3e-3
BETA1 = 0.9
BETA2 = 0.95
EPS = 1e-8
...
def run_riemannian_subspace_adam(theta=0.5, seed=0):
    ...
    # --- FULL-DIMENSIONAL ADAM MOMENTS (For initial full-rank steps) ---
    P_total = sum(p.numel() for p in ps)
    m_full = torch.zeros(P_total)
    v_full = torch.zeros(P_total)

    # --- K-DIMENSIONAL SUBSPACE ADAM MOMENTS ---
    m_sub = {r: torch.zeros(K) for r in idx}
    v_sub = {r: torch.zeros(K) for r in idx}
    ...
    print("=" * 80)
    print("RUNNING RIEMANNIAN SUBSPACE ADAM (Subspace Coordinate Preconditioning)")
    print("=" * 80)
    ...
    for st in range(STEPS):
        ...
        # Compute Raw Gradient with Weight Decay
        th = flat()
        x, y = batch()
        m.zero_grad()
        _, l = m(x, y)
        ...
        if full and used < CAP:
            ...
            # Re-estimate Subspace Basis Q_r via SVD on recent updates
            ...
            # --- RIEMANNIAN SUBSPACE ADAM STEP (In-Frame K-Dim Preconditioning) ---
            ...
            for r, ii in idx.items():
                ...
                # 1. Project Raw Gradient into Subspace Coordinates: a_t in R^K
                a_t = Q_r.T @ g_r

                # 2. Update K-Dimensional Subspace Moments
                m_sub[r] = BETA1 * m_sub[r] + (1.0 - BETA1) * a_t
                v_sub[r] = BETA2 * v_sub[r] + (1.0 - BETA2) * (a_t * a_t)

                bc1 = 1.0 - (BETA1 ** t_step)
                bc2 = 1.0 - (BETA2 ** t_step)

                m_sub_hat = m_sub[r] / bc1
                v_sub_hat = v_sub[r] / bc2

                # 3. Compute Preconditioned Step in Subspace Coordinates: delta_alpha in R^K
                d_alpha = -LR * (m_sub_hat / (torch.sqrt(v_sub_hat) + EPS))

                # 4. Map Back to Parameter Space: Q_r * delta_alpha in R^P
                nu[ii] = Q_r @ d_alpha
...
# --- Execution ---
run_riemannian_subspace_adam(theta=0.5, seed=0)
