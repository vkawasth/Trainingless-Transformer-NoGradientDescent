"""
RESIDUAL INJECTION SUBSPACE ADAM (smallgps_residual_injection_adam.py)

Tracks global momentum (m_t) and variance (v_t) in parameter space R^P.
During constrained steps, it projects the Adam update vector u_t onto the active 
subspace Q_r while injecting a small fraction (alpha = 0.1) of the orthogonal 
residual to preserve high-frequency loss surface curvature and escape plateaus.
"""
...
# Residual Injection Coefficient (Leakage into orthogonal complement)
ALPHA_RESIDUAL = 0.10
...
def run_residual_injection_subspace_adam(theta=0.5, seed=0):
    ...
    print("=" * 80)
    print(f"RUNNING RESIDUAL INJECTION SUBSPACE ADAM (alpha = {ALPHA_RESIDUAL})")
    print("=" * 80)
    ...
    for st in range(STEPS):
        ...
        # 1. Global Adam State Update in R^P
        ...
        if full and used < CAP:
            ...
            # 2. Subspace Projection + Residual Injection Step
            ...
            for r, ii in idx.items():
                ...
                # Parallel projection onto subspace
                u_proj = Q_r @ (Q_r.T @ u_r)

                # Orthogonal complement residual vector
                u_orth = u_r - u_proj

                # Injected update: Subspace Projection + alpha * Orthogonal Residual
                nu[ii] = u_proj + ALPHA_RESIDUAL * u_orth
...
if __name__ == "__main__":
    run_residual_injection_subspace_adam(theta=0.5, seed=0)
