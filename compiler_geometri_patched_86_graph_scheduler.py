...
class GraphCulledAdamW:
    """AdamW that migrates W_V/W_O to scalar SGD once graph drift settles.
    Drop-in for torch.optim.AdamW in Phase 3: supports .param_groups (proxying
    the core AdamW, so lr writes work), .zero_grad(), .step(), plus
    .maybe_cull() to be called each step after backward()."""
    def __init__(self, model, lr, cull_from_start=False, vo_scale=None,
                 thr=0.03, warm=20, hold=15, wd=0.1, b1=0.9, verbose=True):
        self.model = model; self.b1 = b1; self.wd = wd; self.verbose = verbose
        named = list(model.named_parameters())
        self.core = [p for nm, p in named if _g3_group(nm) not in ("W_V", "W_O")]
        self.vo   = [p for nm, p in named if _g3_group(nm) in ("W_V", "W_O")]
        ...
        self.culled = False; self.vo_scale = vo_scale
        if cull_from_start and self.vo:
    ...
    def _vo_median_D(self):
        ...
        st = self.vo_adam.state; b2 = 0.95; eps = 1e-8; Ds = []
        for p in self.vo:
            if p in st and "exp_avg_sq" in st[p]:
                ...
                Ds.append((1.0 / (vhat.sqrt() + eps)).flatten())
        if not Ds: return 1.0
        med = float(torch.cat(Ds).median())
        return min(max(med, 0.05), 40.0)          # clamp: no scalar-SGD blow-up
    ...
    def _do_cull(self, from_start=False):
        if self.vo_scale is None:
            self.vo_scale = self._vo_median_D()
        self.culled = True
        if self.verbose:
            n_vo = sum(p.numel() for p in self.vo)
            n_all = n_vo + sum(p.numel() for p in self.core)
            src = "post-geo-stop" if from_start else "drift settled"
            print(f"  \u27d0 V/O\u2192scalar-SGD ({src}): "
                  f"{100*n_vo/max(n_all,1):.0f}% of params off AdamW 2nd-moment "
                  f"(slr = lr\u00d7{self.vo_scale:.1f})")

    def zero_grad(self):
        self.adam.zero_grad()
        if not self.culled and self.vo_adam is not None:
            self.vo_adam.zero_grad()
        else:
            for p in self.vo:
                if p.grad is not None: p.grad = None

    def maybe_cull(self):
        if self.culled or not self.vo: return
    ...
    def step(self):
        ...
        self.adam.step()
        if not self.culled:
            if self.vo_adam is not None:
                for pg in self.vo_adam.param_groups: pg["lr"] = lr
                self.vo_adam.step()
        else:
            self.vo_t += 1
...
print(f"  V/O cull: {'ON' if getattr(opt_b, 'culled', False) else 'not triggered'}"
      f"  (live-AdamW 2nd-moment footprint "
      f"{100*(1-_p3_vo/_p3_all):.0f}% if culled, else 100%)")
