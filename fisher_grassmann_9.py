...
# =====================================================================
# 4. KRYLOV RANK (k) SWEEP EXECUTION LOOP
# =====================================================================
def run_trial(k_dim, total_steps=120):
    torch.manual_seed(0)
    ...
    update_window = []
    WINDOW_SIZE = max(8, k_dim)  # Window size must be at least k_dim for SVD rank
...
def main():
    ...
    print("=" * 105)
    print(f" KRYLOV SUBSPACE RANK SWEEP (lr_kn=0.35, min_eig=1e-4) | lr_ortho: 2e-3 -> 8e-4")
    print("=" * 105)
    print(f"{'Krylov k':>8} | {'Step 120 Loss':>15} | {'Rho_F (ρ)':>12} | {'Gamma (γ)':>12} | {'Torsion τ_L':>15}")
    print("-" * 105)

    for k in k_ranks:
        diag, final_loss = run_trial(k)
        results[k] = (final_loss, diag)
        print(f"{k:8d} | {final_loss:15.4f} | {diag['rho_F']:12.4f} | {diag['gamma']:12.4f} | {diag['tau_L']:15.4e}")
