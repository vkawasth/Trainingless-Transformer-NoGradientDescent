...
for step in range(1, 101):
    ...
    if step % 8 == 0:
        ...
        if delta < 0.003:  # tighter plateau — need deeper settle before τ-retry
            print(f"  ✓ Plateau (rate={delta:.4f} < 0.003): geometry check")
            break
        if v < 0.15:       # good starting point for τ-retry regardless of rate
            print(f"  ✓ val={v:.4f} < 0.15 — stopping settle early")
            break
...
# Save basin entry state NOW (val≈0.20) before any τ correction
torch.save(model.state_dict(),'basin_entry_state.pt')
print(f"  Saved basin_entry_state.pt (val={v_basin:.4f})")

if tau_b > 5:
    # τ>5: 50CE@BATCH=8 τ-retry
    # Adam needs many small optimizer steps for momentum to stabilize
    # Larger batches do not substitute — fewer Adam steps = worse convergence
    print(f"  ⚠ HIGH τ={tau_b:.2f} — τ-retry 50CE@LR×2")
    opt_lg=torch.optim.AdamW(model.parameters(),lr=LR*2,betas=(0.9,0.95),weight_decay=0.1)
    for _s in range(50):
        lr_s=LR*2*0.5*(1+math.cos(math.pi*_s/50))
        for pg in opt_lg.param_groups: pg['lr']=lr_s
        model.train(); x,y=get_batch(); _,l=model(x,y)
        opt_lg.zero_grad(); l.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt_lg.step()
    v_basin=eval_val(model); pc_b=phi_clean(model); tau_b=gluing_defect(model)
    step_basin=step_basin+50
    print(f"  After τ-retry (50CE@LR×2): val={v_basin:.4f}  Φ_cl={pc_b}/5  τ={tau_b:.2f}")
print()
...
# Compare with joint 25 CE cosine
...
print(f"  K₀ 25 CE (w_FF={w_ff_k0_2:.2f}): val={v_k0:.4f}")
print(f"  Joint 25 CE:                val={v_joint:.4f}")

if v_k0 <= v_joint:
    model=model_k0; v_final=v_k0
    print(f"  ✓ K₀ wins by {v_joint-v_k0:.4f} nats")
else:
    model=model_joint; v_final=v_joint
    print(f"  ~ Joint wins (K₀ gap={v_k0-v_joint:.4f})")
