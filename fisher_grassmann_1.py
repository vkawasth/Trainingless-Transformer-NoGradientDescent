...
# =====================================================================
# 2. SIGN-PRECONDITIONED KRYLOV-NEWTON OPTIMIZER
# =====================================================================
class AdaptiveKrylovSubspaceSignAdam:
    r"""
    Implements the hybrid update rule with Sign-Preconditioned Orthogonal Complement:
        \Delta \theta = \eta_\parallel (Q_K T_K^{-1} Q_K^T g) + \eta_\perp \text{sign}(m_\perp / \sqrt{v_\perp})
    """
    def __init__(self, params, lr_kn=5e-2, lr_ortho=1e-3, beta1=0.9, beta2=0.999, eps=1e-8, k_dim=3):
    ...
    def step(self, Q_K, T_K=None):
        ...
        # 1. In-Subspace Projection (Q_K Q_K^T g)
        g_parallel = Q_K @ (Q_K.T @ g)

        # 2. Exact Subspace Newton Step: Q_K T_K^{-1} Q_K^T g
        if T_K is not None:
            rhs = Q_K.T @ g
            y = torch.linalg.solve(T_K + 1e-4 * torch.eye(T_K.shape[0], device=g.device), rhs)
            step_kn = Q_K @ y
        else:
            step_kn = g_parallel

        # 3. Orthogonal Complement Separation: g_perp = (I - Q_K Q_K^T) g
        g_perp = g - g_parallel

        # 4. Sign-Preconditioned Orthogonal Complement Update
        ...
        # Coordinate-wise Sign Preconditioning on the Complement
        step_ortho = torch.sign(m_hat / (torch.sqrt(v_hat) + self.eps))

        # 5. Hybrid Combined Update
        total_update = (self.lr_kn * step_kn) + (self.lr_ortho * step_ortho)
        self._set_flat_params(p - total_update)
        return total_update, g, step_kn, step_ortho

# =====================================================================
# 3. UNIFIED FISHER-GRASSMANN MANIFOLD TRACKER (WITH SPIN & TORSION)
# =====================================================================
class UnifiedManifoldTracker:
    ...
    def diagnose(self, step, loss_val, update_window, current_grad, full_update, step_kn, step_ortho):
        ...
        # 1. Rotational Velocities & Infinitesimal Spin Matrix Omega = Q^T \dot{Q}
        rotations = {}
        omega_norms = {}
        if self.prev_frames:
            for b_name in self.block_indices:
                Q_prev = self.prev_frames[b_name]
                Q_curr = current_frames[b_name]
                rotations[b_name] = self.gr_distance(Q_prev, Q_curr)
                
                # Discrete Spin Connection Matrix Omega \in so(k)
                Q_dot = Q_curr - Q_prev
                Omega = Q_prev.T @ Q_dot
        ...
        # 6. Coordinate Sign-Stability Index (S_t)
        if self.prev_grad is not None:
            sign_match = (torch.sign(current_grad) == torch.sign(self.prev_grad)).float()
            S_t = float(sign_match.mean())
        else:
            S_t = 1.0

        # 7. Sign-Augmented Curvature Torsion (\tau_L)
        # Measures ratio of out-of-subspace Hessian leakage to in-subspace projection norm
        P_ortho = torch.eye(P_total, device=current_grad.device) - Q_glob @ Q_glob.T
        ortho_step_leakage = P_ortho @ step_ortho
        in_subspace_norm = torch.linalg.norm(Q_glob @ (Q_glob.T @ step_kn)) + 1e-30
        tau_L = float((torch.linalg.norm(ortho_step_leakage) / in_subspace_norm) ** 2)
...
def main():
    ...
    opt = AdaptiveKrylovSubspaceSignAdam(params, lr_kn=5e-2, lr_ortho=1e-3, k_dim=K_DIM)
    tracker = UnifiedManifoldTracker(model, block_indices, k_dim=K_DIM)

    print("=" * 112)
    print(f" INTEGRATED FISHER-GRASSMANN MANIFOLD OPTIMIZATION (P={P}, k={K_DIM})")
    print(" Engine: Subspace Krylov-Newton + Sign-Preconditioned Orthogonal Complement")
    print("=" * 112)
    print(f"{'Step':>6} | {'Loss':>8} | {'Gamma (γ)':>10} | {'Rho_F (ρ)':>10} | {'Sign S_t':>8} | {'Torsion τ_L':>11} | {'Rot ATTN0':>10} | {'||Ω||_F':>8}")
    print("-" * 112)
    ...
    for step in range(1, 121):
        ...
        # Step includes Sign-Preconditioned complement
        full_update, g_flat, step_kn, step_ortho = opt.step(Q_K, T_K)
        ...
        if step >= WINDOW_SIZE and step % 10 == 0:
            diag = tracker.diagnose(step, loss_val, update_window, g_flat, full_update, step_kn, step_ortho)
            Q_K = diag["Q_glob"]

            rot_val = diag["rotations"].get("ATTN0", 0.0)
            omega_val = diag["omega_norms"].get("ATTN0", 0.0)

            print(f"{step:6d} | {loss_val:8.4f} | {diag['gamma']:10.4f} | {diag['rho_F']:10.4f} | {diag['S_t']:8.4f} | {diag['tau_L']:11.4e} | {rot_val:10.4f} | {omega_val:8.4f}")
