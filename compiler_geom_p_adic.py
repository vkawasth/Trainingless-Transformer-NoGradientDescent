#!/usr/bin/env python3
"""
p-adic Basin Movement Compiler
==============================
Moves in the basin using p-adic geometry.
No gradient descent - direct alignment of:
  1. 2-adic: embedding layer (rare tokens → centroids)
  2. 3-adic: FF layers (syntactic structure)
  3. 5-adic: attention layers (clean orbit {0, π} alternating)
...
# p-adic parameters
RARE_TOKEN_PERCENTILE = 10  # Bottom 10% frequency = v₂ ≥ 3
P_ADIC_PRIMES = [2, 3, 5]
...
# p-ADIC COMPUTATIONS
# ============================================================================
def padic_valuation(x, p=2, eps=1e-10):
...
def get_rare_tokens(token_counts, percentile=RARE_TOKEN_PERCENTILE):
    """Get tokens with low frequency (high 2-adic valuation)."""
...
def compute_token_frequencies():
    """Compute token frequencies from corpus."""
    token_counts = {}
    for token in train_ids:
        token_counts[token] = token_counts.get(token, 0) + 1
    return token_counts

def compute_syntactic_centroids(model, token_counts, rare_tokens):
    """
    Compute 3-adic centroids for syntactic structure.
    Uses the embedding gradients of rare tokens.
    """
    # Compute embedding gradient (2-adic direction)
    model.zero_grad()
    # Build a batch with rare tokens
    rare_batch = []
    ...
    direction_2adic = torch.zeros_like(model.te.weight)
    for t in rare_tokens:
        direction_2adic[t] = emb_grad[t] * weights[t]
    
    # 3-adic: syntactic centroids from FF layers
    # The FF layers project from hidden to output
    # The syntactic centroids are the FF output projections
    syntactic_centroids = []
    for l in range(N_STU):
        # The FF output projection is ff.o.weight
        # We want to align it with the 2-adic direction
        ff_o = model.blocks[l].ff.o.weight.data.clone()
        # Project the 2-adic direction onto the FF output space
        # This is the 3-adic centroid
        centroid = ff_o + 0.1 * direction_2adic.flatten()[:ff_o.numel()].reshape(ff_o.shape)
        syntactic_centroids.append(centroid)
    
    return syntactic_centroids, direction_2adic

def compute_orbit_alignment(model):
    """
    Compute 5-adic orbit alignment.
    The clean orbit is {0, π} alternating.
    """
    angles = sheet_angles(model)
    clean_orbit = []
    for i, a in enumerate(angles):
        if not np.isnan(a):
            # Move toward 0 or π
            if abs(a) < ORBIT_TOLERANCE:
                clean_orbit.append(0.0)
            elif abs(abs(a) - math.pi) < ORBIT_TOLERANCE:
                clean_orbit.append(math.pi)
            else:
                # Closest to 0 or π
                if abs(a) < abs(a - math.pi):
                    clean_orbit.append(0.0)
                else:
                    clean_orbit.append(math.pi)
        else:
            clean_orbit.append(float('nan'))
    
    return clean_orbit

# ============================================================================
# p-ADIC BASIN MOVEMENT
# ============================================================================
# ── FIXED: p-ADIC BASIN MOVEMENT ──
def p_adic_basin_move(model, token_counts, rare_tokens):
    """
    Move in the basin using p-adic geometry.
    No gradient descent - direct alignment.
    """
    print("\n━━━ p-ADIC BASIN MOVEMENT ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("  2-adic: embedding alignment (rare tokens)")
    print("  3-adic: FF alignment (syntactic structure)")
    print("  5-adic: attention alignment (clean orbit)")
    
    ce_used = 0
    
    # Step 1: Compute 2-adic direction (embedding)
    print("\n  [1] 2-adic: Embedding alignment")
    model.zero_grad()
    
    # Build batch with rare tokens
    rare_batch = []
    for _ in range(BATCH):
        found = False
        for _ in range(50):
    ...
    # Apply 2-adic alignment to embedding layer
    model.te.weight.data += 0.1 * direction_2adic
    ce_used += 1
    
    print(f"      Applied 2-adic alignment to embedding layer")
    print(f"      Direction norm: {direction_2adic.norm().item():.4f}")
    
    # Step 2: 3-adic alignment (FF layers)
    print("\n  [2] 3-adic: FF layer alignment")
    
    syntactic_centroids = []
    for l in range(N_STU):
        ff_o = model.blocks[l].ff.o.weight.data.clone()
        proj_size = min(ff_o.numel(), direction_2adic.numel())
        direction_flat = direction_2adic.flatten()[:proj_size]
        direction_reshaped = direction_flat.reshape(ff_o.shape)
        alpha = 0.05
        model.blocks[l].ff.o.weight.data += alpha * direction_reshaped
        syntactic_centroids.append(model.blocks[l].ff.o.weight.data.clone())
    
    ce_used += N_STU
    print(f"      Aligned {N_STU} FF layers")
    
    # Step 3: 5-adic alignment (attention layers)
    print("\n  [3] 5-adic: Attention orbit alignment")
    
    angles = sheet_angles(model)
    print(f"      Current orbit: {[f'{a:.2f}' if not np.isnan(a) else '?' for a in angles]}")
    ...
    # Align WK layers to clean orbit using proper rotation
    for l in range(N_STU - 1):
        if not np.isnan(clean_orbit[l]):
            WK = model.blocks[l].attn.WK.weight.data.clone()
            
            # Get the current angle
            current_angle = angles[l]
            if not np.isnan(current_angle):
                target_angle = clean_orbit[l]
                delta = target_angle - current_angle
                
                # Create a rotation matrix
                # For a simple rotation in the WK space, we rotate the weight matrix
                # using the delta angle. Since WK is a linear transformation,
                # we apply a small rotation to its singular vectors.
                
                # Method: Add a small perturbation in the direction of the clean orbit
                # Use the WK matrix itself to define the perturbation direction
                # The perturbation is: delta * (WK * rotation_angle)
                rotation_factor = 0.01 * delta / max(abs(delta), 0.1)
                
                # Create a perturbation matrix
                # We want to rotate WK toward the clean orbit
                # A simple way: add a small multiple of WK multiplied by sign(delta)
                perturbation = rotation_factor * WK
                model.blocks[l].attn.WK.weight.data += perturbation
    
    ce_used += N_STU - 1
    print(f"      Aligned {N_STU - 1} attention layers")
    ...
    print(f"      After p-adic movement: val={v_new:.4f}, phi={phi_new}/5, tau={tau_new:.2f}")
    
    return model, v_new, tau_new, phi_new, ce_used
...
# ============================================================================
# p-ADIC BASIN MOVEMENT ACTION
# ============================================================================
def action_p_adic_basin_move(state):
    """
    p-adic basin movement action.
    """
    model = state.model
    ...
    token_counts = compute_token_frequencies()
    rare_tokens = get_rare_tokens(token_counts)
    
    # Apply p-adic basin movement
    model_try = copy.deepcopy(model)
    model_try, v_new, tau_new, phi_new, ce_used = p_adic_basin_move(
        model_try, token_counts, rare_tokens
    )
    
    print(f"\n    p-adic basin movement complete:")
    ...
    if v_new < v0_val * 0.95:
        return CompilerState(model_try, state.step + 1, 'p_adic_basin_move',
                            v_new, phi_new, tau_new,
                            state.action_history + ['p_adic_basin_move'],
                            parent=state)
...
def get_smart_actions(state, analyzer):
    ...
    # p-ADIC BASIN MOVEMENT: when near floor and in orbit
    if near_floor and phi >= 4 and val < 0.2:
        actions.append(('p_adic_basin_move', {}))
        print(f"    → p_adic_basin_move: val={val:.4f}, phi={phi}/5")
...
# ============================================================================
# SMART DP COMPILER WITH p-ADIC BASIN MOVEMENT
# ============================================================================
class SmartDPCompiler:
    ...
    def apply_action(self, state, action_name, params):
        ...
        elif action_name == 'p_adic_basin_move':
            return action_p_adic_basin_move(state)
        return None
    
    def run(self):
        print("=" * 70)
        print("p-ADIC BASIN MOVEMENT COMPILER")
        print("2-adic → 3-adic → 5-adic alignment")
        print("Target: 0.0147 in ≤20 CE")
        print("=" * 70)
        print()
...
def main():
    ...
    if final_state.val > 0.1 or final_state.phi < PHI_CLEAN_TARGET:
        ...
        torch.save(final_state.model.state_dict(), 'p_adic_basin_gd_final.pt')
        print(f"\n  Saved: p_adic_basin_gd_final.pt")
    else:
        torch.save(final_state.model.state_dict(), 'p_adic_basin_final.pt')
        print(f"\n  Saved: p_adic_basin_final.pt")
