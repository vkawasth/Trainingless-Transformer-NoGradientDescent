...
DONORS=[40,120]; EVAL=[0,1,2,5,10,20,30]; LAYER=3
...
DELTA=float(sys.argv[2]) if len(sys.argv)>2 else 0.095
_n3ref=math.sqrt(sum(float((TSTATE[120][n]**2).sum()) for n,_ in tnamed if n.startswith(pre)))
nasc=0
while nasc<6000:
    if nasc%5==0:
        with torch.no_grad():
            _cur=math.sqrt(sum(float(((p.data-TSTATE[120][n])**2).sum())
                               for n,p in tnamed if n.startswith(pre)))
        if _cur/max(_n3ref,1e-30)>=DELTA: break
...
print(f"  uniform expectation for one of {_nb} blocks: {1/math.sqrt(max(_nb,1)):.3f}",
      flush=True)
print(f"  delta={DELTA:.3f} teacher UNLEARNED from 120: {nasc} steps, L {Lu:.4f}  O "
...
SPEC["WU"]=("U","W"); SPEC["WU3"]=("U","W"); SPEC["S120"]=(120,"S"); SPEC["WS120"]=(120,"WS")
d,what=SPEC[ARM]
