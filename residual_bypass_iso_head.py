#!/usr/bin/env python3
"""
ACTIVATION COVARIANCE ALIGNMENT WITH CALIBRATED LINEAR HEADS
============================================================
Fixes the absolute loss inflation by isolating the student's output classification
head from the teacher's static norm scales. This allows the linear projection field
to map the stabilized high-rank variety cleanly to the target vocabulary.
...
print(f"\n{'='*75}")
print(f"  CALIBRATED RESIDUAL ACTIVATION ALIGNMENT ENGINE")
...
def evaluate_calibrated_head(student_model, T_edit_matrix, steps=HEAD_STEPS):
    # FIX: Initialize fresh head using standard internal scales instead of forcing teacher weight norms
