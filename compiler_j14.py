...
# ── SADDLE EXIT (required before K₀ — stage2c starts from post-saddle state)
print("━━━ SADDLE EXIT (before K₀, matches stage2c confirmed setup) ━━━")
print("  stage2c_step_reduction.py starts from val≈3.45 (post-saddle)")
print("  K₀ from val=4.47 (no saddle) only reaches val=3.39 not 2.54")
model.zero_grad()
ls=[model(*get_batch())[1] for _ in range(8)]; torch.stack(ls).mean().backward()
g_sad=torch.cat([p.grad.flatten() if p.grad is not None else torch.zeros(p.numel())
                 for p in model.parameters()]).detach(); model.zero_grad()
v_neg_sad=-g_sad/g_sad.norm()
w0_sad=model.flat_params(); best_v_sad=eval_val(model); best_a_sad=0.
for alpha in [0.5,1.0,1.43,2.0,3.0]:
    model.set_flat(w0_sad+alpha*v_neg_sad); vt=eval_val(model,n=6)
    if vt<best_v_sad: best_v_sad=vt; best_a_sad=alpha
model.set_flat(w0_sad+best_a_sad*v_neg_sad)
v_saddle=eval_val(model)
print(f"  Saddle exit α*={best_a_sad:.2f}: val={v_saddle:.4f}  (target ≈3.45)")
print()

print(f"━━━ PHASE K₀: {N_K0} BRANCHED CE (Emb+FF / Attn) ━━━━━━━━━━━━")
print(f"  Confirmed: 13 K₀ CE = 25 joint CE (stage2c_step_reduction.py)")
print(f"  K₀ split removes Emb-FF interference → each group converges")
...
print("━━━ PHASE Φ: DRINFELD SCALAR CORRECTION (Φ_KZ) ━━━━━━━━━━")
print(f"  W_FF* = (w_FF × Φ_KZ) × ΔFF  [scalar Drinfeld, not matrix bracket]")
print(f"  Φ_KZ = 1 + ζ(2)κ²/2 + ζ(3)κ³/6  (KZ associator leading terms)")
print(f"  Matrix bracket [[X,[X,Y]]-[Y,[X,Y]]] excluded: X,Y don't satisfy KZ")
print(f"  relations here (E₀ subspace ≠ abstract Lie generators)")

# K₀ already applied: params_post[FF] = params_pre[FF] + w_ff × delta_FF
# Drinfeld adds the RESIDUAL correction only: (Φ_KZ - 1) × w_ff × delta
# = [ζ(2)κ²/2 + ζ(3)κ³/6] × w_ff × delta (second-order terms)
phi_residual = phi_kz - 1.0   # ≈ 0.2898 for w_FF=3.5
print(f"  Φ_KZ residual = Φ_KZ - 1 = {phi_residual:.4f}  [correction only]")
print(f"  ΔW_FF_correction = {phi_residual:.4f} × {w_ff_use:.2f} × ΔFF")

model_phi = copy.deepcopy(model)  # starts from K₀ result
total_corr = 0.0
with torch.no_grad():
    for name, p in model_phi.named_parameters():
        if ptype(name) != 'FF': continue
        # delta = K₀ FF update = params_post - params_pre
        delta_ff = params_post[name] - params_pre[name]
        # Add Drinfeld residual on top of existing K₀ update
        corr = phi_residual * w_ff_use * delta_ff
        p.data.add_(corr)   # model_phi already has K₀ update; just add correction
        total_corr += float(corr.norm())

v_phi = eval_val(model_phi)
phi_phi = sheet_angles(model_phi.blocks)
print(f"  ||ζ(3) correction|| total = {total_corr:.6f}")
print(f"  After Φ correction: val={v_phi:.4f}  Φ={phi_phi}")

# Use corrected model if it's better, else keep K₀ result
if v_phi < v_k0:
    model = model_phi
    print(f"  ✓ Drinfeld correction improves: {v_k0:.4f} → {v_phi:.4f}")
else:
    print(f"  ~ Drinfeld correction neutral ({v_phi:.4f} vs {v_k0:.4f}), keeping K₀")
print()
