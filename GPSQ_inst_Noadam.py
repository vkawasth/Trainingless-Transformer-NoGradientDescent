"""
ADAM-LESS QUOTIENT SPACE TRAJECTORY TRACKER (smallgps_adamless.py)

Reconstructs Adam-like adaptive momentum and precondition scaling using pure
tensor linear algebra inside the Quotient Space tracking framework, completely
bypassing PyTorch's native optimizer loops.
"""
...
# Manual Adam-like Hyperparameters
LR = 3e-3
BETA1 = 0.9
BETA2 = 0.95
EPS = 1e-8
...
def run_adamless_tracker(theta=0.5, seed=0):
    ...
    # --- MANUAL ADAM-LIKE STATE TENSORS ---
    P_total = sum(p.numel() for p in ps)
    m_first = torch.zeros(P_total)  # Exponential First Moment
    v_second = torch.zeros(P_total)  # Exponential Second Moment
    ...
    print("=" * 80)
    print("RUNNING MANUAL ADAM-LIKE MATH IN QUOTIENT TRACKER (No optimizer instance)")
    print("=" * 80)
    ...
    for st in range(STEPS):
        ...
        if st < DETECT:
            ...
            for r, ii in idx.items():
                ...
                # Quotient vector w.r.t base frame Q[r]
                pj = Q[r] @ (Q[r].T @ gi)
                q_i = gi - pj
        ...
        # Weight Decay Gradient Augmentation
        g = g + WD * th

        # --- ADAM-LIKE MATH (MANUAL TENSOR ARITHMETIC) ---
        t_step = st + 1
        
        # 1. Moment Updates
        m_first = BETA1 * m_first + (1.0 - BETA1) * g
        v_second = BETA2 * v_second + (1.0 - BETA2) * (g * g)
        ...
        # 3. Preconditioned Step Direction (Adam-like Direction)
        du_raw = -LR * (m_hat / (torch.sqrt(v_hat) + EPS))

        if full and used < CAP:
            used += 1
            for r, ii in idx.items():
                recent[r].append(du_raw[ii].clone())
            ...
            # Subspace Project Preconditioned Step Direction
            nu = torch.zeros_like(du_raw)
            for r, ii in idx.items():
                nu[ii] = Q[r] @ (Q[r].T @ du_raw[ii])
            setth(th + nu)
...
# --- Execution ---
run_adamless_tracker(theta=0.5, seed=0)
