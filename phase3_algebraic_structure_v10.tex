...
\emph{Extended in \S\ref{sec:regime-test}}: the same gain, measured on the
authentic corpus at five checkpoints from the drift-dominated regime to the
memorised minimum, never approaches zero. The finding holds across corpora and
across regimes.
\end{observation}
...
\emph{Limitations.} Two seeds, one checkpoint at $80$ steps, one architecture.
A subsequent audit (\S\ref{sec:adequacy}) established that at this checkpoint the
model has learned only token frequencies on \texttt{english} and reaches its
corpus floor only on \texttt{code} and \texttt{repeat}, so four of the six
corpora were probed on a representation close to unigram. That concern is
\emph{superseded} by \S\ref{sec:regime-test}, which reruns the identical
probe at five checkpoints spanning val $3.0\to0.07$ on the authentic corpus
and finds the gain invariant in $[-0.171,-0.092]$. The result is
corpus-general and regime-general. The negative result is a statement
...
\emph{Resolved in \S\ref{sec:regime-negative}}: the deterministic population
is real but early. On the programme's own corpus $35.5\%$ of block
coordinates exceed $r=0.5$ at step $10$, falling to $0.9\%$ by step $200$,
with $\mathrm{corr}(\text{val},r_{90})=+0.749$ pooled across corpora. This
$80$-step checkpoint sits at the depleted end, so the two-phase prediction was
tested in the one regime where it has least purchase.
\end{observation}
...
dominates and positional identity modulates. \emph{None of the conditions in
this table is matched on surface perturbation}; \S\ref{sec:matched} imposes
that control and finds the resulting structural term to be zero early in
training, so the orderings here should be read as surface-distance orderings
and not as evidence about symmetry.
\end{observation}
