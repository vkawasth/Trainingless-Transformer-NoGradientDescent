...
def forward_pass(token_ids, WQ, WK, WV, WO, WE, D, n_layers=24):
    """
    Simplified GPT2 forward pass (no positional encoding, single head).
    Returns hidden states at each layer: dict[layer] → H [D, T]
    """
    T = len(token_ids)
    if WE is None:
        # No embedding — use random init
        rng = np.random.RandomState(42)
        H = rng.randn(D, T).astype(np.float32) * 0.1
    else:
        # Lookup embeddings: H = WE[token_ids]^T  [D, T]
        H = WE[token_ids].T.astype(np.float32)  # [D, T]

    hidden = {-1: H.copy()}  # layer -1 = input embeddings
    ...
    for k in range(n_layers):
        ...
        if wv is not None and wo is not None:
            V  = wv @ H   # [D, T]
            # Attention: A = softmax(Q^T K / sqrt(D))  [T, T]
            scale = np.sqrt(D)
            scores = (Q.T @ K) / scale   # [T, T]
            # Causal mask
            ...
            A = np.exp(scores)
            A /= A.sum(axis=1, keepdims=True)   # [T, T]
            # Context
            ctx = V @ A.T   # [D, T]
            H = H + wo @ ctx   # residual
        else:
            H = H + K  # fallback: just add K

        # LayerNorm (simplified: mean-center and scale)
        H = H - H.mean(axis=0, keepdims=True)
        H = H / (H.std(axis=0, keepdims=True) + 1e-5)
        hidden[k] = H.copy()

    return hidden

# ── 3. Lagrangians and intersection score ─────────────────────────────────────
def query_lagrangian(WQ_k, dim):
    U, _, _ = svd(WQ_k, full_matrices=False)
    return U[:, :dim]

def memory_lagrangian(WK_k, H, dim):
    """U_K(x) = top-dim left singular vecs of W_K^(k) @ H(x)."""
    K = WK_k @ H                      # [D, T]
    d = min(dim, K.shape[1])
    U, _, _ = svd(K, full_matrices=False)
    return U[:, :d]
...
def intersection_score(UQ, UK):
    ...
    angles = np.arccos(sv)
    return float(angles[0]), float(np.mean(angles)), angles

# ── 4. Tokenizer (simple character-level for demo; use tiktoken for real) ─────
def simple_tokenize(text, vocab_size=50257, seed=0):
    """
    Simple pseudo-tokenizer: maps words to token IDs.
    For real use, replace with tiktoken or GPT2Tokenizer.
    """
    words = text.lower().split()
    rng = np.random.RandomState(seed)
    # Consistent hash-based IDs
    ids = [hash(w) % vocab_size for w in words]
    return ids[:32]  # max 32 tokens
...
def run_real(WQ, WK, WV, WO, WE, D, layers, n_layers):
    ...
    results = {}
    UQ_cache = {k: query_lagrangian(WQ[k], DIM) for k in layers if k in WQ}
    ...
    for k in sorted(layers):
        ...
        for pt in prompt_types:
            ...
            for prompt in PROMPTS[pt]:
                tok = load_tiktoken(prompt)
                if WE is not None and max(tok) >= WE.shape[0]:
                    tok = [t % WE.shape[0] for t in tok]
                hidden = forward_pass(tok, WQ, WK, WV, WO, WE, D, n_layers)
                H = hidden.get(k, hidden.get(k-1, np.random.randn(D,8)))
                UK = memory_lagrangian(WK[k], H, DIM)
                rho_min, rho_mean, _ = intersection_score(UQ_cache[k], UK)
                rhos.append(rho_min)
...
def run_synthetic():
    ...
    UQ = {k: query_lagrangian(WQ[k], DIM) for k in layers}
    for k in layers:
        rhos = {}
        for pt in prompt_types:
            rs = []
            for seed in range(5):
                ...
                if pt=='factual':
                    ...
                    H = rng2.randn(D, 6)
                UK = memory_lagrangian(WK[k], H, DIM)
                rho,_,_ = intersection_score(UQ[k], UK)
                rs.append(rho)
